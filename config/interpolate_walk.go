...
func hasUnknownValue(variable ast.Variable) bool {
	switch variable.Type {
	case ast.TypeString:
		return variable.Value == UnknownVariableValue
	case ast.TypeList:
		for _, value := range variable.Value.([]ast.Variable) {
			if value.Type == ast.TypeString {
				return value.Value == UnknownVariableValue
			}
		}
		return false
	default:
