...
**Audience:** Terraform Core engineers, architects, and agents  
...
### 0.1 Fundamentals of Architectural Reasoning
...
This paper's method rests on one claim: **a proposed feature cannot
conflict with an established architectural premise without resolution, even if
it is not clear what the failure case might be.** The reviewer's job is to
identify the premise and show where the proposal conflicts with it. The
designer's job is to respond in one of three ways — demonstrate that the
premise still holds, replace it along with the contracts that depend on it, or
define a separate contract with an explicit boundary. Whichever route the
design takes, a changed guarantee has to be visible to the people relying on
it.
...
Documentation references use the supplied Terraform **v1.16.x** and Plugin
...
A more apt description is **data flow**. In a data-flow language a program is a
network rather than a sequence: named values are the nodes, dependencies
between them are the edges, and a node computes once its inputs are available.
Nothing runs because it appears earlier in a file, and the runtime derives the
schedule from the network instead of from the text. Terraform's configuration
is such a network. Expressions say how values are obtained and combined,
references say which values depend on which, and Core reads the references to
decide what may be evaluated when.

### 1.2 References

A **reference** names a subject elsewhere in the configuration. It is the only
construct that creates a dependency, and it is worth separating carefully from
the value that subject happens to hold.

In `aws_instance.example.id` the reference is `aws_instance.example`; the
trailing `.id` is a traversal that selects information out of whatever value
that subject has. Only the first part creates a dependency, which is why the
...
References are found by reading syntax, before anything is evaluated — HCL can
report the traversals in an expression without computing it. Core depends on
this, because it must build the dependency graph before it can evaluate the
expressions that graph orders. The consequence shows up most clearly in
conditionals: `var.enabled ? aws_instance.a.id : "none"` depends on
`aws_instance.a` whatever `var.enabled` turns out to be, because static
analysis sees both branches. James Bardin explains how this feeds graph
construction in [Terraform issue #21953][static-references].

### 1.3 Evaluation Order

A declarative language has no explicit control flow — no statement sequence, no
loop that iterates, no branch that transfers control. Terraform nonetheless has
several constructs that look like control flow, so it is worth saying precisely
what they do instead.

They compute values. A conditional expression selects between two values. A
`for` expression builds one collection from another. A `dynamic` block builds
nested configuration, and `for_each` on a resource or module selects which
instances exist. Each of these changes the work Terraform has to do; none of
them tells Terraform in what order to do it.
...
`depends_on` reads like an ordering statement, which makes it the apparent
counterexample to P2. It is instead the clearest illustration of it: a
reference that supplies no value (§1.2), and therefore an application of the
...
### 1.4 Single Assignment and Bounded Evaluation

Two further properties of the model are easy to overlook, because nothing in
the language draws attention to them.

Every named object has exactly one value for the duration of an evaluation.
There is no assignment statement and no way to rebind a name, so a local value,
an input variable, or a resource attribute cannot mean one thing in one part of
the configuration and something else in another. That is what makes a
reference's meaning independent of where it appears, and therefore what makes
the dependency graph well defined at all.

Evaluation is also bounded. A configuration author cannot define a function,
there is no recursion, and the only repetition is over a collection that must
already have been computed. Expression evaluation terminates, and the
dependency graph must be acyclic — Core reports a cycle as an error rather than
choosing some order to break it. A feature that introduced recursion, or that
let a value be observed and then revised, would remove a property the rest of
the system quietly relies on.

### 1.5 A Useful Analogy
...
### 1.6 Language-Design Preferences
...
The scope of that claim matters, because planning is not done in isolation. A
planning run can refresh managed resources, read data sources, and open and
close ephemeral resources. Initialization and state operations have effects of
their own, and a few functions read external inputs or produce unpredictable
results and need special handling (§5.3.4). P3 draws a line between expression
computation and those operations. It does not confine every external
...
delete, replace, and no-op. Replacement carries an ordering choice with it as
well, normally delete-then-create or create-then-delete.
...
`Renew`, and `Close` for ephemeral resources. CRUD (create, read, update,
delete) is a good introduction to the model for providers, but a poor inventory
of it, so it is the job of the Plugin Framework to map the RPCs for Terraform's
actions onto the more familiar calls the provider's remote API offers.
...
are allowed to happen. Atkins describes a plan as an overview of the high-level
actions Terraform intends to take *and* a partial description of the inputs to
those operations; the separation drawn here is this paper's reading of that
pair. [Source: [*Unknown Values*][unknown-values].]
...
Both rules are checked at Terraform's own boundaries. Core can compare the
values a provider returns against the values it promised. It cannot verify that
the provider made the right API calls to get there. A plan describes
Terraform-level change, not a trace of provider implementation.
...
**Drift** is any change Terraform sees when it re-evaluates a plan against the
most recently applied configuration: the remote object no longer matches what
the last apply recorded, and nothing in the configuration accounts for the
difference. The term describes an observation, not a cause.
...
— surfaces as drift at the next refresh, however convergent the previous apply
was. Refresh is a fresh observation, not a retroactive verdict on the earlier
run.
...
This is HCL's own design position, not a Terraform convention layered on top:
HCL is a structured configuration language rather than a serialization format,
and is always decoded using an application-defined schema. A `Body` is a
logical object rather than a physical one, which is what lets the same decoding
machinery serve two syntaxes and extensions such as `dynamic` blocks.
[Sources: [HCL's introductory guide][hcl-guide];
[the HCL specification][hcl-spec].]
...
chiefly for generators and tools. Because a JSON object is always just an
object, the surrounding Terraform schema decides whether a given object
represents a Terraform value or nested configuration; the notation alone cannot
say.
...
Starting with v1.16, Terraform allows module `source` and `version` to use
constant expressions built from local values and from input variables declared
`const = true`:
...
Terraform represents values and types with the [`cty`][cty] library. Beyond the
concrete values common to most programming languages, the model adds four
things, each of which appears in expression evaluation and in the contracts
between Core and providers:

- A **null** is the absence of a value, and it still has a definite type.
  `null` of type `string` is a different value from `null` of type `number`.
- An **unknown** stands for a typed value that will be known later. The type is
  available now; the value is not.
- A **refinement** records what is already known about an unknown — a string
  prefix, a collection length, the fact that it will not be null.
- A **mark** carries metadata alongside a value, saying how the value must be
  handled rather than what it is.
...
Lists, sets, and maps hold any number of elements of one common type. Objects
and tuples hold elements of possibly different types — named attributes for
objects, positional elements for tuples.
...
This structural treatment of object types is worth understanding historically,
because the usual justification for it came second. Atkins' original conversion
rules were stricter; attribute-dropping was adopted during v0.12 as a
compatibility compromise for modules that had been faking objects with map
types, since Terraform before v0.12 had no object types at all. The benefit
usually cited now — that a producer and a consumer of an argument can evolve
independently, the consumer accepting more by adding optional attributes and
the producer providing more without breaking anyone — was a secondary effect
that has since become the primary one. Treating it as a first-principles design
choice overstates the case.
[Source: [Atkins on object type conversion][object-typing].]
...
them is null — a distinction that matters whenever a provider has to
distinguish "the author set this to empty" from "the author said nothing." At
an argument boundary, null normally means no value was supplied, as interpreted
by that context's schema, defaults, and conversion rules.
...
`startswith(arn, "arn:aws:s3")` already evaluates to `true`.

Refinements compose but do not imply one another. A prefix says nothing about
nullness, so `arn != null` follows only if the producer also applied the
not-null refinement; cty keeps the two as independent fields. Terraform's
functions commonly apply both, which makes it easy to mistake one for a
...
Two structural limits are worth knowing before designing against refinements.
They attach to values rather than to type constraints, so they cannot nest: an
unknown map cannot carry refinements describing its hypothetical elements,
because there are no element values yet for those refinements to attach to. And
a value of `cty.DynamicPseudoType` cannot be refined at all, since an unknown
value of an unknown type is entirely unconstrained.
...
A mark travels with a value and says how the value must be handled. Unlike a
refinement, a mark narrows nothing about what the value might be; it is
metadata attached to the value, carried along by evaluation. Terraform uses
several — for sensitivity, for ephemerality, and for deprecation.
...
Propagation must also be consistent across knownness. A mark on an unknown
value must survive that value becoming known, so the known result carries the
same mark the unknown one did. The risk runs the other way: an unknown value
can arrive without a mark it should have carried, and the mark is then lost for
good. Whether a mark is present is itself always known, never unknown, and
Terraform takes care to keep it that way.
[Sources: [`internal/lang/marks`][marks];
[sensitive-data handling][sensitive-data].]

Sets need particular care, for a reason rooted in how marks compose. A set's
elements are identified by their values, so a mark on one element cannot stay
attached to that element alone; it is hoisted to the containing set. Mark one
password inside a set of credential objects and the whole set becomes
sensitive. Set element correlation adds further constraints (§10.3).
...
it cannot be recovered from the saved plan.
...
Set hoisting is tolerable for disclosure but unacceptable here, because a
write-only attribute needs per-attribute nulling in the persisted object and a
hoisted mark identifies only the whole set. The schema validator therefore
rejects write-only attributes inside set blocks and nested set attributes
outright.
[Source: [`configschema/internal_validate.go`][schema-validation].]

#### 5.5.3 Deprecated

**Deprecation** marks carry a message and a description of where the
deprecation originated, which is what lets a diagnostic name the responsible
declaration rather than the expression that happened to consume the value.
Core applies the mark to deprecated module output values, deprecated input
variables, and provider schema attributes and blocks flagged as deprecated.

Tracing deprecation through marks rather than through a separate analysis is
what makes it work across module boundaries: a deprecated output consumed three
modules away still arrives marked, so the warning can be raised at the point of
use.
[Sources: [`marks.DeprecationMark`][marks];
[`internal/deprecation/schema.go`][deprecation-schema].]
...
| Deprecation mark | The value came from a declaration whose author has marked it for removal. |
...
The static address is a consequence of a lifetime requirement, not the reason
for it. A provider configuration is needed to destroy an object as well as to
create or update it, so it must remain available for longer than every resource
instance it manages. A module that declares its own provider configurations
cannot satisfy that if the module call is expanded: removing one instance of
the module would remove the resource instances and the provider configuration
that must destroy them at the same moment, leaving nothing able to perform the
destroy. Terraform records which provider configuration last applied each
resource precisely so that it can still be located after the `resource` block
is gone, and an expanded module defeats that.

Provider validation therefore rejects `count`, `for_each`, and `depends_on` on
a module containing `provider` blocks, and names it a legacy module in the
diagnostic. The lifetime argument accounts for `count` and `for_each` directly;
`depends_on` was restricted alongside them when module expansion was introduced
in v0.13, because the implementation of these features conflicted with the
legacy pattern. Reusable modules should take provider configurations from their
caller through the supported association mechanism instead (§13.3).
...
so adding one obliges the evaluation context to resolve it. Referenceable
addresses are strictly within the scope of the caller's module. `Targetable`
...
Graph vertices are a wider category than addresses. Some nodes, such as
expansion nodes, are proxies for configuration constructs, and others, such as
"close" nodes, take part in execution without being anything a configuration
can name.
...
results in state. Removal is part of the same story: it has its own ordering
requirements and its own demands on how long information must survive.

## 7. Graph Construction and Execution

The graph is a derived artifact: Core builds one for each operation out of
configuration, state, and run options, walks it, and discards it. Nothing
persists a graph, and no part of one is a stable interface — which is what
leaves the construction pipeline free to change while the language it
implements stays fixed.

Construction and execution are separate concerns, and keeping them separate
prevents most confusion about Terraform's ordering. Construction decides what
the vertices are and which edges connect them. Execution decides only when a
vertex becomes eligible to run. Reading an edge as an instruction to the walker
rather than as a statement about dependency is the single most common way to
get the destroy ordering backwards.
...
validation stages should treat it.

Cycles make that concrete. A cycle is a property of the finished graph, so an
edge drawn by a lifecycle transformer closes a loop just as surely as one drawn
from a reference — and the resulting error reports a cycle the configuration
author never wrote and often cannot fix by editing references. A transformer
that adds edges cannot reason about them locally.

The common act of deleting a resource block shows why the graph cannot come
from configuration alone. The object still exists and still has to be
destroyed, so its vertex and its dependency information come from state
instead. Such objects are called **orphans**, a name for what they lack — a
current declaration — rather than for what they still have, which is a managed
identity.

### 7.2 Dependency Direction

Edges are added to represent a dependency of some sort, not a global execution
order. The walker uses them to decide when a vertex becomes eligible to run,
which makes the two coincide during creation and diverge during destruction.
...
`DiffTransformer` turns each recorded instance change into an operation
vertex. The configuration graph must be rebuilt, because Core has to
re-evaluate all values that were unknown at plan time.
...
is set by `-parallelism`. A dependency constrains readiness and says nothing
...
it works in two steps. Once Core has evaluated the repetition expression for
...
The method names reflect those two steps. `SetModuleCount` and
...
letting the configuration describe a different desired result because
...
| Create then delete | The replacement can become current while the old object remains tracked as **deposed** — still real, still Terraform's responsibility, no longer current (§9.3). |
...
### 9.2 Create Before Destroy

`create_before_destroy` is the only mechanism by which a configuration author
can change the relative ordering of create and destroy operations, and it does
not stay confined to the resource that declares it. Core propagates it to that
resource's dependencies and rewrites the operation graph to match.
`ForcedCBDTransformer` does the propagation and `CBDEdgeTransformer` makes the
corresponding edge changes.

The propagation is usually explained as a consequence of dependency semantics:
if a dependent object must survive until its replacement exists, destroying
something it depends on early would defeat the purpose. That explanation is
correct and incomplete. Propagation is also a requirement of graph
construction, and this is the stronger reason.

Consider `B` depending on `A`, where `B` has `create_before_destroy` and `A`
does not. Enabling the behavior for `B` means reversing the edges incident to
`B destroy`, so that the destroy happens after the create rather than before
it. But `A destroy` still has its original ordering, and the reversed edges now
run in both directions between the two subgraphs. The result is a cycle.
Reversing the incoming edges to `A destroy` as well resolves it, which is to
say: propagating the reversal through the dependencies is not a policy choice
about semantics but the alternative ordering that leaves the graph acyclic.

The same argument explains why an inherited `create_before_destroy` cannot be
overridden. Turning it off for `A` would restore exactly the edge orientation
that produced the cycle, so there is no configuration in which the override
could be honored. The [resource destruction notes][destroying] work through
these cases with dependency diagrams and are worth reading before changing
lifecycle ordering.

Lifecycle metadata saved in state preserves the behavior after an object loses
its configuration, since the ordering requirement outlives the declaration that
created it. [Source: [`transform_destroy_cbd.go`][cbd-transform].]
...
### 9.3 Object Status: Current, Deposed, and Tainted
...
complete — most often what a create that failed partway through leaves behind.
A tainted object gets planned for replacement rather than accepted as a
...
For planning purposes, Core offers the provider prior state and current
configuration of the resource instance. As a convenience, Core also merges
configuration with prior state into a **proposed new object**. The proposed
new object from Core is always a valid planned result, and the provider can
use that object as the basis for its plan, modify it within the rules of the
protocol, or ignore it entirely and derive its own proposed object solely from
the configuration and prior state.
...
actually remove the nested object instead of restoring the prior one. The
...
Collection type also decides how old elements get matched to new ones. Lists
match by position, maps by key, and a single nested object matches recursively.
Sets have none of these, because a set element's identity is its own value.
Change one field of one element and, as far as the set is concerned, an old
value vanished and an unrelated new one appeared:
...
In order to decide on changes within sets, one must fall back on heuristics.
In Core, `validPriorFromConfig` asks whether a prior element could have come
from a given configuration element with its computed fields filled in, and
pairs them if so. It often works, and it establishes less than a key would.
Provider schema design has to account for which fields constitute element
identity and which are expected to change. The limitation is confined to nested
collections and says nothing about the instance keys of resource `for_each`.
[Source: [Atkins on set element identity][set-identity].]
...
substitute the prior value when the new value presents no functional change —
Core can compare values but has no way to judge semantic equivalence, so it
takes the provider's word. Computed attributes are always set at the provider's
discretion wherever configuration leaves them unset, and write-only attributes
must always be returned as null.
...
Protocol version does not account for all compatibility behavior. The
...
For design work the distinction that matters is between the modern contract and
the compatibility path that preserves older behavior. Both are real, and
neither should be quietly substituted for the other. Additional compatibility
constraints are now defined by adding **Capabilities**. Capabilities can be
declared in the global configuration or on individual RPCs, and in both
directions between Core and the provider, which allows fine-grained handling of
new features alongside legacy behaviors.
...
because a remote API can enumerate its objects without knowing which
...
### 11.3 Refresh and Changes Outside of Terraform
...
hiding that behind the prior value would conceal drift.

Core cannot make that judgment for an arbitrary domain, so `ReadResource`
assigns the responsibility to the provider.
[Source: [resource-instance change lifecycle][resource-lifecycle].]

The planning process maintains two snapshots so it can answer two questions.
`PrevRunState` is what the last run recorded; `PriorState` is what refresh just
observed. The name `PriorState` is unfortunate: during planning it holds the
current state, and only once the plan is formed does it describe a proposed
future in which that state is the prior one. Comparing those two produces the
drift report. Comparing `PriorState` against the desired configuration produces
the proposed changes. Plan output shows both results together, which makes it
easy to forget they answer different questions.
...
region, and a name — and where encoding all three in one import string was
...
namespace (with the exceptions carved out for refactoring and importing), and a
child cannot name arbitrary objects in its caller. The expressions that supply
inputs and consume outputs create the references, and the references create the
dependencies.

`depends_on` on a module call is coarser than it looks. It makes
...
more precise than a whole-module edge. `depends_on` on a module call is never
required, and passing values through inputs and outputs instead avoids both the
extra dependencies and the delayed execution.
[Source: [`depends_on` reference][depends-on-doc].]

The input variable and output value interface is more than a list of names.
Type constraints decide what a module accepts and what survives conversion into
it (§5.1), `nullable` decides whether absence is allowed, and sensitive or
ephemeral declarations carry handling obligations across the boundary. Two
modules compose when these agree, not when their attribute names happen to
match.
...
author can delete from the same file documents an expectation and enforces it
during the run, but it is not an independent approval control.
...
information must not be persisted at all, ephemeral values in particular. The
...
So "Test uses the same runtime" is only partly right. Resource behavior really
is exercised through Core, and setup, assertions, expected failures, and
cleanup are not. A cross-cutting feature has to say which side of that line it
falls on, and how a test can supply inputs to it and observe its outcome.
...
itself a semantic divergence. The comparison worth making is between
contracts — what is evaluated, against which state, what is planned, what is
approved, and what is persisted. Appendix B carries these questions into the
...
Set iteration order shows how far this reaches, and how little it depends on
anyone having made a promise. cty has never guaranteed an order for iterating
the elements of a set, and its documentation says so. Terraform nonetheless
exposes that order to its users through the language, so configurations depend
on it whether or not their authors realize it. When Atkins considered changing
the hashing function, he concluded that cty's callers have in effect imposed a
stricter constraint than cty ever offered: the order must now never change,
even in future versions. An undocumented implementation detail became a
commitment by being observable.
[Source: [go-cty issue #131][set-ordering].]
...
| --- | --- | --- | --- | --- | --- |
| P1 | A | Data Flow | Configuration describes relationships; Core determines evaluation order. | 1.3 | - |
| P2 | A | Reference Order | References alone establish configuration evaluation dependencies. | 1.3 | - |
...
| P14 | P | Ephemeral Non-Persistence | Ephemeral values must not enter persisted plan or state value storage. | 5.5.2 | P13 |
...
will not explain *why* a particular proposal affects a particular contract;
...
checklist organizes an investigation; answering every row does not by itself
certify a design.
...
| External inputs | Filesystem contents and other changing inputs need a lifetime model. P11 concerns refined knowledge of the same computation, not arbitrary re-evaluation against changed inputs. |
...
cover evaluation and handling metadata, with
[`internal/deprecation`][deprecation-schema] applying deprecation marks from
schemas. [`internal/plans/objchange`][object-change-package] contains
proposed-state construction and consistency checks.
...
[HCL][hcl] supplies structural and expression syntax, with its own
[specification][hcl-spec] and [design guide][hcl-guide].
...
prediction, state knownness, and possible deferral.
[Issue #33570][object-typing] records the origin of structural object-type
conversion, and a [Terraform Core forum thread][set-identity] gives the
clearest short account of why set elements have no stable identity. Issue
comments and forum replies provide technical rationale and historical context;
they are not, by themselves, release specifications.
...
[deprecation-schema]: https://github.com/hashicorp/terraform/blob/05cecbb315e87abc2c2bdb182963dd8e36d574fb/internal/deprecation/schema.go
...
[hcl-guide]: https://github.com/hashicorp/hcl/blob/main/guide/intro.rst
...
[object-typing]: https://github.com/hashicorp/terraform/issues/33570#issuecomment-3304103698
...
[set-identity]: https://discuss.hashicorp.com/t/typeset-is-not-picking-up-correct-changes/23735/2
[set-ordering]: https://github.com/zclconf/go-cty/issues/131
