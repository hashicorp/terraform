...
// LegacyProviderInstallation is the structure of the "provider_installation"
// nested block within the CLI configuration.
type LegacyProviderInstallation struct {
...
// legacyDecodeProviderInstallationFromConfig uses the HCL AST API directly to
...
func legacyDecodeProviderInstallationFromConfig(hclFile *hclast.File) ([]*LegacyProviderInstallation, tfdiags.Diagnostics) {
	var ret []*LegacyProviderInstallation
	...
	for _, block := range root.Items {
		...
		for _, methodBlock := range body.List.Items {
			...
			pi.Methods = append(pi.Methods, &LegacyProviderInstallationMethod{
				Location: location,
				Include:  include,
				Exclude:  exclude,
			})
...
// LegacyProviderInstallationMethod represents an installation method block inside
// a provider_installation block.
type LegacyProviderInstallationMethod struct {
