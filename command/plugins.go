...
func (r *multiVersionProviderResolver) ResolveProviders(constraints map[string]semver.Range) (map[string]terraform.ResourceProviderFactory, []error) {
	factories := make(map[string]terraform.ResourceProviderFactory, len(constraints))
	var errs []error

	candidates := r.Available.ConstrainVersions(constraints)
