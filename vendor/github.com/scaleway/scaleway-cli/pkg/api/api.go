...
func (s *ScalewayAPI) PutServer(serverID string, definition ScalewayServerDefinition) error {
	resp, err := s.PutResponse(ComputeAPI, fmt.Sprintf("servers/%s", serverID), definition)
	if resp != nil {
		defer resp.Body.Close()
	}
	if err != nil {
