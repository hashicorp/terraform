...
var httpTransport = http.DefaultTransport // overridden during tests, to skip TLS verification
...
func (d *Disco) discover(host svchost.Hostname) Host {
	...
	client := &http.Client{
		...
		CheckRedirect: func(req *http.Request, via []*http.Request) error {
			log.Printf("Service discovery redirected to %s", req.URL)
	...
	log.Printf("Starting service discovery at %s", discoURL)
	...
	if err != nil {
		log.Printf("Failed to request discovery document: %s", err)
		return ret // empty
	}
	if resp.StatusCode != 200 {
		log.Printf("Failed to request discovery document: %s", resp.Status)
		return ret // empty
	}
	...
	if err != nil {
		log.Printf("Discovery URL has malformed Content-Type %q", contentType)
		return ret // empty
	}
	if mediaType != "application/json" {
		log.Printf("Discovery URL returned Content-Type %q, rather than application/json", mediaType)
		return ret // empty
	}
	...
	if resp.ContentLength > maxDiscoDocBytes {
		...
		log.Printf("Discovery doc response is too large (got %d bytes; limit %d)", resp.ContentLength, maxDiscoDocBytes)
		return ret // empty
	}
	...
	if err != nil {
		log.Printf("Error reading discovery document body: %s", err)
		return ret // empty
	}
	...
	if err != nil {
		log.Printf("Failed to decode discovery document as a JSON object: %s", err)
		return ret // empty
	}
