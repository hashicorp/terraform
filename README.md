...
git remote -v
```
1. If it is added as a remote, you should see a similar output to this:
```
origin	git@github.com:hashicorp/terraform-docs-internal.git (fetch)
origin	git@github.com:hashicorp/terraform-docs-internal.git (push)
terraform-public	git@github.com:hashicorp/hashicorp/terraform.git (fetch)
terraform-public	git@github.com:hashicorp/hashicorp/terraform.git (push) 
```
1. If you don't have it as a remote, add it as one:
```
git remote add terraform-public git@github.com:hashicorp/hashicorp/terraform.git
```

1. Fetch changes from the `hashicorp/terraform` repository
```
git fetch terraform-public
```

1. Checkout the `website` directory from the `hashicorp/terraform` repository:
```
git checkout terraform-public/main -- website
```

1. Commit the changes:
```
git add website
git commit -m "Sync website directory from hashicorp/terraform repository"
```

# Cherry-Picking Commits from `hashicorp/terraform-docs-internal` to `hashicorp/terraform` Repo
1. Identify the commit hashes you want to cherry-pick. You can use `git log` to find the relevant commits.
...
1. Fetch the latest changes from the `hashicorp/terraform` repository:
```
git fetch terraform-public
```

1. Create a new branch based on the `hashicorp/terraform` repository's main branch:
```
git checkout -b <branch-name> terraform-public/main
```
...
1. Resolve any conflicts that arise during the cherry-pick process.

1. Push the changes to the `hashicorp/terraform` repository:
```
git push terraform-public cherry-pick-branch:<target-branch>
```

## TL;DR:
```
# Add the hashicorp/terraform repository as a remote
git remote add terraform-public git@github.com:hashicorp/hashicorp/terraform.git

# Fetch changes from the hashicorp/terraform repository
git fetch terraform-public

# Checkout the website directory from the hashicorp/terraform repository
# main can be replaced with the desired branch
git checkout terraform-public/main -- website 
...
# Cherry-pick commits from hashicorp/terraform-docs-internal to hashicorp/terraform repo
# Assuming you have identified the commit hashes to cherry-pick
git checkout -b <your-branch-name> terraform-public/main
git cherry-pick <commit-hash>

# Resolve any conflicts if necessary
git push terraform-public <your-branch-name>:<hashicorp/terraform-target-branch>
```
