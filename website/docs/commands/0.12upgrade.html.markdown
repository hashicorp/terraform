...
If you are running on OSX, `find` does not have `-printf` by default. You will need to install gnu find using homebrew:

```
brew install findutils
```
Once installed, you need to run the command with `gfind`, instead of `find`.
