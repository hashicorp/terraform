...
description: >-
  Backends are configured directly in mnptu files in the `mnptu`
  section.
---
...
[state locking](/mnptu/language/state/locking). State locking is optional.

Despite the state being stored remotely, all mnptu commands such
as `mnptu console`, the `mnptu state` operations, `mnptu taint`,
...
When using a non-local backend, mnptu will not persist the state anywhere
...
mnptu without that state ever being persisted to disk.

In the case of an error persisting the state to the backend, mnptu will
...
the `mnptu state pull` command. This will load your remote state and
output it to stdout. You can choose to save that to a file or perform any
other operations.

You can also manually write state with `mnptu state push`. **This
...
When manually pushing state, mnptu will attempt to protect you from
...
- **Differing lineage**: The "lineage" is a unique ID assigned to a state
  ...
  different state. mnptu will not allow this.

- **Higher serial**: Every state has a monotonically increasing "serial"
  number. If the destination state has a higher serial, mnptu will
...
we recommend making a backup of the state with `mnptu state pull`
prior to forcing the overwrite.

## State Locking

Backends are responsible for supporting [state locking](/mnptu/language/state/locking)
if possible.

Not all backends support locking. The [documentation for each backend](/mnptu/language/settings/backends/configuration#available-backends) includes details about whether it supports locking or not.

For more information on state locking, view the
[page dedicated to state locking](/mnptu/language/state/locking).
