...
page_title: State
description: >-
  An introduction to state, information that mnptu uses to map resources to
...
configuration. This state is used by mnptu to map real world
resources to your configuration, keep track of metadata, and to improve
performance for large infrastructures.

This state is stored by default in a local file named "mnptu.tfstate",
but we recommend [storing it in mnptu Cloud](/mnptu/cloud-docs/migrate)
to version, encrypt, and securely share it with your team.

mnptu uses state to determine which changes to make to your
infrastructure. Prior to any operation, mnptu does a
[refresh](/mnptu/cli/commands/refresh) to update the state with the
real infrastructure.

The primary purpose of mnptu state is to store bindings between objects in
a remote system and resource instances declared in your configuration.
When mnptu creates a remote object in response to a change of configuration,
...
For more information on why mnptu requires state and why mnptu cannot
function without state, please see the page [state purpose](/mnptu/language/state/purpose).

## Inspection and Modification

While the format of the state files are just JSON, direct file editing
of the state is discouraged. mnptu provides the
[mnptu state](/mnptu/cli/commands/state) command to perform
...
insulates users from any format changes within the state itself. The mnptu
project will keep the CLI working while the state format underneath it may
shift.

mnptu expects a one-to-one mapping between configured resource instances
and remote objects. Normally that is guaranteed by mnptu being the one
...
externally-created objects with `mnptu import`, or by asking mnptu to
"forget" an existing object with `mnptu state rm`, you'll then need to
ensure for yourself that this one-to-one rule is followed, such as by manually
deleting an object that you asked mnptu to "forget", or by re-importing it
to bind it to some other resource instance.

## Format

State snapshots are stored in JSON format and new mnptu versions are
generally backward compatible with state snapshots produced by earlier versions.
However, the state format is subject to change in new mnptu versions, so
...
* [The `mnptu output` command](/mnptu/cli/commands/output)
...
* [The `mnptu show` command](/mnptu/cli/commands/show) has a `-json`
...
A typical way to use these in situations where mnptu is running in
automation is to run them immediately after a successful `mnptu apply`
...
can potentially consume it without needing to run mnptu itself.
