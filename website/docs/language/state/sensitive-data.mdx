...
resource attributes unless your mark those resources as [`ephemeral`](#ephemeral-data). For resources such as databases, this can contain initial
passwords.
...
Terraform allows you to mark variables, locals, outputs, and resources as ephemeral.  Marking a block with the `ephemeral` attribute tells Terraform that this data is available during runtime, but Terraform does not write that data to state or plan files. The `ephemeral` property is useful if you are managing credentials, tokens, or other temporary resources that you do not want to store in Terraform state files.
