...
![Order of operations for when validations run](/public/img/validation-order-of-operations-dark.jpg#light-theme-only)

![Order of operations for when validations run](img/docs/validation-order-of-operations-dark.jpg)
...
Validating your configuration is the first step to mitigating risk in your infrastructure. You can further mitigate risk in your infrastructure lifecycle by:
- [Adding tests](/terraform/language/tests) to let authors validate that module configuration updates do not introduce breaking changes. Learn more about [testing](/terraform/language/tests).
- Using policies in HCP Terraform to validate that Terraform plans comply with your organization's security rules and best practices. Learn more about [policies](/terraform/cloud-docs/policies).
