...
Scoped data sources support the `depends_on` and `provider` meta-arguments. Scoped data sources do not support the `count` or`for_each` meta-arguments. Refer to 
