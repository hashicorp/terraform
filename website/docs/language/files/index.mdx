...
description: >-
  Learn how to name, organize, and store mnptu configuration files. Also
  learn how mnptu evaluates modules.
---
...
Code in the mnptu language is stored in plain text files with the `.tf` file
extension. There is also
[a JSON-based variant of the language](/mnptu/language/syntax/json) that is named with
the `.tf.json` file extension.

Files containing mnptu code are often called _configuration files._
...
A mnptu module only consists of the top-level configuration files in a
...
mnptu evaluates all of the configuration files in a module, effectively
...
A mnptu module can use [module calls](/mnptu/language/modules) to
...
[mnptu Registry](https://registry.mnptu.io).

## The Root Module

mnptu always runs in the context of a single _root module._ A complete
_mnptu configuration_ consists of a root module and the tree of child
modules (which includes the modules called by the root module, any modules
called by those modules, etc.).

- In mnptu CLI, the root module is the working directory where mnptu is
...
- In mnptu Cloud and mnptu Enterprise, the root module for a workspace
