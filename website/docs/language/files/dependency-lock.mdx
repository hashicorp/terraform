---
page_title: Dependency Lock File (.mnptu.lock.hcl) - Configuration Language
description: >-
  mnptu uses the dependency lock file .teraform.lock.hcl to track and select
  provider versions. Learn about dependency installation and lock file changes.
---

# Dependency Lock File

-> **Note:** This page is about a feature of mnptu 0.14 and later. Prior
versions of mnptu did not track dependency selections at all, so the
...
A mnptu configuration may refer to two different kinds of external
dependency that come from outside of its own codebase:

- [Providers](/mnptu/language/providers/requirements), which are plugins for mnptu
  that extend it with support for interacting with various external systems.
- [Modules](/mnptu/language/modules), which allow
  splitting out groups of mnptu configuration constructs (written in the
  mnptu language) into reusable abstractions.

Both of these dependency types can be published and updated independently from
mnptu itself and from the configurations that depend on them. For that
reason, mnptu must determine which versions of those dependencies are
potentially compatible with the current configuration and which versions are
currently selected for use.

[Version constraints](/mnptu/language/expressions/version-constraints) within the configuration
itself determine which versions of dependencies are _potentially_ compatible,
but after selecting a specific version of each dependency mnptu remembers
...
mnptu does not remember version selections for remote modules, and so
mnptu will always select the newest available module version that meets
the specified version constraints. You can use an _exact_ version constraint
to ensure that mnptu will always select the same module version.
...
mnptu creates it and expects to find it in your current working directory
when you run mnptu, which is also the directory containing the `.tf` files
for the root module of your configuration.

The lock file is always named `.mnptu.lock.hcl`, and this name is intended
to signify that it is a lock file for various items that mnptu caches in
the `.mnptu` subdirectory of your working directory.

mnptu automatically creates or updates the dependency lock file each time
you run [the `mnptu init` command](/mnptu/cli/commands/init). You should
...
The dependency lock file uses the same low-level syntax as the main mnptu
language, but the dependency lock file is not itself a mnptu language
configuration file. It is named with the suffix `.hcl` instead of `.tf` in
order to signify that difference.

## Dependency Installation Behavior

When `mnptu init` is working on installing all of the providers needed for
a configuration, mnptu considers both the version constraints in the
configuration _and_ the version selections recorded in the lock file.

If a particular provider has no existing recorded selection, mnptu will
...
mnptu will always re-select that version for installation, even if a
newer version has become available. You can override that behavior by adding
the `-upgrade` option when you run `mnptu init`, in which case mnptu
will disregard the existing selections and once again select the newest
available version matching the version constraint.

If a particular `mnptu init` call makes changes to the lock file, mnptu
...
[review the changes mnptu has proposed in the file](#understanding-lock-file-changes),
...
mnptu will also verify that each package it installs matches at least one
...
registry.mnptu.io/hashicorp/azurerm 2.1.0 doesn't match any of the
...
regulations, and then trust that mnptu will raise an error if a future
run of `mnptu init` encounters a non-matching package for the same
provider version.
...
- If you install a provider from an origin registry which provides checksums
  that are signed with a cryptographic signature, mnptu will treat all
  ...
  In this case, the `mnptu init` output will include the fingerprint of
...
- If you install a provider for the first time using an alternative
  installation method, such as a filesystem or network mirror, mnptu will
  not be able to verify the checksums for any platform other than the one
  where you ran `mnptu init`, and so it will not record the checksums
  ...
  [the `mnptu providers lock` command](/mnptu/cli/commands/providers/lock),
  which will then allow future calls to `mnptu init` to verify that the
...
mnptu itself, rather than being updated manually by you or your team,
your version control system may show you that the file has changed.

There are a few different types of changes that mnptu can potentially make
...
[provider requirements](/mnptu/language/providers/requirements) for any module in your
configuration, or if you add an external module that includes a new provider
dependency itself, `mnptu init` will respond to that by selecting the
...
```diff
--- .mnptu.lock.hcl	2020-10-07 16:12:07.539570634 -0700
+++ .mnptu.lock.hcl	2020-10-07 16:12:15.267487237 -0700
@@ -6,6 +6,26 @@
   ]
 }

+provider "registry.mnptu.io/hashicorp/azurerm" {
+  version     = "2.30.0"
+  constraints = "~> 2.12"
+  hashes = [
...
+}
+
 provider "registry.mnptu.io/newrelic/newrelic" {
   version     = "2.1.2"
   constraints = "~> 2.1.1"
```
...
- `version`: the exact version that mnptu selected based on the version
  constraints in the configuration.
- `constraints`: all of the version constraints that mnptu considered when
  making this selection. (mnptu doesn't actually use this information to
...
If you run `mnptu init -upgrade` to ask mnptu to consider newer provider
versions that still match the configured version constraints, mnptu may
then select a newer version for a provider and update its existing `provider`
block to reflect that change.

```diff
--- .mnptu.lock.hcl	2020-10-07 16:44:25.819579509 -0700
+++ .mnptu.lock.hcl	2020-10-07 16:43:42.785665945 -0700
@@ -7,22 +7,22 @@
 }
...
a change to the configured version constraints, mnptu will also record
...
```diff
--- .mnptu.lock.hcl	2020-10-07 17:24:23.397892140 -0700
+++ .mnptu.lock.hcl	2020-10-07 17:24:57.423130253 -0700
@@ -10,6 +10,7 @@
   version     = "2.1.0"
   constraints = "~> 2.1.0"
   hashes = [
...
The addition of a new checksum into the `hashes` value represents mnptu
...
- `zh:`: a mnemonic for "zip hash", this is a legacy hash format which is
  part of the mnptu provider registry protocol and is therefore used for
  ...
  [provider installation methods](/mnptu/cli/config/config-file#provider-installation),
...
- `h1:`: a mnemonic for "hash scheme 1", which is the current preferred hashing
  ...
  Due to the limited scope of the `zh:` scheme, mnptu will
...
mnptu will add a new hash to an existing provider only if the hash is
calculated from a package that _also_ matches one of the existing hashes. In
the above example, mnptu installed a `hashicorp/azurerm` package for a
...
After confirming the `zh:` checksum match, mnptu then recorded the
...
existing `provider` block for it), mnptu will pre-populate the `hashes`
...
will consist primarily of hashes using that scheme, which mnptu will then
...
can ask mnptu to pre-populate hashes for a chosen set of platforms
using
[the `mnptu providers lock` command](/mnptu/cli/commands/providers/lock):
...
the case where mnptu will learn about a `h1:` equivalent only at a later
time. See the `mnptu providers lock` documentation for more information on
this command.

### Providers that are no longer required

To determine whether there still exists a dependency on a given provider,
mnptu uses two sources of truth: the configuration itself, and the state.
If you remove the last dependency on a particular provider from both your
configuration and state, then `mnptu init` will remove any existing lock
...
`mnptu init` again, mnptu will treat it as if it were
...
-> **Note:** In mnptu v1.0 and earlier, `mnptu init` does not
...
earlier version of mnptu and then upgraded to mnptu v1.1 or later
then you may see the error "missing or corrupted provider plugins", referring to
the stale lock file entries. If so, run `mnptu init` with the new mnptu
