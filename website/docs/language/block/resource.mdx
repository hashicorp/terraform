...
The `resource` block defines a piece of infrastructure and specifies the settings for Terraform to create it with. The arguments that an individual resource supports are determined by the provider. Refer to the provider documentation for more information about specific resource configuration. Refer to [Create and manage resources](/terraform/language/resources) for additional information about resources in Terraform.
...
- [`resource "<TYPE>" "<LABEL>"`](#resource) &nbsp; block
   - [`PROVIDER_ARGUMENTS`](#provider-arguments) &nbsp; block | refer to your provider documentation
...
In blocks where `count` is set, Terraform exposes an additional `count` object in expressions so you can modify the configuration of each instance. The `count` object has an `index` attribute, starting from `0`, that you can use to referrence each instance.

To refer to an individual instance of a resource created using the `count` meta-argument, use the `<TYPE>.<NAME>[INDEX]` syntax. For example, `aws_instance.server[0]` refers to the first instance of the `aws_instance` resource named `server`. Refer to [Create and manage resources](/terraform/language/manage-resource/create) for additional information.
...
- [`precondition`](#precondition): Specifies a condition that Terraform evaluates before creating the resource. Refer to [Validate your configuration](/terraform/language/mitigate-risk/validate) for additional information.
- [`postcondition`](#postcondition): Specifies a condition that Terraform evaluates after creating the resource. Refer to [Validate your configuration](/terraform/language/mitigate-risk/validate) for additional information.
...
| --- | --- | --- | --- | 
| `condition` | Expression that must return `true` for Terraform to proceed with an operation. You can refer to any other object in the same configuration scope unless the reference creates a cyclic dependency. | Expression that can include references, strings, and operators. | Required |
| `error_message` | Message that Terraform prints to the console if the `condition` returns `false`. | String | Required |
...
Refer to [Validate your configuration](/terraform/language/mitigate-risk/validate) for additional information.
