...
The examples in this section illustrate how to specify resource dependencies.

The following `resource` block partially defines an `aws_iam_role` resource. Refer to the [AWS provider documentation](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role) for all configuration settings. The `aws_iam_instance_profile` resource references the `aws_iam_role` resource, instructing Terraform to create the upstream resource first. 
...
The following `resource` block configures an IAM policy that lets the EC2 instance access the S3 API.
