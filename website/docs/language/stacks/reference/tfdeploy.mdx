...
Specifies a value to export from your current Stack, which other Stacks in the same project can consume. Declare one `publish_output` block for each value to export your Stack. To learn more, refer to [Pass data from one Stack to another](/terraform/language/stacks/deploy/link-stacks). 
