...
-> **Note**: To use the `publish_output` block you must use at least Terraform version `terraform_1.10.0-alpha20241009` or higher. We recommend downloading the [latest version of Terraform](https://releases.hashicorp.com/terraform/) to use the most up-to-date functionality.
...
To learn more about passing information between Stacks, reference [Pass data from one Stack to another](/terraform/language/stacks/link-stacks).

## `upstream_input` block configuration 

-> **Note**: To use the`upstream_input` block you must use at least version`terraform_1.10.0-alpha20241009` or higher. We recommend downloading the[latest version of Terraform](https://releases.hashicorp.com/terraform/) to use the most up-to-date functionality.
...
Your application Stack can now securely consume outputs from your network Stack, such as its `vpc_id`, in this Stack's deployments. 
