...
We refer to any method that requires management plane access as 'Indirect'. Any method that does not require management plane access is referred to as 'Direct'

At a high level, the `azurerm` backend has 5 methods it can use to authenticate to the storage account data plane:

- [Direct Azure Active Directory](#direct-azure-active-directory) **(Recommended)**
- [Indirect Azure Active Directory with URI Lookup](#indirect-azure-active-directory-with-uri-lookup) **(Recommended for use with Azure DNS zone endpoints)**
- [Indirect Access Key Lookup](#indirect-access-key-lookup) *(Not recommended for new workloads)*
- [Direct SAS Token](#direct-sas-token) *(Not recommended for new workloads)*
- [Direct Access Key](#direct-access-key) *(Not recommended for new workloads)*

## Direct Azure Active Directory

This method requires a valid Azure Active Directory principal and a predictable storage account data plane URI.

There are 5 sub-types of authentication supported with this method.
...
- Compute Attached User or System Assigned Managed Identity
...
These sub-types can be supplied via inputs or via a pre-authenticated Azure CLI.

### Required Inputs

The following inputs are always required for this method:

- `use_azuread_auth` - Set to `true` to use Azure Active Directory authentication to the storage account data plane.
- `tenant_id` - The tenant ID of the Azure Active Directory principal is required to authenticate to the storage account data plane.
- `storage_account_name` - The name of the storage account is required to read / update the data plane.
- `container_name` - The name of the container within the storage account is required to read / update the data plane.
- `key` - The name of the blob key within the container is required to read / update the data plane.

### Storage Account Required Role Assignments

The minimum data plane role assignments required for this method are either one of:

- `Storage Blob Data Owner` on the storage account container (Recommended)
- `Storage Blob Data Contributor` on the storage account

No management plane role assignments are required

### Direct Azure Active Directory with OpenID Connect / Workload identity federation

#### Required Inputs

The following additional inputs are always required for this sub-type:

- `use_oidc` - Set to `true` to use OpenID Connect / Workload identity federation to authenticate to the storage account data plane.
- `client_id` - The client ID of the Azure Active Directory Service Principal / App Registration or User Assigned Managed Identity is required to authenticate to the storage account data plane.
...
### Direct Azure Active Directory with Compute Attached Managed Identity

#### Required Inputs

The following additional inputs are always required for this sub-type:

- `use_msi` - Set to `true` to use the managed identity to authenticate to the storage account data plane.

#### Optional Inputs

The following additional inputs are optional for this sub-type:

- `client_id` - The client ID of the User Assigned Managed Identity is required to authenticate to the storage account data plane. This is not required for System Assigned Managed Identity.
...
### Direct Azure Active Directory with Azure CLI
...
- `use_cli` - Set to `true` to use the Azure CLI session authenticate to the storage account data plane.
...
### Direct Azure Active Directory with Client Secret
...
- `client_id` - The client ID of the Azure Active Directory Service Principal / App Registration is required to authenticate to the storage account data plane.
- `client_secret` - The client secret of the Azure Active Directory Service Principal / App Registration is required to authenticate to the storage account data plane.
...
### Direct Azure Active Directory with Client Certificate
...
- `client_id` - The client ID of the Azure Active Directory Service Principal / App Registration is required to authenticate to the storage account data plane.
- `client_certificate_path` - The path to the client certificate bundle is required to authenticate to the storage account data plane.
- `client_certificate_password` - The password for the client certificate bundle is required to authenticate to the storage account data plane.
...
## Indirect Azure Active Directory with URI lookup

This method requires a valid Azure Active Directory principal and is for an unpredictable storage account data plane URI when using Azure DNS zone endpoints.

There are 4 sub-types of authentication supported with this method.
...
- Service Principal / App Registration with Client Secret
- Service Principal / App REgistration with Client Certificate
- User Account with Azure CLI (for local development only)
...
- `use_azuread_auth` - Set to `true` to use Azure Active Directory authentication to the storage account data plane.
- `lookup_blob_endpoint` - Set to `true` to enable the lookup of the storage account data plane URI from the management plane.
- `tenant_id` - The tenant ID of the Azure Active Directory principal is required to authenticate to the storage account management and data plane.
- `subscription_id` - The subscription ID of the storage account is required to query the management plane.
- `resource_group_name` - The resource group name of the storage account is required to query the management plane.
- `storage_account_name` - The name of the storage account is required to read from the management plane and read / update the data plane.
- `container_name` - The name of the container within the storage account is required to read / update the data plane.
- `key` - The name of the blob key within the container is required to read / update the data plane.
...
The minimum management plane role assignments required for this method are:

- `Reader` on the storage account

### Indirect Azure Active Directory with OpenID Connect / Workload identity federation
...
- `use_oidc` - Set to `true` to use OpenID Connect / Workload identity federation to authenticate to the storage account management and data plane.
- `client_id` - The client ID of the Azure Active Directory Service Principal / App Registration or User Assigned Managed Identity is required to authenticate to the storage account management and data plane.
...
terraform {
  backend "azurerm" {
    use_oidc             = true                                    # Can also be set via `ARM_USE_OIDC` environment variable.
    use_azuread_auth     = true                                    # Can also be set via `ARM_USE_AZUREAD` environment variable.
    lookup_blob_endpoint = true                                    # Can also be set via `ARM_USE_DNS_ZONE_ENDPOINT` environment variable.
    tenant_id            = "00000000-0000-0000-0000-000000000000"  # Can also be set via `ARM_TENANT_ID` environment variable.
...
### Indirect Azure Active Directory with Compute Attached Managed Identity
...
### Indirect Azure Active Directory with Azure CLI
...
### Indirect Azure Active Directory with Client Secret
...
### Indirect Azure Active Directory with Client Certificate
...
This method queries the management plane to get the storage account Access Key and then uses that Access Key to authenticate to the storage account data plane. As such it required elevated permissions on the storage account.
...
### Indirect Access Key Lookup with OpenID Connect / Workload identity federation
...
### Indirect Access Key Lookup with Compute Attached Managed Identity
...
### Indirect Access Key Lookup with Azure CLI
...
### Indirect Access Key Lookup with Client Secret
...
### Indirect Access Key Lookup with Client Certificate
...
- `access_key` - The Access Key of the storage account is required to authenticate to the storage account data plane.
- `storage_account_name` - The name of the storage account is required to read from the management plane and read / update the data plane.
- `container_name` - The name of the container within the storage account is required to read / update the data plane.
- `key` - The name of the blob key within the container is required to read / update the data plane.
...
- `sas_token` - The SAS Token for the storage account container or blob is required to authenticate to the storage account data plane.
- `storage_account_name` - The name of the storage account is required to read from the management plane and read / update the data plane.
- `container_name` - The name of the container within the storage account is required to read / update the data plane.
- `key` - The name of the blob key within the container is required to read / update the data plane.
...
For example to use [Direct Azure Active Directory authentication with OpenID Connect / Workload identity federation for GitHub](#direct-azure-active-directory-with-openid-connect--workload-identity-federation) you would use the following configuration:
...
* `use_aks_workload_identity` (Optional) Should Azure AKS Workload Identity be used for Authentication? This is only required for advanced scenarios or third party integrations. This can also be sourced from the `ARM_USE_AKS_WORKLOAD_IDENTITY` environment variable.

* `use_msi` - (Optional) Should a Compute Attached Managed Service Identity be used for authentication to the storage account management and data planes? This can also be sourced from the `ARM_USE_MSI` environment variable.
