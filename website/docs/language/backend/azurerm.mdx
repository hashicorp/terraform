...
With Azure DevOps, the ID Token endpoint environment variables are automatically found, but you need to supply the service connection ID. If we are using the `AzureCLI` or `AzurePowerShell` tasks, that is automatically set for us in the `AZURESUBSCRIPTION_SERVICE_CONNECTION_ID` environment variable.
