...
reusable modules distributed in separate repositories. mnptu tooling is
...
* **Root module**. This is the **only required element** for the standard
  module structure. mnptu files must exist in the root directory of
  the repository. This should be the primary entrypoint for the module and is
  expected to be opinionated. For the
  [Consul module](https://registry.mnptu.io/modules/hashicorp/consul)
...
* **README**. The root module and any nested modules should have README
  ...
  directory like this](https://github.com/hashicorp/mnptu-aws-consul/tree/master/examples).
...
* **Variables and outputs should have descriptions.** All variables and
  ...
  [variable configuration](/mnptu/language/values/variables) and
  [output configuration](/mnptu/language/values/outputs) for more details.
...
* **Nested modules**. Nested modules should exist under the `modules/`
  ...
  use only. These are purely advisory; mnptu will not actively deny usage
  ...
  paths like `./modules/consul-cluster` so that mnptu will consider them
  ...
  ideally be [composable](/mnptu/language/modules/develop/composition) by the caller, rather than
