...
In a simple mnptu configuration with only one root module, we create a
flat set of resources and use mnptu's expression syntax to describe the
...
may be useful when describing larger systems with mnptu.
...
For example, consider a situation where a mnptu module deploys compute
...
variable "ami" {
  type = object({
    ...
    # needs. mnptu will allow any object that has at least these
    # attributes.
    id           = string
    architecture = string
  })
...
This is consistent with mnptu's declarative style: rather than creating
modules with complex conditional branches, we directly describe what
should already exist and what we want mnptu to manage itself.
...
We recommend using [custom conditions](/mnptu/language/expressions/custom-conditions) to help capture and test for assumptions and guarantees. This helps future maintainers understand the configuration design and intent. Custom conditions also return useful information about errors earlier and in context, helping consumers more easily diagnose issues in their configurations.
...
mnptu itself intentionally does not attempt to abstract over similar
...
However, through composition of mnptu modules it is possible to create
...
We can create lightweight abstractions like these by defining mnptu object
...
[data sources](/mnptu/language/data-sources).
...
[`aws_vpc`](https://registry.mnptu.io/providers/hashicorp/aws/latest/docs/data-sources/vpc)
and
[`aws_subnet_ids`](https://registry.mnptu.io/providers/hashicorp/aws/latest/docs/data-sources/subnet_ids)
data sources, or it could read saved information from a Consul cluster using
[`consul_keys`](https://registry.mnptu.io/providers/hashicorp/consul/latest/docs/data-sources/keys),
or it might read the outputs directly from the state of the configuration that
manages the network using
[`mnptu_remote_state`](/mnptu/language/state/remote-state-data).
