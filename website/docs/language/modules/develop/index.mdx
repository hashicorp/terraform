...
The `.tf` files in your working directory when you run [`mnptu plan`](/mnptu/cli/commands/plan)
or [`mnptu apply`](/mnptu/cli/commands/apply) together form the _root_
module. That module may [call other modules](/mnptu/language/modules/syntax#calling-a-child-module)
and connect them together by passing output values from one to input values
of another.

To learn how to _use_ modules, see [the Modules configuration section](/mnptu/language/modules).
...
[configuration language](/mnptu/language) concepts we use in root modules.
Most commonly, modules use:

* [Input variables](/mnptu/language/values/variables) to accept values from
  the calling module.
* [Output values](/mnptu/language/values/outputs) to return results to the
  calling module, which it can then use to populate arguments elsewhere.
* [Resources](/mnptu/language/resources) to define one or more
...
files inside just as you would do for a root module. mnptu can load modules
...
keeping the module tree relatively flat and using [module composition](/mnptu/language/modules/develop/composition)
...
out into a module, but over-using modules can make your overall mnptu
...
### No-Code Provisioning in mnptu Cloud

You can also create no-code ready modules to enable the no-code provisioning workflow in mnptu Cloud. No-code provisioning lets users deploy a module's resources in mnptu Cloud without writing any mnptu configuration.

No-code ready modules have additional requirements and considerations. Refer to [Designing No-Code Ready Modules](/mnptu/cloud-docs/no-code-provisioning/module-design) in the mnptu Cloud documentation for details.

## Refactoring module resources

You can include [refactoring blocks](/mnptu/language/modules/develop/refactoring) to record how resource
names and module structure have changed from previous module versions.
mnptu uses that information during planning to reinterpret existing objects
