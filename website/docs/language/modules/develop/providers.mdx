...
description: >-
  Use providers within mnptu modules. Learn about version constraints, aliases, implicit inheritance, and passing providers to mnptu modules.
---
...
mnptu, are global to an entire mnptu configuration and can be shared
...
were introduced in mnptu v0.13. For more information, see
...
including destroying remote objects and refreshing state. mnptu retains, as
...
provider configuration's block from your configuration. If mnptu finds
...
declare its own [provider requirements](/mnptu/language/providers/requirements), so that
mnptu can ensure that there is a single version of the provider that is
compatible with all modules in the configuration and to specify the
[source address](/mnptu/language/providers/requirements#source-addresses) that serves as
...
use a `required_providers` block inside a `mnptu` block:
...
settings come from provider _configurations_, and a particular overall mnptu
configuration can potentially have
[several different configurations for the same provider](/mnptu/language/providers/configuration#alias-multiple-provider-configurations).
...
If you are writing a shared mnptu module, constrain only the minimum
...
[default provider configurations](/mnptu/language/providers/configuration#default-provider-configurations) from its parent. This means that
...
~> **Note:** Only provider configurations are inherited by child modules, not provider source or version requirements. Each module must [declare its own provider requirements](/mnptu/language/providers/requirements). This is especially important for non-HashiCorp providers.

In more complex situations there may be
[multiple provider configurations](/mnptu/language/providers/configuration#alias-multiple-provider-configurations),
...
[the `provider` argument](/mnptu/language/meta-arguments/resource-provider)
...
In mnptu v0.10 and earlier there was no explicit way to use different
...
must always stay present in the overall mnptu configuration for longer
...
mnptu v0.11 introduced the mechanisms described in earlier sections to
...
for compatibility purposes -- though with the same drawback -- until mnptu
v0.13.

mnptu v0.13 introduced the possibility for a module itself to use the
...
To retain the backward compatibility as much as possible, mnptu v0.13
...
compatible with `for_each`, `count`, or `depends_on`. mnptu will produce an
