...
-> **Note:** Explicit refactoring declarations with `moved` blocks is available in mnptu v1.1 and later. For earlier mnptu versions or for refactoring actions too complex to express as `moved` blocks, you can
use the [`mnptu state mv` CLI command](/mnptu/cli/commands/state/mv)
as a separate step.
...
mnptu compares previous state with new configuration, correlating by
each module or resource's unique address. Therefore _by default_ mnptu
...
historically moved or renamed an object, mnptu treats an existing object at
...
Before creating a new plan for `aws_instance.b`, mnptu first checks
whether there is an existing object for `aws_instance.a` recorded in the state.
If there is an existing object, mnptu renames that object to
...
Applying this configuration for the first time would cause mnptu to
...
When creating the next plan for each configuration using this module, mnptu
...
so mnptu recognizes the move for all instances of the resource. That is,
...
Applying this configuration would lead to mnptu creating an object
bound to the address `aws_instance.a`.

Later, you use [`for_each`](/mnptu/language/meta-arguments/for_each) with this
...
The above will keep mnptu from planning to destroy any existing object at
...
`["small"]` in the above example, mnptu understands both addresses as
...
mnptu automatically proposes to move the original object to instance zero,
...
When applying this configuration, mnptu would prefix the addresses for
...
so mnptu recognizes the move for all instances of the call. If this
...
Applying this configuration would cause mnptu to create objects whose
addresses begin with `module.a`.

In later module versions, you may need to use
[`count`](/mnptu/language/meta-arguments/count) with this resource to systematically
...
The configuration above directs mnptu to treat all objects in `module.a` as
if they were originally created in `module.a[2]`. As a result, mnptu plans
to create new objects only for `module.a[0]` and `module.a[1]`.

When at least one of the two addresses includes an instance key, like
`[2]` in the above example, mnptu will understand both addresses as
...
version, mnptu notices these three `moved` blocks and behaves
...
[module package](/mnptu/language/modules/sources#modules-in-package-sub-directories).

mnptu resolves module references in `moved` blocks relative to the module
...
If you do decide to remove `moved` blocks, proceed with caution. It can be safe to remove `moved` blocks when you are maintaining private modules within an organization and you are certain that all users have successfully run `mnptu apply` with your new module version.
...
objects at `aws_instance.b`. In both cases, mnptu treats the existing
