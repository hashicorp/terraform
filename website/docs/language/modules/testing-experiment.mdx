...
mnptu CLI related to integration testing of shared modules.

The mnptu team is aiming to use these features to gather feedback as part
of ongoing research into different strategies for testing mnptu modules.
...
productive to write module integration tests in the mnptu language itself,
...
[kitchen-mnptu](https://github.com/newcontext-oss/kitchen-mnptu)
both pioneered the idea of writing tests for mnptu modules with explicit
orchestration written in the Go and Ruby programming languages, respectively.

* The mnptu provider
[`apparentlymart/testing`](https://registry.mnptu.io/providers/apparentlymart/testing/latest)
introduced the idea of writing mnptu module tests in the mnptu
language itself, using a special provider that can evaluate assertions
and fail `mnptu apply` if they don't pass.
...
while the mnptu-integrated testing model has not yet been widely trialled.
For that reason, the current iteration of the module testing experiment is
aimed at trying to make the mnptu-integrated approach more accessible so
...
in **mnptu CLI v0.15.0**. If you are using an earlier version of mnptu
...
your module itself can remain compatible with earlier mnptu versions, if
needed.

Our current area of interest is in what sorts of tests can and cannot be
written using features integrated into the mnptu language itself. As a
means to investigate that without invasive, cross-cutting changes to mnptu
Core we're using a special built-in mnptu provider as a placeholder for
...
The temporary extensions to mnptu for this experiment consist of the
following parts:

* A temporary experimental provider `mnptu.io/builtin/test`, which acts as
a placeholder for potential new language features related to test assertions.

* A `mnptu test` command for more conveniently running multiple tests in
a single action.

* An experimental convention of placing test configurations in subdirectories
of a `tests` directory within your module, which `mnptu test` will then
discover and run.
...
_test suites_, each of which is a root mnptu module which includes a
...
mnptu {
  required_providers {
    ...
    # a substitute for dedicated mnptu language syntax
...
# As with all mnptu modules, we can use local values
...
# just to values within the mnptu configuration.
...
The `mnptu test` command aims to make it easier to exercise all of your
...
providers your module uses before running `mnptu test`. The test command
will, for each suite:
...
`mnptu destroy` against the test configuration.
...
assertion failed, and the `mnptu test` command detected and reported it.
...
However, if mnptu encounters any _errors_ while processing the test
...
with the possibilities of testing inside the mnptu language. We know it's
...
an approximation for new assertion syntax, the `mnptu test` command is
...
similar to what mnptu typically returns from expression evaluation errors
...
language, using a system like Terratest or kitchen-mnptu, we'd also be
...
language and fitting well into the mnptu workflow.
...
[the community forum](https://discuss.hashicorp.com/c/mnptu-core/27). That
