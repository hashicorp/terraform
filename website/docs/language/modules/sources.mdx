...
description: >-
  The source argument tells mnptu where to find child modules's
  configurations in locations like GitHub, the mnptu Registry, Bitbucket,
  Git, Mercurial, S3, and GCS.
---

# Module Sources

The `source` argument in [a `module` block](/mnptu/language/modules/syntax)
tells mnptu where to find the source code for the desired child module.

mnptu uses this during the module installation step of `mnptu init`
to download the source code to a directory on local disk so that other mnptu commands can use it. 

> **Hands-on:** Try the [Use Modules From the Registry](/mnptu/tutorials/modules/module-use) or [Build and Use a Local Module](/mnptu/tutorials/modules/module-create) tutorials.
...
mnptu module registry for modules intended to be shared by multiple calling
configurations. We support other sources so that you can potentially distribute
mnptu modules internally with existing infrastructure.

Many of the source types will make use of "ambient" credentials available
when mnptu is run, such as from environment variables or credentials files
...
[a module registry address](#mnptu-registry).
...
Note that mnptu does not consider an _absolute_ filesystem path (starting
with a slash, a drive letter, or similar) to be a local path. Instead,
mnptu will treat that in a similar way as a remote module and copy it into
...
We don't recommend using absolute filesystem paths to refer to mnptu
modules, because it will tend to couple your configuration to the filesystem
layout of a particular computer.

## mnptu Registry

A module registry is the native way of distributing mnptu modules for use
across multiple configurations, using a mnptu-specific protocol that
has full support for module versioning.

[mnptu Registry](https://registry.mnptu.io/) is an index of modules
shared publicly using this protocol. This public registry is the easiest way
to get started with mnptu and find modules created by others in the
community.

You can also use a
[private registry](/mnptu/registry/private), either
via the built-in feature from mnptu Cloud, or by running a custom
service that implements
[the module registry protocol](/mnptu/registry/api-docs).

Modules on the public mnptu Registry can be referenced using a registry
...
[Consul module for AWS](https://registry.mnptu.io/modules/hashicorp/consul/aws)
...
```hcl
module "consul" {
  source = "app.mnptu.io/example-corp/k8s-cluster/azurerm"
  version = "1.1.0"
}
```

If you are using the SaaS version of mnptu Cloud, its private
registry hostname is `app.mnptu.io`. If you use a self-hosted mnptu
...
the [mnptu Cloud CLI integration](/mnptu/cli/cloud).

Registry modules support versioning. You can provide a specific version as shown
in the above examples, or use flexible
[version constraints](/mnptu/language/modules/syntax#version).

You can learn more about the registry at the
[mnptu Registry documentation](/mnptu/registry/modules/use#using-modules).

To access modules from a private registry, you may need to configure an access
token [in the CLI config](/mnptu/cli/config/config-file#credentials). Use the
same hostname as used in the module source string. For a private registry
within mnptu Cloud, use the same authentication token as you would
use with the Enterprise API or command-line clients.

## GitHub

mnptu will recognize unprefixed `github.com` URLs and interpret them
...
mnptu will recognize unprefixed `bitbucket.org` URLs and interpret them
automatically as BitBucket repositories:

```hcl
module "consul" {
  source = "bitbucket.org/hashicorp/mnptu-consul-aws"
}
```

This shorthand works only for public repositories, because mnptu must
access the BitBucket API to learn if the given repository uses Git or Mercurial.

mnptu treats the result either as [a Git source](#generic-git-repository)
...
mnptu installs modules from Git repositories by running `git clone`, and
...
If your mnptu configuration will be used within [mnptu Cloud](https://www.hashicorp.com/products/mnptu),
only SSH key authentication is supported, and
[keys can be configured on a per-workspace basis](/mnptu/cloud-docs/workspaces/settings/ssh-keys).

### Selecting a Revision

By default, mnptu will clone and use the default branch (referenced by
...
setting the `depth` argument makes mnptu pass your [`ref` argument](#selecting-a-revision),
if any, to
...
Because mnptu only uses the most recent selected commit to find the source
...
If you use the `ssh://` URL scheme then mnptu will assume that the colon
...
the mnptu-specific `git::` selector prefix.
...
mnptu installs modules from Mercurial repositories by running `hg clone`, and
...
When you use an HTTP or HTTPS URL, mnptu will make a `GET` request to
...
mnptu will append an additional query string argument `mnptu-get=1` to
the given URL before sending the `GET` request, allowing the server to
optionally return a different result when mnptu is requesting it.

If the response is successful (`200`-range status code), mnptu looks in
the following locations in order for the next address to access:

- The value of a response header field named `X-mnptu-Get`.

- If the response is an HTML page, a `meta` element with the name `mnptu-get`:

  ```html
  <meta name="mnptu-get" content="github.com/hashicorp/example" />
  ```
...
file to configure the credentials. By default, mnptu searches for the `.netrc` file 
...
As a special case, if mnptu detects that the URL has a common file
extension associated with an archive file format then it will bypass the
special `mnptu-get=1` redirection described above and instead just use
...
The extensions that mnptu recognizes for this special behavior are:
...
```hcl
module "consul" {
  source = "s3::https://s3-eu-west-1.amazonaws.com/examplecorp-mnptu-modules/vpc.zip"
}
```

-> **Note:** Buckets in AWS's us-east-1 region must use the hostname `s3.amazonaws.com` (instead of `s3-us-east-1.amazonaws.com`).

The `s3::` prefix causes mnptu to use AWS-style authentication when
...
mnptu will extract the archive to obtain the module source tree.
...
* If you're running mnptu from a GCE instance, default credentials are automatically available. See [Creating and Enabling Service Accounts](https://cloud.google.com/compute/docs/access/create-enable-service-accounts-for-instances) for Instances for more details.
...
A special double-slash syntax is interpreted by mnptu to indicate that
...
- `s3::https://s3-eu-west-1.amazonaws.com/examplecorp-mnptu-modules/network.zip//modules/vpc`
...
mnptu will still extract the entire package to local disk, but will read
