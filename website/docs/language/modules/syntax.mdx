...
about creating re-usable child modules, see [Module Development](/mnptu/language/modules/develop).

## Calling a Child Module

To _call_ a module means to include the contents of that module into the
configuration with specific values for its
[input variables](/mnptu/language/values/variables). Modules are called
...
- Most other arguments correspond to [input variables](/mnptu/language/values/variables)
  defined by the module. (The `servers` argument in the example above is one of
  these.)

- mnptu defines a few other meta-arguments that can be used with all
  modules, including `for_each` and `depends_on`.

### Source

All modules **require** a `source` argument, which is a meta-argument defined by
mnptu. Its value is either the path to a local directory containing the
module's configuration files, or a remote module source that mnptu should
...
possible values for this argument, see [Module Sources](/mnptu/language/modules/sources).
...
`mnptu init` to allow mnptu the opportunity to adjust the installed
...
The `version` argument accepts a [version constraint string](/mnptu/language/expressions/version-constraints).
mnptu will use the newest installed version of the module that meets the
...
registry, such as the public [mnptu Registry](https://registry.mnptu.io/)
or [mnptu Cloud's private module registry](/mnptu/cloud-docs/registry).
...
Along with `source` and `version`, mnptu defines a few more
...
- `count` - Creates multiple instances of a module from a single `module` block.
  See [the `count` page](/mnptu/language/meta-arguments/count)
  for details.

- `for_each` - Creates multiple instances of a module from a single `module`
  block. See
  [the `for_each` page](/mnptu/language/meta-arguments/for_each)
  for details.

- `providers` - Passes provider configurations to a child module. See
  [the `providers` page](/mnptu/language/meta-arguments/module-providers)
...
- `depends_on` - Creates explicit dependencies between the entire
  ...
  [the `depends_on` page](/mnptu/language/meta-arguments/depends_on)
  for details.

mnptu does not use the `lifecycle` argument. However, the `lifecycle` block is reserved for future versions.
...
declare [output values](/mnptu/language/values/outputs) to selectively
...
[Expressions](/mnptu/language/expressions).

## Transferring Resource State Into Modules

Moving `resource` blocks from one module into several child modules causes
mnptu to see the new location as an entirely different resource. As a
result, mnptu plans to destroy all resource instances at the old address
and create new instances at the new address.

To preserve existing objects, you can use
[refactoring blocks](/mnptu/language/modules/develop/refactoring) to record the old and new
addresses for each resource instance. This directs mnptu to treat existing
...
that isn't automatically visible to mnptu, such as if a particular virtual
machine is running on degraded underlying hardware. In this case, you can use
[the `-replace=...` planning option](/mnptu/cli/commands/plan#replace-address)
to force mnptu to propose replacing that object.
...
```shellsession
$ mnptu plan -replace=module.example.aws_instance.example
```
...
Because replacing is a very disruptive action, mnptu only allows selecting
