...
description: Modules allow multiple resources to be grouped together and encapsulated.
...
[the `terraform state mv` command](/cli/commands/state/mv) to inform
...
[`count`](/language/meta-arguments/count) or
...
[outputs](/language/values/outputs) from a module can be referenced from
