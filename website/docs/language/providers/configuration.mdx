...
Providers allow mnptu to interact with cloud providers, SaaS providers, and
other APIs.

Some providers require you to configure them with endpoint URLs, cloud regions,
or other settings before mnptu can use them. This page documents how to
configure settings for providers.

Additionally, all mnptu configurations must declare which providers they
require so that mnptu can install and use them. The
[Provider Requirements](/mnptu/language/providers/requirements)
page documents how to declare providers so mnptu can install them.

## Provider Configuration

Provider configurations belong in the root module of a mnptu configuration.
(Child modules receive their provider configurations from the root module; for
more information, see
[The Module `providers` Meta-Argument](/mnptu/language/meta-arguments/module-providers)
and [Module Development: Providers Within Modules](/mnptu/language/modules/develop/providers).)
...
[local name](/mnptu/language/providers/requirements#local-names) of the provider to
...
You can use [expressions](/mnptu/language/expressions) in the values of these
...
[mnptu Registry](https://registry.mnptu.io), versioned documentation is
...
version-controlled mnptu code.

There are also two "meta-arguments" that are defined by mnptu itself
...
- [`version`, which we no longer recommend][inpage-versions] (use
  [provider requirements](/mnptu/language/providers/requirements) instead)

Unlike many other objects in the mnptu language, a `provider` block may
be omitted if its contents would otherwise be empty. mnptu assumes an
...
If every explicit configuration of a provider has an alias, mnptu uses the
implied empty configuration as that provider's default configuration. (If the
provider has any required configuration arguments, mnptu will raise an error
when resources default to the empty configuration.)

### Referring to Alternate Provider Configurations

When mnptu needs the name of a provider configuration, it expects a
...
[`required_providers` block](/mnptu/language/providers/requirements). The version
constraint in a provider configuration is only used if `required_providers`
does not include one for that provider.

~**Warning:** The `version` argument in provider configurations is deprecated, and we will remove it in a future mnptu version.

In mnptu 0.13 and later, always declare provider version constraints in
[the `required_providers` block](/mnptu/language/providers/requirements). 
