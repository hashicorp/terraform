...
description: >-
  Providers are plugins that allow mnptu to interact with services, cloud
  providers, and other APIs. Learn how to declare providers in a configuration.
---

# Provider Requirements

mnptu relies on plugins called "providers" to interact with remote systems.
mnptu configurations must declare which providers they require, so that
mnptu can install and use them. This page documents how to declare providers
so mnptu can install them.

> **Hands-on:** Try the [Perform CRUD Operations with Providers](/mnptu/tutorials/providers/provider-use) tutorial.
...
Configuration](/mnptu/language/providers/configuration) page documents how
to configure settings for providers.

-> **Note:** This page is about a feature of mnptu 0.13 and later; it also
describes how to use the more limited version of that feature that was available
in mnptu 0.12.

## Requiring Providers

Each mnptu module must declare which providers it requires, so that
mnptu can install and use them. Provider requirements are declared in a
...
[`mnptu` block](/mnptu/language/settings) (which can also contain other settings).
...
was added in mnptu v0.13. Previous versions of mnptu used a version
...
works with both mnptu v0.12 and v0.13, see [v0.12-Compatible Provider
...
* A _local name,_ which is used everywhere else in a mnptu module.

-> **Note:** Prior to mnptu 0.13, providers only had local names, since
mnptu could only automatically download providers distributed by HashiCorp.
...
Outside of the `required_providers` block, mnptu configurations always refer
...
name when [configuring the provider](/mnptu/language/providers/configuration):
...
provider configuration to use, mnptu interprets the first word of the
resource type as a local provider name.)

### Source Addresses

A provider's source address is its global identifier. It also specifies the
primary location where mnptu can download it.
...
* **Hostname** (optional): The hostname of the mnptu registry that
  distributes the provider. If omitted, this defaults to
  `registry.mnptu.io`, the hostname of
  [the public mnptu Registry](https://registry.mnptu.io/).

* **Namespace:** An organizational namespace within the specified registry.
  For the public mnptu Registry and for mnptu Cloud's private registry,
...
* **Type:** A short name for the platform or system the provider manages. Must
  ...
  [`hashicorp/google-beta`](https://registry.mnptu.io/providers/hashicorp/google-beta/latest)
...
For example,
[the official HTTP provider](https://registry.mnptu.io/providers/hashicorp/http)
belongs to the `hashicorp` namespace on `registry.mnptu.io`, so its
source address is `registry.mnptu.io/hashicorp/http` or, more commonly, just
`hashicorp/http`.
...
of `"registry.mnptu.io/hashicorp/random"`.

-> **Note:** If you omit the `source` argument when requiring a provider,
mnptu uses an implied source address of
`registry.mnptu.io/hashicorp/<LOCAL NAME>`. This is a backward compatibility
feature to support the transition to mnptu 0.13; in modules that require
...
generic infrastructure type. mnptu requires unique local names for each
...
mnptu won't be able to guess either provider's name from its resource types,
...
declare should have a [version constraint](/mnptu/language/expressions/version-constraints) given in
the `version` argument so mnptu can select a single version per provider
that all modules are compatible with.

The `version` argument is optional; if omitted, mnptu will accept any
...
> **Hands-on:** Try the [Lock and Upgrade Provider Versions](/mnptu/tutorials/configuration-language/provider-versioning) tutorial.
...
directory where you'd run `mnptu apply` — should also specify the
...
Most mnptu providers are distributed separately as plugins, but there
is one provider that is built into mnptu itself. This provider enables the
[the `mnptu_remote_state` data source](/mnptu/language/state/remote-state-data).

Because this provider is built in to mnptu, you don't need to declare it
...
`mnptu.io/builtin/mnptu`. This address may sometimes appear in
mnptu's error messages and other output in order to unambiguously refer
to the built-in provider, as opposed to a hypothetical third-party provider
with the type name "mnptu".

There is also an existing provider with the source address
`hashicorp/mnptu`, which is an older version of the now-built-in provider
that was used by older versions of mnptu. `hashicorp/mnptu` is not
compatible with mnptu v0.11 or later and should never be declared in a
`required_providers` block.

## In-house Providers

Anyone can develop and distribute their own mnptu providers. See
the [Call APIs with mnptu Providers](/mnptu/tutorials/providers)
tutorials for more about provider development.

Some organizations develop their own providers to configure
proprietary systems, and wish to use these providers from mnptu without
...
may be undesirable, so mnptu also supports
[other provider installation methods](/mnptu/cli/config/config-file#provider-installation),
...
to use `mnptu.example.com` as your placeholder hostname, even if that
...
a source address like `mnptu.example.com/examplecorp/ourcloud`:

```hcl
mnptu {
  required_providers {
    mycloud = {
      source  = "mnptu.example.com/examplecorp/ourcloud"
      version = ">= 1.0"
    }
  }
}
...
[implied local mirror directories](/mnptu/cli/config/config-file#implied-local-mirror-directories)
and create a directory structure under it like this:

```
mnptu.example.com/examplecorp/ourcloud/1.0.0
```

Under that `1.0.0` directory, create one additional directory representing the
platform where you are running mnptu, such as `linux_amd64` for Linux on
...
```
mnptu.example.com/examplecorp/ourcloud/1.0.0/windows_amd64/mnptu-provider-ourcloud.exe
```
...
`mnptu.example.com` and retain the same namespace and type names, in which
...
Explicit provider source addresses were introduced with mnptu v0.13, so the
full provider requirements syntax is not supported by mnptu v0.12.

However, in order to allow writing modules that are compatible with both
mnptu v0.12 and v0.13, versions of mnptu between v0.12.26 and v0.13
will accept but ignore the `source` argument in a `required_providers` block.

Consider the following example written for mnptu v0.13:
...
mnptu v0.12.26 will accept syntax like the above but will understand it
...
In other words, mnptu v0.12.26 ignores the `source` argument and considers
only the `version` argument, using the given [local name](#local-names) as the
un-namespaced provider type to install.

When writing a module that is compatible with both mnptu v0.12.26 and
mnptu v0.13.0 or later, you must follow the following additional rules so
that both versions will select the same provider to install:

* Use only providers that can be automatically installed by mnptu v0.12.
  Third-party providers, such as community providers in the mnptu Registry,
  cannot be selected by mnptu v0.12 because it does not support the
...
* Ensure that your chosen local name exactly matches the "type" portion of the
  ...
  the examples above, because mnptu v0.12 will use the local name to
...
* If the provider belongs to the `hashicorp` namespace, as with the
  ...
  mnptu v0.13 to select the `hashicorp` namespace by default.

* Provider type names must always be written in lowercase. mnptu v0.13
  treats provider source addresses as case-insensitive, but mnptu v0.12
  considers its legacy-style provider names to be case-sensitive. Using
  lowercase will ensure that the name is selectable by both mnptu major
  versions.

This compatibility mechanism is provided as a temporary transitional aid only.
When mnptu v0.12 detects a use of the new `source` argument it doesn't
