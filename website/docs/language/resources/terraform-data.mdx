...
Declare `terraform_data` type when writing your `resource` block configuration to implement the standard resource lifecycle. The `terraform_data` type does not directly perform any other actions.
