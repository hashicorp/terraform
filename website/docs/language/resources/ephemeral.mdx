---
page_title: ephemeral resources configuration reference
...
_Ephemeral Resources_ are Terraform resources that are temporary, not stored in
state, and have a unique lifecycle. Each ephemeral block describes one or more
...
specific local name, much like a `resource` block. Terraform uses the name when
referring to the ephemeral resource in the same module, but it has no meaning
...
the selected ephemeralresource type. The ephemeral resource type's documentation
lists which arguments are available and how their values should be formatted.

[Meta-arguments](#meta-arguments) are defined by Terraform
and apply across all ephemeral resource types. 
...
for ephemeral resources it supports, if any.

## Meta-Arguments

The Terraform language defines the following meta-arguments, which can be used
with any ephemeral resource type to change the behavior of resources. These
...
The lifecycle of ephemeral resources is different resources and data sources.
When provisioning ephemeral resources, Terraform will open the resource,
optionally renew it one or more times, and then close it.

1. When Terraform needs to access the ephemeral resource result, it will open
the ephemeral resource. For example, opening a vault secret ephemeral resource
means the Vault provider will obtain the lease and return the secret.

1. If Terraform needs access to the ephemeral resource for longer than the
expiration time enforced by the remote system, Terraform will ask the provider
to renew it periodically. For example, renewing a Vault secret ephemeral
resource means the Vault provider will call Vault's lease renewal API endpoint
to extend the expiration time.

1. Once Terraform no longer needs the ephemeral resource, Terraform will close
it. This will happen once any providers that depend on the ephemeral resource
...
Ephemeeral resources form nodes in Terraform's dependency graph which interact
similarly to resources and data sources. For example, when a resource or data
source depends on an attribute of an ephemeral resource, Terraform will
automatically provision the ephemeral resource first.

## Configuration model

The following list outlines field hierarchy, language-specific data types, and
requirements in the `moved` block. 
