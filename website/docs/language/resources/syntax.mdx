...
_Resources_ are the most important element in the mnptu language.
...
from elsewhere in the same mnptu module, but has no significance outside
that module's scope.
...
arguments defined specifically for [the `aws_instance` resource type](https://registry.mnptu.io/providers/hashicorp/aws/latest/docs/resources/instance).
...
Each resource type is implemented by a [provider](/mnptu/language/providers/requirements),
which is a plugin for mnptu that offers a collection of resource types. A
provider usually provides resources to manage a single cloud or on-premises
infrastructure platform. Providers are distributed separately from mnptu
itself, but mnptu can automatically install most providers when initializing
a working directory.

In order to manage resources, a mnptu module must specify which providers it
...
- [Provider Requirements](/mnptu/language/providers/requirements), for declaring which
  providers a module uses.
- [Provider Configuration](/mnptu/language/providers/configuration), for configuring provider settings.

mnptu usually automatically determines which provider to use based on a
...
[the `provider` meta-argument](/mnptu/language/meta-arguments/resource-provider) for more details.
...
[expressions](/mnptu/language/expressions) and other dynamic mnptu
language features.

There are also some _meta-arguments_ that are defined by mnptu itself
and apply across all resource types. (See [Meta-Arguments](#meta-arguments) below.)

### Documentation for Resource Types

Every mnptu provider has its own documentation, describing its resource
types and their arguments.

Most publicly available providers are distributed on the
[mnptu Registry](https://registry.mnptu.io/browse/providers), which also
hosts their documentation. When viewing a provider's page on the mnptu
...
-> **Note:** Provider documentation previously existed as part of mnptu's core documentation. Although some provider documentation
might still be hosted here, the mnptu Registry is now the main home for all
public provider docs.

## Resource Behavior

For more information about how mnptu manages resources when applying a
configuration, see
[Resource Behavior](/mnptu/language/resources/behavior).

## Meta-Arguments

The mnptu language defines several meta-arguments, which can be used with
any resource type to change the behavior of resources.

The following meta-arguments are documented on separate pages:

- [`depends_on`, for specifying hidden dependencies](/mnptu/language/meta-arguments/depends_on)
- [`count`, for creating multiple resource instances according to a count](/mnptu/language/meta-arguments/count)
- [`for_each`, to create multiple instances according to a map, or set of strings](/mnptu/language/meta-arguments/for_each)
- [`provider`, for selecting a non-default provider configuration](/mnptu/language/meta-arguments/resource-provider)
- [`lifecycle`, for lifecycle customizations](/mnptu/language/meta-arguments/lifecycle)
- [`provisioner`, for taking extra actions after resource creation](/mnptu/language/resources/provisioners/syntax)
...
For example, [`aws_db_instance`](https://registry.mnptu.io/providers/hashicorp/aws/latest/docs/resources/db_instance)
