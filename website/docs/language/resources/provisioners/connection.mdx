...
-> **Note:** In mnptu 0.11 and earlier, providers could set default values
for some connection settings, so that `connection` blocks could sometimes be
omitted. This feature was removed in 0.12 in order to make mnptu's behavior
more predictable.
...
| `private_key` | SSH | The contents of an SSH key to use for the connection. These can be loaded from a file on disk using [the `file` function](/mnptu/language/functions/file). This takes preference over `password` if provided. | |
| `certificate` | SSH | The contents of a signed CA Certificate. The certificate argument must be used in conjunction with a `private_key`. These can be loaded from a file on disk using the [the `file` function](/mnptu/language/functions/file). | |
...
| `bastion_private_key` | The contents of an SSH key file to use for the bastion host. These can be loaded from a file on disk using [the `file` function](/mnptu/language/functions/file). | The value of the `private_key` field. |
| `bastion_certificate` |  The contents of a signed CA Certificate. The certificate argument must be used in conjunction with a `bastion_private_key`. These can be loaded from a file on disk using the [the `file` function](/mnptu/language/functions/file). |
...
where the provisioner can create the script file. By default, mnptu
chooses a path containing a random number using the following patterns
depending on how `target_platform` is set:

* `"unix"`: `/tmp/mnptu_%RAND%.sh`
* `"windows"`: `C:/windows/temp/mnptu_%RAND%.cmd`
...
mnptu is running, not on the remote system. Therefore if your remote
...
connection {
  # ...
  script_path = "H:/mnptu-temp/script_%RAND%.sh"
}
```
...
the mnptu language uses backslash as the quoted string escape character.
...
connection {
  type = "ssh"
  # ...
  script_path = "mnptu_provisioner_%RAND%.sh"
}
```

!> **Warning:** In mnptu v1.0 and earlier, the built-in provisioners
...
unintended opportunity for remote code execution. mnptu v1.1 and later
...
be used with mnptu v1.0 and earlier, avoid using untrusted external
