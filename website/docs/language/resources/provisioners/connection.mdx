...
| `proxy_command` | A command to execute that will pipe stdin and stdout through to establish the connection. This is useful for connecting through custom networking setups. | |
...
resource "aws_instance" "example" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      ...
      proxy_command = "nc -X connect -x proxy.example.com:8080 ${self.private_ip} 22"
    }
  }
}
```
