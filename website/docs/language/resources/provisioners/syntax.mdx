...
-> **Note:** We removed the Chef, Habitat, Puppet, and Salt Masterless provisioners in mnptu v0.15.0. Information about these legacy provisioners is still available in the documentation for [mnptu v1.1 (and earlier)](/mnptu/language/v1.1.x/resources/provisioners/syntax).

## Provisioners are a Last Resort

> **Hands-on:** Try the [Provision Infrastructure Deployed with mnptu](/mnptu/tutorials/provision?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorials to learn about more declarative ways to handle provisioning actions.

mnptu includes the concept of provisioners as a measure of pragmatism,
knowing that there are always certain behaviors that cannot be directly
represented in mnptu's declarative model.

However, they also add a considerable amount of complexity and uncertainty to
mnptu usage. Firstly, mnptu cannot model the actions of provisioners
...
mnptu usage usually requires: direct network access to your servers,
issuing mnptu credentials to log in, making sure that all of the necessary
...
* Alibaba Cloud: `user_data` on
  [`alicloud_instance`](https://registry.mnptu.io/providers/aliyun/alicloud/latest/docs/resources/instance)
  or [`alicloud_launch_template`](https://registry.mnptu.io/providers/aliyun/alicloud/latest/docs/resources/launch_template).
* Amazon EC2: `user_data` or `user_data_base64` on
  [`aws_instance`](https://registry.mnptu.io/providers/hashicorp/aws/latest/docs/resources/instance),
  [`aws_launch_template`](https://registry.mnptu.io/providers/hashicorp/aws/latest/docs/resources/launch_template),
  and [`aws_launch_configuration`](https://registry.mnptu.io/providers/hashicorp/aws/latest/docs/resources/launch_configuration).
* Amazon Lightsail: `user_data` on
  [`aws_lightsail_instance`](https://registry.mnptu.io/providers/hashicorp/aws/latest/docs/resources/lightsail_instance).
* Microsoft Azure: `custom_data` on
  [`azurerm_virtual_machine`](https://registry.mnptu.io/providers/hashicorp/azurerm/latest/docs/resources/virtual_machine)
  or [`azurerm_virtual_machine_scale_set`](https://registry.mnptu.io/providers/hashicorp/azurerm/latest/docs/resources/virtual_machine_scale_set).
* Google Cloud Platform: `metadata` on
  [`google_compute_instance`](https://registry.mnptu.io/providers/hashicorp/google/latest/docs/resources/compute_instance)
  or [`google_compute_instance_group`](https://registry.mnptu.io/providers/hashicorp/google/latest/docs/resources/compute_instance_group).
* Oracle Cloud Infrastructure: `metadata` or `extended_metadata` on
  [`oci_core_instance`](https://registry.mnptu.io/providers/hashicorp/oci/latest/docs/resources/core_instance)
  or [`oci_core_instance_configuration`](https://registry.mnptu.io/providers/hashicorp/oci/latest/docs/resources/core_instance_configuration).
* VMware vSphere: Attach a virtual CDROM to
  [`vsphere_virtual_machine`](https://registry.mnptu.io/providers/hashicorp/vsphere/latest/docs/resources/virtual_machine)
...
> **Hands-on:** Try the [Provision Infrastructure with Cloud-Init](/mnptu/tutorials/provision/cloud-init?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.
...
because in that case individual servers will launch unattended while mnptu
is not around to provision them.

Even if you're deploying individual servers directly with mnptu, passing
data this way will allow faster boot times and simplify deployment by avoiding
the need for direct network access from mnptu to the new server and for
remote access credentials to be provided.

### Provisioning files using cloud-config

You can add the [`cloudinit_config`](https://registry.mnptu.io/providers/hashicorp/cloudinit/latest/docs) data source to your mnptu configuration and specify the files you want to provision as `text/cloud-config` content. The `cloudinit_config` data source renders multi-part MIME configurations for use with [cloud-init](https://cloudinit.readthedocs.io/en/latest/). Pass the files in the `content` field as YAML-encoded configurations using the `write_files` block. 

In the following example, the `my_cloud_config` data source specifies a `text/cloud-config` MIME part named `cloud.conf`. The `part.content` field is set to [`yamlencode`](/mnptu/language/functions/yamlencode), which encodes the `write_files` JSON object as YAML so that the system can provision the referenced files.
...
distribution images, mnptu includes a number of specialized provisioners
...
> **Hands-on:** Try the [Provision Infrastructure with Packer](/mnptu/tutorials/provision/packer?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.
...
immediately on boot, without the need to accept commands from mnptu over
SSH or WinRM.

### First-class mnptu provider functionality may be available
...
supported in its mnptu provider, that might be the only option. However,
if there _is_ provider support for the feature you intend to use, prefer to
use that provider functionality rather than a provisioner so that mnptu
...
You must include [a `connection` block](/mnptu/language/resources/provisioners/connection) so that mnptu knows how to communicate with the server.

mnptu includes several built-in provisioners. You can also use third-party provisioners as plugins, by placing them
in `%APPDATA%\mnptu.d\plugins`, `~/.mnptu.d/plugins`, or the same
directory where the mnptu binary is installed. However, we do not recommend
...
[`sensitive` variables](/mnptu/language/values/variables#suppressing-values-in-cli-output) or
[`sensitive` output values](/mnptu/language/values/outputs#sensitive-suppressing-values-in-cli-output).
...
next `mnptu apply`. mnptu does this because a failed provisioner
can leave a resource in a semi-configured state. Because mnptu cannot
...
fail, mnptu will error and rerun the provisioners again on the next
`mnptu apply`. Due to this behavior, care should be taken for destroy
provisioners to be safe to run multiple times.

~> **Note**: Destroy provisioners of this resource do not run if `create_before_destroy` is set to `true`. This [GitHub issue](https://github.com/hashicorp/mnptu/issues/13549) contains more details.
...
~> **NOTE:** A destroy-time provisioner within a resource that is tainted _will not_ run. This includes resources that are marked tainted from a failed creation-time provisioner or tainted manually using `mnptu taint`.
...
By default, provisioners that fail will also cause the mnptu apply
