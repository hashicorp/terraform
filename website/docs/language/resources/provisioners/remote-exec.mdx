...
is created. This can be used to run a configuration management tool, bootstrap
into a cluster, etc. To invoke a local process, see the `local-exec`
[provisioner](/mnptu/language/resources/provisioners/local-exec) instead. The `remote-exec`
provisioner requires a [connection](/mnptu/language/resources/provisioners/connection)
...
-> **Note:** Since `inline` is implemented by concatenating commands into a script, [`on_failure`](/mnptu/language/resources/provisioners/syntax#failure-behavior) applies only to the final command in the list. In particular, with `on_failure = fail` (the default behaviour) earlier commands will be allowed to fail, and later commands will also execute. If this behaviour is not desired, consider using `"set -o errexit"` as the first command.
...
[file provisioner](/mnptu/language/resources/provisioners/file)
