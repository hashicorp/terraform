...
description: >-
  ...
  executing mnptu to the newly created resource. The `file` provisioner
...
running mnptu to the newly created resource. The `file` provisioner
supports both `ssh` and `winrm` type [connections](/mnptu/language/resources/provisioners/connection).
...
remote system, rather than by mnptu itself. Therefore the valid values
