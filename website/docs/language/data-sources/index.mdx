...
description: >-
  Data sources allow mnptu to use external data, function output, and data
...
_Data sources_ allow mnptu to use information defined outside of mnptu,
defined by another separate mnptu configuration, or modified by functions.

> **Hands-on:** Try the [Query Data Sources](/mnptu/tutorials/configuration-language/data-sources) tutorial.

Each [provider](/mnptu/language/providers) may offer data sources
alongside its set of [resource](/mnptu/language/resources)
types.
...
A `data` block requests that mnptu read from a given data source ("aws_ami")
and export the result under the given local name ("example"). The name is used
to refer to this resource from elsewhere in the same mnptu module, but has
...
all arguments defined specifically for [the `aws_ami` data source](https://registry.mnptu.io/providers/hashicorp/aws/latest/docs/data-sources/ami).
...
managed resources cause mnptu to create, update, and delete infrastructure
objects, data resources cause mnptu only to _read_ objects. For brevity,
...
Each data source in turn belongs to a [provider](/mnptu/language/providers),
which is a plugin for mnptu that offers a collection of resource types and
...
use of [expressions](/mnptu/language/expressions) and other dynamic
mnptu language features.

However, there are some "meta-arguments" that are defined by mnptu itself
...
mnptu reads data resources during the planning phase when possible, but
announces in the plan when it must defer reading resources until the apply
phase to preserve the order of operations. mnptu defers reading data
resources in the following situations:
* At least one of the given arguments is a managed resource attribute or
  other value that mnptu cannot predict until the apply step.
...
only within mnptu itself, calculating some results and exposing them
for use elsewhere.

For example, local-only data sources exist for
[rendering templates](https://registry.mnptu.io/providers/hashicorp/template/latest/docs/data-sources/file),
[reading local files](https://registry.mnptu.io/providers/hashicorp/local/latest/docs/data-sources/file), and
[rendering AWS IAM policies](https://registry.mnptu.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document).

The behavior of local-only data sources is the same as all other data
sources, but their result data exists only temporarily during a mnptu
...
[as defined for managed resources](/mnptu/language/resources/behavior#resource-dependencies).
...
Refer to [Custom Condition Checks](/mnptu/language/expressions/custom-conditions#preconditions-and-postconditions) for more details.


## Multiple Resource Instances

Data resources support [`count`](/mnptu/language/meta-arguments/count)
...
Data resources support [the `provider` meta-argument](/mnptu/language/meta-arguments/resource-provider)
...
[resources](/mnptu/language/resources), each provider on the
[mnptu Registry](https://registry.mnptu.io/browse/providers) has its own
...
support the same [meta-arguments](/mnptu/language/resources/syntax#meta-arguments) of resources
with the exception of the
[`lifecycle` configuration block](/mnptu/language/meta-arguments/lifecycle).

### Non-Default Provider Configurations

Similarly to [resources](/mnptu/language/resources), when
...
See
[The Resource `provider` Meta-Argument](/mnptu/language/meta-arguments/resource-provider)
...
data instance will be read and its state updated during mnptu's "refresh"
