...
run "third" {
  // This run block can only be executed after the `primary_db` run block is completed, because it references the output of the `primary_db` run block.
  // After that, it can be executed in parallel with the `second` run block.
  state_key = "unique_3"
  variables {
    input = run.primary_db.value
  }
...
run "fifth" {
  ...
  // Therefore, it will wait for the run `using_external_db` to complete.
  state_key = "unique_5"
...
run "sixth" {
  // This run block will wait for the run `fifth` to complete, because run `fifth` is its preceding run and has the `parallel` attribute set to `false`.
  state_key = "unique_6"
  variables {
    input = run.secondary_db.value
  }
...
run "seventh" {
  // This run block can only be executed after the run `sixth` is completed, because it references the output of the run `sixth`.
  state_key = "unique_7"
  variables {
    input = run.sixth.value
  }
...
run "same_state" {
  // This run block uses an existing state key "unique_7", so it will wait for the run `seventh` to complete.
  state_key = "unique_7"
  variables {
