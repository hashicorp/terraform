...
Terraform run block assertions are a way of [Validating your configuration](/terraform/language/mitigate-risk/validate), consisting of a [condition](/terraform/language/expressions/conditionals) and an [error message](/terraform/language/mitigate-risk/validate#error-messages).
...
By default, if , including `check` block assertions, fail during the execution of a Terraform test file then the overall command reports the test as a failure.
