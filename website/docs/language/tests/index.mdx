...
The `parallel` attribute allows you to run multiple `run` blocks in parallel. By default, this attribute is set to `false`. When set to `true`, Terraform will attempt to execute the `run` block in parallel with other `run` blocks that also have the `parallel` attribute set to `true` and are not dependent on each other.
...
When `parallel = true` for a `run` block, Terraform will attempt to parallelize such blocks wherever possible. Two or more `run` blocks can be executed in parallel if:
