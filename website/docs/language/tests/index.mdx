...
You can override the normal testing behavior by updating the `command` attribute within a [`run`](#run-blocks) block (examples below). By default, each `run` block executes with `command = apply` instructing Terraform to execute a complete `apply` operation against your configuration. Replacing the `command` value with `command = plan` instructs Terraform to _not_ create new infrastructure for this `run` block. This allows test authors to validate logical operations and other conditions within their infrastructure in a process analogous to unit testing.
...
Terraform run block assertions are a way of [Validating your configuration](/terraform/language/mitigate-risk/validate). Each `run` block consist of a [condition](/terraform/language/expressions/conditionals) and an [error message](/terraform/language/mitigate-risk/validate#error-messages).
...
By default, if any [validation](/terraform/language/mitigate-risk/validate), including `check` block assertions, fails during the execution of a Terraform test file then the overall command reports the test as a failure.
