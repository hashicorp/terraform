...
Terraform executes `run` blocks in order (unless configured to be parallel. See [`Parallel Execution`](#parallel-execution)), simulating a series of Terraform commands executing directly within the configuration directory. The order of the `variables` and `provider` blocks doesn't matter, Terraform processes all the values within these blocks at the beginning of the test operation. We recommend defining your `variables` and `provider` blocks first, at the beginning of the test file.
...
Adding to the test file from the [Test config example](#test-block) above:
...
When `parallel = true` for a `run` block, Terraform will attempt to parallelize such block wherever possible. Two or more `run` blocks can be executed in parallel if:
...
- All other `run` blocks that they depend on have completed.
...
run "site_four" {
  // This run block has set `parallel = false`, implying that it does not want to be executed in parallel.
...
-> **Note:** When configuring a series of runs which have `parallel=true` in a test file, if you include a single run with `parallel=false` in this sequence, it creates a synchronization point that effectively divides the workflow into two groups:
* All runs before the `parallel=false` run must complete first
* Only after the `parallel=false` run finishes can the subsequent runs begin.
...
Runs A and B can execute simultaneously. Run C will wait until both A and B are done before starting. Finally, runs D and E can run in parallel, but only after C completes.