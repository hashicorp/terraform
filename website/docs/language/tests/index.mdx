...
- Zero to one [`test`](#test-blocks) blocks.
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------- |
| `parallel`                 | An optional boolean attribute, which sets each run to be parallelized, unless overridden at the run level.              | `false`       |
...
The `parallel` attribute allows you to run multiple `run` blocks in parallel. By default, this attribute is set to `false`. When set to `true`, Terraform will attempt to execute the `run` block in parallel with other `run` blocks that also have the `parallel` attribute set to `true`, and
which there is no dependency between them.

Adding to the test file from the [Test config example](#test-blocks) above:
...
In the above example, the `first` and `second` `run` blocks will implicitly have `parallel` set to `true`. The `third` `run` block will override the global setting and set `parallel` to `false`.
...
By default, Terraform executes `run` blocks sequentially. However, you can set the `parallel` attribute to `true` in the file's `test` block or in individual `run` blocks to enable parallel execution.

When `parallel = true` is set in the `test` block, Terraform will attempt to parallelize `run` blocks wherever possible. Two or more `run` blocks can be executed in parallel if:

- They do not reference outputs from each other.
- They do not share the same state file. The state file is determined by the state key or the module source (when the state key is not set). Therefore, `run` blocks that share the same module configuration must specify different state keys to be executed in parallel.
- Both runs have the `parallel` attribute set to `true`.

A quick example below demonstrates how terraform will execute `run` blocks in parallel.
...
run "second" {
  ...
  assert {
    condition = run.first.value == var.foo
...
run "fifth" {
  state_key = "unique_5"
  parallel = false // This overrides the global parallel flag
  variables {
    input = "foo"
  }
...
- The runs `first`, `fourth`, `seventh` can be executed in parallel from the beginning, because they do not reference outputs from each other and they do not share the same state file.
- The run `second` can only be executed after the run `first` is completed, because it references the output of the run `first`.
- The run `third` can only be executed after the run `second` is completed.
- The run `fifth` will not be executed in parallel with any of its preceding runs, because it has the `parallel` attribute set to `false`. It will also force run `sixth` to wait for it to complete.
- The run `sixth` can only be executed after the run `fifth` is completed, because run `fifth` has the `parallel` attribute set to `false`.
- The run `same_state` can only be executed after the run `first` is completed, because it shares the same state key with the run `first`.