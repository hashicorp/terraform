...
The workflow for generating configuration is similar to the [`import` block workflow](/terraform/language/import/index#plan-and-apply-an-import), with the extra step of generating configuration during the planning stage. You can then review and modify the generated configuration before applying. 
...
The import block's `to` argument points to the address of the `resource` instance this import is targeting.  If a resource in your configuration already exists at this address, Terraform attempts to import into that resource and does not generate configuration.
...
In our example, we specified that Terraform should generate configuration in a file named `generated.tf`. The below code is an example of a `generated.tf` file.
