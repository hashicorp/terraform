...
Terraform can generate code for the resources it plans to import that do not already exist in your configuration. Terraform produces HCL to act as a template that contains Terraform's best guess at the appropriate value for each resource argument. 
...
If any resources targeted by an [`import` block](/terraform/language/import) do not exist in your configuration, Terraform generates and writes configuration for those resources in `generated_resources.tf`.
