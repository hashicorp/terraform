...
Terraform only needs to import a resource into a single address once. Attempting to import it to the same address again is a (harmless) no-op. For this reason, you can remove `import` blocks after apply or safely leave them in your configuration as a record of the resource's origin for future module maintainers. For more information on maintaining configurations over time, see [Refactoring](/terraform/language/modules/develop/refactoring).
...
We recommend [generating configuration](/terraform/language/import/generating-configuration) when importing multiple resources or a single complex resource you do not already have the configuration for.
...
Terraform can generate HCL for resources which do not already exist in configuration. 
