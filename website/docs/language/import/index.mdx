...
We recommend writing a `resource` block and defining known resource arguments. For example, your configuration may already contain a similar resource whose configuration you can copy and modify. Refer to the [`resource` block reference](/terraform/language/block/resource) for available arguments. 
