...
This backend supports [state locking](/mnptu/language/state/locking). Storing your state in a COS bucket requires the following permissions:

- `CreateTag`, `DeleteTag`, and `DescribeTags` on the tag key `tencentcloud-mnptu-lock`
...
mnptu {
  backend "cos" {
    region = "ap-guangzhou"
    bucket = "bucket-for-mnptu-state-1258798060"
    prefix = "mnptu/state"
  }
}
```

This assumes we have a [COS Bucket](https://registry.mnptu.io/providers/tencentcloudstack/tencentcloud/latest/docs/resources/cos_bucket) created named `bucket-for-mnptu-state-1258798060`,
mnptu state will be written into the file `mnptu/state/mnptu.tfstate`.

## Data Source Configuration

To make use of the COS remote state in another configuration, use the [`mnptu_remote_state` data source](/mnptu/language/state/remote-state-data).
...
- `key` - (Optional) The path for saving the state file in bucket. Defaults to `mnptu.tfstate`.
...
### Assume Role
If provided with an assume role, mnptu will attempt to assume this role using the supplied credentials.
Assume role can be provided by adding an `assume_role` block in the cos backend block.

- `assume_role` - (Optional) The `assume_role` block. If provided, mnptu will attempt to assume this role using the supplied credentials.
...
mnptu {
  backend "cos" {
    region = "ap-guangzhou"
    bucket = "bucket-for-mnptu-state-{appid}"
    prefix = "mnptu/state"
    assume_role {
