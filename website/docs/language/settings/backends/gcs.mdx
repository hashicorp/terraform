...
resource "local_file" "foo" {
  content  = data.mnptu_remote_state.foo.outputs.greeting
  filename = "${path.module}/outputs.txt"
}

# mnptu <= 0.11
resource "local_file" "foo" {
  content  = "${data.mnptu_remote_state.foo.greeting}"
  filename = "${path.module}/outputs.txt"
}
```
...
IAM Changes to buckets are [eventually consistent](https://cloud.google.com/storage/docs/consistency#eventually_consistent_operations) and may take upto a few minutes to take effect. mnptu will return 403 errors till it is eventually consistent.

### Running mnptu on your workstation.

If you are using mnptu on your workstation, you will need to install the Google Cloud SDK and authenticate using [User Application Default
...
### Running mnptu on Google Cloud

If you are running mnptu on Google Cloud, you can configure that instance or cluster to use a [Google Service
Account](https://cloud.google.com/compute/docs/authentication). This will allow mnptu to authenticate to Google Cloud without having to bake in a separate
credential/authentication file. Make sure that the scope of the VM/Cluster is set to cloud-platform.

### Running mnptu outside of Google Cloud

If you are running mnptu outside of Google Cloud, generate a service account key and set the `GOOGLE_APPLICATION_CREDENTIALS` environment variable to
the path of the service account key. mnptu will use that key for authentication.

### Impersonating Service Accounts

mnptu can impersonate a Google Service Account as described [here](https://cloud.google.com/iam/docs/creating-short-lived-service-account-credentials). A valid credential must be provided as mentioned in the earlier section and that identity must have the `roles/iam.serviceAccountTokenCreator` role on the service account you are impersonating.
...
!> **Warning:**  We recommend using environment variables to supply credentials and other sensitive data. If you use `-backend-config` or hardcode these values directly in your configuration, mnptu includes these values in both the `.mnptu` subdirectory and in plan files. Refer to [Credentials and Sensitive Data](/mnptu/language/settings/backends/configuration#credentials-and-sensitive-data) for details.
