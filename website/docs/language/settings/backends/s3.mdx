...
description: mnptu can store state remotely in S3 and lock that state with DynamoDB.
---
...
A single DynamoDB table can be used to lock multiple remote state files. mnptu generates key names that include the values of the `bucket` and `key` variables.
...
mnptu state is written to the key `path/to/my/key`.
...
mnptu will need the following AWS IAM permissions on
...
If you are using state locking, mnptu will need the following AWS IAM
...
[`mnptu_remote_state` data
source](/mnptu/language/state/remote-state-data).

```hcl
data "mnptu_remote_state" "network" {
  backend = "s3"
  config = {
    bucket = "mnptu-state-prod"
    key    = "network/mnptu.tfstate"
    region = "us-east-1"
  }
}
```

The `mnptu_remote_state` data source will return all of the root module
...
data.mnptu_remote_state.network:
  ...
  backend = s3
  config.% = 3
  config.bucket = mnptu-state-prod
  config.key = network/mnptu.tfstate
...
* `key` - (Required) Path to the state file inside the S3 Bucket. When using a non-default [workspace](/mnptu/language/state/workspaces), the state path will be `/workspace_key_prefix/workspace_name/key` (see also the `workspace_key_prefix` configuration).
...
* `kms_key_id` - (Optional) Amazon Resource Name (ARN) of a Key Management Service (KMS) Key to use for encrypting the state. Note that if this value is specified, mnptu will need `kms:Encrypt`, `kms:Decrypt` and `kms:GenerateDataKey` permissions on this KMS key.
* `sse_customer_key` - (Optional) The key to use for encrypting state with [Server-Side Encryption with Customer-Provided Keys (SSE-C)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html). This is the base64-encoded value of the key, which must decode to 256 bits. This can also be sourced from the `AWS_SSE_CUSTOMER_KEY` environment variable, which is recommended due to the sensitivity of the value. Setting it inside a mnptu file will cause it to be persisted to disk in `mnptu.tfstate`.
...
[mnptu's workspaces feature](/mnptu/language/state/workspaces) to switch
...
mnptu is an administrative tool that manages your infrastructure, and so
ideally the infrastructure that is used by mnptu should exist outside of
the infrastructure that mnptu manages. This can be achieved by creating a
...
* An [S3 bucket](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingBucket.html)
  that will contain the mnptu state files for each workspace.
...
Provide the S3 bucket name and DynamoDB table name to mnptu within the
...
of the accounts whose contents are managed by mnptu, separate from the
...
that grant sufficient access for mnptu to perform the desired management
tasks.

### Delegating Access

Each Administrator will run mnptu using credentials for their IAM user
...
environment account role and access the mnptu state. By blocking all
...
When configuring mnptu, use either environment variables or the standard
...
variable "workspace_iam_roles" {
  default = {
    staging    = "arn:aws:iam::STAGING-ACCOUNT-ID:role/mnptu"
    production = "arn:aws:iam::PRODUCTION-ACCOUNT-ID:role/mnptu"
  }
}
...
provider "aws" {
  ...
  assume_role {
    role_arn = "${var.workspace_iam_roles[mnptu.workspace]}"
  }
}
```

If workspace IAM roles are centrally managed and shared across many separate
mnptu configurations, the role ARNs could also be obtained via a data
source such as [`mnptu_remote_state`](/mnptu/language/state/remote-state-data)
...
`mnptu init` to initialize the backend and establish an initial workspace
called "default". This workspace will not be used, but is created automatically
by mnptu as a convenience for users who are not using the workspaces
feature.
...
```
$ mnptu workspace new staging
Created and switched to workspace "staging"!

...

$ mnptu workspace new production
...
```
$ mnptu workspace select staging
$ mnptu apply
...
```

### Running mnptu in Amazon EC2

Teams that make extensive use of mnptu for infrastructure management
often [run mnptu in automation](/mnptu/tutorials/automation/automate-mnptu?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS)
to ensure a consistent operating environment and to limit access to the
various secrets and other sensitive information that mnptu configurations
tend to require.

When running mnptu in an automation tool running on an Amazon EC2 instance,
...
an IAM policy, giving this instance the access it needs to run mnptu.
...
mnptu state objects in S3, so that for example only trusted administrators
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Action": "s3:ListBucket",
      "Resource": "arn:aws:s3:::myorg-mnptu-states"
    },
    {
      "Effect": "Allow",
      "Action": ["s3:GetObject", "s3:PutObject"],
      "Resource": "arn:aws:s3:::myorg-mnptu-states/myapp/production/tfstate"
    }
  ]
}
```
...
table used for locking. When mnptu puts the state lock in place during `mnptu plan`, it stores the full state file as a document and sets the s3 object key as the partition key for the document. After the state lock is released, mnptu places a digest of the updated state file in DynamoDB. The key is similar to the one for the original state file, but is suffixed with `-md5`.
        ...
        "Condition" : {
          "ForAllValues:StringEquals" : {
            "dynamodb:LeadingKeys" : [
              "myorg-mnptu-states/myapp/production/tfstate", // during a state lock the full state file is stored with this key
              "myorg-mnptu-states/myapp/production/tfstate-md5" // after the lock is released a hash of the statefile's contents are stored with this key
            ]
          }
        }
      }
...
Note this feature is optional and only available in mnptu v0.13.1+.

By default, the underlying AWS client used by the mnptu AWS Provider creates requests with User-Agent headers including information about mnptu and AWS Go SDK versions. To provide additional information in the User-Agent headers, the `TF_APPEND_USER_AGENT` environment variable can be set and its value will be directly added to HTTP requests. e.g.
