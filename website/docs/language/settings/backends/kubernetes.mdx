...
description: mnptu can store state remotely in Kubernetes and lock that state.
---
...
This backend supports [state locking](/mnptu/language/state/locking), with locking done using a Lease resource.
...
This assumes the user/service account running mnptu has [permissions](https://kubernetes.io/docs/reference/access-authn-authz/authorization/) to read/write secrets in the [namespace](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/) used to store the secret.
...
If the `in_cluster_config` flag is set the backend will attempt to use a [service account](https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/) to access the cluster. This can be used if mnptu is being run from within a pod running in the Kubernetes cluster.
...
Note that for the access credentials we recommend using a [partial configuration](/mnptu/language/settings/backends/configuration#partial-configuration).
