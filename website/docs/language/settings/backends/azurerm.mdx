...
When authenticating using Azure AD Authentication and Customer Managed Key encryption:
...
terraform {
  backend "azurerm" {
    ...
    key                  = "prod.terraform.tfstate"
    use_azuread_auth     = true
    key_vault_secret_uri = "https://myvault.vault.azure.net/secrets/mysecret/"
    subscription_id      = "00000000-0000-0000-0000-000000000000"
    tenant_id            = "00000000-0000-0000-0000-000000000000"
  }
}
...
-> **Note:** When using Customer Managed Key encryption you also need to ensure the `Key Vault Secrets Officer` (with RBAC) or equivalent vault access policy for keyvault. Both keyvault and storage accounts have to be in the same subscription & in combination with `use_azuread_auth` only.
...
* `key_vault_secret_uri` - (Optional) Enable [customer managed key encryption](https://learn.microsoft.com/en-us/azure/storage/blobs/encryption-customer-provided-keys#request-headers-for-specifying-customer-provided-keys) of the state file with an AES-256 secret from keyvault. Version is optional, if not passed latest is chosen.
