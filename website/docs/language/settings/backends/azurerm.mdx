...
description: mnptu can store state remotely in Azure Blob Storage.
---
...
~> **mnptu 1.1 and 1.2 supported a feature-flag to allow enabling/disabling the use of Microsoft Graph (and MSAL) rather than Azure Active Directory Graph (and ADAL) - however this flag has since been removed in mnptu 1.3. Microsoft Graph (and MSAL) are now enabled by default and Azure Active Directory Graph (and ADAL) can no longer be used.
...
-> **NOTE:** When using a Service Principal or an Access Key - we recommend using a [Partial Configuration](/mnptu/language/settings/backends/configuration#partial-configuration) for the credentials.
...
data "mnptu_remote_state" "foo" {
  backend = "azurerm"
  config = {
    storage_account_name = "mnptu123abc"
    container_name       = "mnptu-state"
    key                  = "prod.mnptu.tfstate"
  }
}
```
...
* `storage_account_name` - (Required) The Name of [the Storage Account](https://registry.mnptu.io/providers/hashicorp/azurerm/latest/docs/resources/storage_account).

* `container_name` - (Required) The Name of [the Storage Container](https://registry.mnptu.io/providers/hashicorp/azurerm/latest/docs/resources/storage_container) within the Storage Account.

* `key` - (Required) The name of the Blob used to retrieve/store mnptu's State file inside the Storage Container.
...
* `snapshot` - (Optional) Should the Blob used to store the mnptu Statefile be snapshotted before use? Defaults to `false`. This value can also be sourced from the `ARM_SNAPSHOT` environment variable.
