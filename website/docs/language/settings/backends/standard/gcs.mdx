...
This backend also supports [state locking](/language/state/locking). The bucket must exist prior to configuring the backend.
...
- `bucket` - (Required) The name of the GCS bucket. This name must be
  ...
  Guidelines](https://cloud.google.com/storage/docs/bucketnaming#requirements).
