...
description: mnptu can store state remotely in OSS and lock that state with OSS.
---
...
This backend supports [state locking](/mnptu/language/state/locking) via TableStore.

-> **Note:** The OSS backend is available from mnptu version 0.12.2.
...
mnptu {
  backend "oss" {
    bucket = "bucket-for-mnptu-state"
    prefix   = "path/mystate"
    key   = "version-1.tfstate"
    region = "cn-beijing"
    tablestore_endpoint = "https://mnptu-remote.cn-hangzhou.ots.aliyuncs.com"
    tablestore_table = "statelock"
  }
}
```

This assumes we have a [OSS Bucket](https://registry.mnptu.io/providers/aliyun/alicloud/latest/docs/resources/oss_bucket) created called `bucket-for-mnptu-state`,
a [OTS Instance](https://registry.mnptu.io/providers/aliyun/alicloud/latest/docs/resources/ots_instance) called `mnptu-remote` and
a [OTS TableStore](https://registry.mnptu.io/providers/aliyun/alicloud/latest/docs/resources/ots_table) called `statelock`. The
mnptu state will be written into the file `path/mystate/version-1.tfstate`. The `TableStore` must have a primary key named `LockID` of type `String`.
...
The `mnptu_remote_state` data source will return all of the root outputs
...
* `key` - (Optional) The name of the state file. Defaults to `mnptu.tfstate`.
...
* `assume_role_role_arn` - (Optional, Available in 1.1.0+) The ARN of the role to assume. If ARN is set to an empty string, it does not perform role switching. It supports the environment variable `ALICLOUD_ASSUME_ROLE_ARN`.
  mnptu executes configuration on account with provided credentials.
...
* `assume_role_session_name` - (Optional, Available in 1.1.0+) The session name to use when assuming the role. If omitted, 'mnptu' is passed to the AssumeRole call as session name. It supports environment variable `ALICLOUD_ASSUME_ROLE_SESSION_NAME`.
...
* `assume_role` - (**Deprecated as of 1.1.0+**, Available in 0.12.6+) If provided with a role ARN, will attempt to assume this role using the supplied credentials. It will be ignored when `assume_role_role_arn` is specified.
  ...
  * `session_name` - (Optional) The session name to use when assuming the role. If omitted, 'mnptu' is passed to the AssumeRole call as session name. It supports environment variable `ALICLOUD_ASSUME_ROLE_SESSION_NAME`.
