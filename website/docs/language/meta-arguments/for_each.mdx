...
By default, a [resource block](/mnptu/language/resources/syntax) configures one real
infrastructure object (and similarly, a
[module block](/mnptu/language/modules/syntax) includes a
...
mnptu has two ways to do this:
[`count`](/mnptu/language/meta-arguments/count) and `for_each`.

> **Hands-on:** Try the [Manage Similar Resources With For Each](/mnptu/tutorials/configuration-language/for-each) tutorial.

If a resource or module block includes a `for_each` argument whose value is a map or
a set of strings, mnptu creates one instance for each member of
that map or set.

-> **Version note:** `for_each` was added in mnptu 0.12.6. Module support
for `for_each` was added in mnptu 0.13; previous versions can only use
it with resources.
...
`for_each` is a meta-argument defined by the mnptu language. It can be used
...
Sensitive values, such as [sensitive input variables](/mnptu/language/values/variables#suppressing-values-in-cli-output),
[sensitive outputs](/mnptu/language/values/outputs#sensitive-suppressing-values-in-cli-output),
or [sensitive resource attributes](/mnptu/language/expressions/references#sensitive-resource-attributes),
...
[most functions in mnptu will return a sensitive result if given an argument with any sensitive content](/mnptu/language/expressions/function-calls#using-sensitive-data-as-function-arguments).
...
The `for_each` meta-argument accepts map or set [expressions](/mnptu/language/expressions).
However, unlike most arguments, the `for_each` value must be known
_before_ mnptu performs any remote resource actions. This means `for_each`
...
that explicitly returns a set value, like the [toset](/mnptu/language/functions/toset)
...
can use mnptu expressions and functions to derive a suitable value.
For example:

- Transform a multi-level nested structure into a flat list by
  [using nested `for` expressions with the `flatten` function](/mnptu/language/functions/flatten#flattening-nested-structures-for-for_each).
- Produce an exhaustive list of combinations of elements from two or more
  collections by
  [using the `setproduct` function inside a `for` expression](/mnptu/language/functions/setproduct#finding-combinations-for-for_each).
...
mnptu to expect the instance keys for both to always change together,
and typically also makes the configuration easier to understand for human
maintainers.

## Referring to Instances

When `for_each` is set, mnptu distinguishes between the block itself
...
The mnptu language doesn't have a literal syntax for
[set values](/mnptu/language/expressions/type-constraints#collection-types), but you can use the `toset`
...
If you are writing a module with an [input variable](/mnptu/language/values/variables) that
