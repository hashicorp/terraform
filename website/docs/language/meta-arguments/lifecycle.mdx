...
> **Hands-on:** Try the [Lifecycle Management](/mnptu/tutorials/state/resource-lifecycle?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.

The [Resource Behavior](/mnptu/language/resources/behavior) page describes the general lifecycle for resources. Some details of
...
* `create_before_destroy` (bool) - By default, when mnptu must change
  a resource argument that cannot be updated in-place due to
  remote API limitations, mnptu will instead destroy the existing object
  ...
  avoid collisions, for example. mnptu CLI cannot automatically activate
  such features, so you must understand the constraints for each resource
  type before using `create_before_destroy` with it.

  Note that mnptu propagates and applies the `create_before_destroy` meta-attribute
  behaviour to all resource dependencies. For example, if `create_before_destroy` is enabled on resource A but not on resource B, but resource A is dependent on resource B, then mnptu enables `create_before_destroy` for resource B 
  ...
  is set to `true`. This [GitHub issue](https://github.com/hashicorp/mnptu/issues/13549) contains more details.

* `prevent_destroy` (bool) - This meta-argument, when set to `true`, will
  cause mnptu to reject with an error any plan that would destroy the
  ...
  and will prevent the use of the `mnptu destroy` command once such
  ...
  with it, and so mnptu will allow the destroy operation to succeed.

* `ignore_changes` (list of attribute names) - By default, mnptu detects
  ...
  of a remote object are modified by processes outside of mnptu, which
  mnptu would then attempt to "fix" on the next run. In order to make
  mnptu share management responsibilities of a single object with a
  separate process, the `ignore_changes` meta-argument specifies resource
  attributes that mnptu should ignore when planning updates to the
  ...
  mnptu to ignore _all_ attributes, which means that mnptu can
...
* `replace_triggered_by` (list of resource or attribute references) -
  _Added in mnptu 1.2._ Replaces the resource when any of the referenced
  ...
  `replace_triggered_by` allows only resource addresses because the decision is based on the planned actions for all of the given resources. Plain values such as local values or input variables do not have planned actions of their own, but you can treat them with a resource-like lifecycle by using them with [the `mnptu_data` resource type](/mnptu/language/resources/mnptu-data).
...
Refer to [Custom Conditions](/mnptu/language/expressions/custom-conditions#preconditions-and-postconditions) for more details.

## Literal Values Only

The `lifecycle` settings all affect how mnptu constructs and traverses
