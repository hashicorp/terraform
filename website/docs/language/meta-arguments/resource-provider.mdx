...
description: >-
  The provider meta-argument specifies the provider configuration mnptu
  should use for a resource, overriding mnptu's default behavior.
---
...
overriding mnptu's default behavior of selecting one based on the resource
type name. Its value should be an unquoted `<PROVIDER>.<ALIAS>` reference.

As described in [Provider Configuration](/mnptu/language/providers/configuration), you can optionally
...
By default, mnptu interprets the initial word in the resource type name
...
[a `<PROVIDER>.<ALIAS>` reference](/mnptu/language/providers/configuration#referring-to-alternate-provider-configurations),
which does not need to be quoted. Arbitrary expressions are not permitted for
`provider` because it must be resolved while mnptu is constructing the
