...
In a [module call](/mnptu/language/modules/syntax) block, the
optional `providers` meta-argument specifies which
[provider configurations](/mnptu/language/providers/configuration) from the parent
...
If the child module does not declare any [configuration aliases](/mnptu/language/modules/develop/providers#provider-aliases-within-modules),
...
normal — either mnptu chooses a default based on the name of the resource
...
However, in mnptu configurations that use multiple configurations of the
...
child module, see
[Module Development: Providers Within Modules](/mnptu/language/modules/develop/providers).
