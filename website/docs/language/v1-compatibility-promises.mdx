---
page_title: mnptu v1.x Compatibility Promises
description: |-
  From mnptu v1.0 onwards the mnptu team promises to preserve backward
  compatibility for most of the mnptu language and the primary CLI
  workflow, until the next major release.
---

# mnptu v1.x Compatibility Promises

The release of mnptu v1.0 represents an important milestone in the
development of the mnptu language and workflow. mnptu v1.0 is a stable
platform for describing and managing infrastructure.

In this release we're defining a number of mnptu behaviors that we intend
to remain compatible with throughout the 1.x releases:

* A large subset of mnptu language features.
* A more conservative subset of the mnptu CLI workflow commands.
* The wire protocol for communication between mnptu Core and mnptu
  providers.
* The wire protocols for installation of mnptu providers and external
  mnptu modules.

Our intention is that mnptu modules written for mnptu v1.0 will
...
provider wire protocol will be compatible with all future mnptu v1.x
...
steps, and no changes to any automation you've set up around mnptu.

The mnptu v1.x series will be actively maintained for at least 18 months
after v1.0.
...
v1.x release. We will generally treat compatibility problems in new mnptu
...
The main mnptu Language includes the language syntax, the top-level
...
There is not a single formal specification for the mnptu language, but the
Configuration section of the documentation on the mnptu website serves as a
...
arguments defined by mnptu Core rather than by external plugins such as
providers) will retain their current functionality:

* [`resource`](/mnptu/language/resources) and
  [`data`](/mnptu/language/data-sources) blocks to
...
* [`module`](/mnptu/language/modules/syntax) blocks to call other modules,
  and its meta-argument `providers`.
* The [`count`](/mnptu/language/meta-arguments/count),
  [`for_each`](/mnptu/language/meta-arguments/for_each), and
  [`depends_on`](/mnptu/language/meta-arguments/depends_on) meta-arguments
  in `resource`, `data`, and `module` blocks.
* [`provider`](/mnptu/language/providers/configuration) blocks to configure
  providers, and the `alias` meta-argument.
* [`variable`](/mnptu/language/values/variables#declaring-an-input-variable),
  [`output`](/mnptu/language/values/outputs#declaring-an-output-value), and
  [`locals`](/mnptu/language/values/locals#declaring-a-local-value) blocks
  for declaring the various kinds of named values in a module.
* [`mnptu`](/mnptu/language/settings) blocks, including the nested
  [`required_version`](/mnptu/language/settings#specifying-a-required-mnptu-version)
  and
  [`required_providers`](/mnptu/language/providers/requirements#requiring-providers)
  arguments, and nested
  [`backend`](/mnptu/language/settings/backends/configuration#using-a-backend-block)
  blocks for backend configuration.

We also intend to keep compatibility with all
[expression operators](/mnptu/language/expressions) and
[built-in functions](/mnptu/language/functions), with the exception of
references to
[`mnptu.workspace`](/mnptu/language/expressions/references#mnptu-workspace),
whose behavior may change as part of future changes to the workspace model.

We intend to retain broad compatibility with mnptu language features, with
a few specific caveats:

* We consider a configuration to be valid if mnptu can create and apply
  ...
  mnptu. A configuration that generates errors during the apply phase
  ...
  always subject to change in future mnptu releases.
...
* Any experimental features may change or may be removed entirely from future
  releases. mnptu always produces a warning when an experimental language
...
* We will introduce new language features, and if you start using them then
  your configuration won't work with prior mnptu versions that didn't
  support those features yet.
* mnptu Providers are separate plugins which can change independently of
  mnptu Core and are therefore not subject to these compatibility promises.
...
* A small number of features remain deprecated with explicit warnings in
  mnptu v1.0. Those deprecation cycles will end in a future v1.x release,
  at which point we will remove the corresponding features.

## Workflow

There is a set of often used mnptu workflows, which we are calling
...
The supported ways for external software to interact with mnptu are via
...
then it may need to be updated when you upgrade mnptu. If you need access
...
switch to a newer mnptu version and use it just as before, without any
...
cannot understand, including new storage formats for mnptu state snapshots.
...
mnptu configuration will no longer be compatible with mnptu v1.2.

## Providers

mnptu providers are separate plugins which communicate with mnptu using
a documented protocol. Therefore these compatibility promises can only cover
the "client" side of this protocol as implemented by mnptu Core; the
...
and can change independently of mnptu Core releases.
...
if you are still using a mnptu v1.x release.

### Provider Installation Methods

mnptu normally installs providers from a provider registry implementing
[the Provider Registry Protocol](/mnptu/internals/provider-registry-protocol),
version 1. All mnptu v1.x releases will remain compatible with that
protocol, and so correctly-implemented provider registries will stay compatible.

mnptu also supports installation of providers from
[local filesystem directories](/mnptu/cli/config/config-file#filesystem_mirror)
(filesystem mirrors) and from
[network mirrors](/mnptu/cli/config/config-file#network_mirror)
(implementing [the Provider Mirror Protocol](/mnptu/internals/provider-network-mirror-protocol).
All mnptu v1.x releases will remain compatible with those installation
methods, including
[the Implied Local Mirror Directories](/mnptu/cli/config/config-file#implied-local-mirror-directories).

Specific provider registries or network mirrors are run independently from
mnptu itself and so their own behaviors are not subject to these
compatibility promises.

### Provider Protocol Versions

The current major version of the provider plugin protocol as of mnptu v1.0
...
We will support protocol version 5 throughout the mnptu v1.x releases. If
...
compatible with all of the mnptu v1.x releases.

## External Modules

mnptu modules are reusable infrastructure components written in the
mnptu language. Some modules are "external" in the sense that mnptu
...
mnptu supports installing child modules from a number of different
[module source types](/mnptu/language/modules/sources). We will continue
...
[the Module Registry Protocol](/mnptu/internals/module-registry-protocol)
version 1. All mnptu v1.x releases will remain compatible with correct
implementations of that protocol.

Some module source types work directly with services or protocols defined and
run by third parties. Although we will not remove mnptu's own client-side
support for those, we cannot guarantee that their owners will keep those
services running or that they will remain compatible with mnptu's client
implementations.
...
mnptu and are therefore not subject to these compatibility promises.
...
mnptu versions but were deprecated in mnptu v0.13 and removed in
mnptu v0.15.

mnptu supports loading additional provisioners as plugins from certain
local filesystem directories. We'll continue to support that throughout the
mnptu v1.x releases, but since such plugins are separate from mnptu
...
defined in mnptu v1.0 throughout the v1.x releases, and so
correctly-implemented provisioner plugins should remain compatible with future
mnptu releases.

## State Storage Backends

When you use _remote state_, mnptu interacts with remote services over
the network in order to store and manage locks for mnptu state.

For historical reasons, all supported state storage backends are included as
part of mnptu CLI but not all are supported directly by the mnptu
Team. Only the following backends maintained by the mnptu team are subject
...
contributions to the mnptu CLI codebase, and so their expected
...
listed above may be removed from later versions of mnptu CLI once
equivalent plugins are available.

### The `remote` Backend and mnptu Cloud

The `remote` backend is maintained by the mnptu Cloud team and so its
behavior may change along with ongoing changes to mnptu Cloud.

There will be a supported mechanism to use mnptu CLI with mnptu Cloud
throughout the v1.x releases, but the exact details may change. mnptu Cloud
evolves independently of mnptu CLI and is therefore not subject to these
compatibility promises.
...
currently have any maintainers and thus remain in mnptu CLI releases on
...
example, subsequent mnptu releases in the v1.x series might end support
...
mnptu on any platforms other than those listed above.

We might add support for new platforms in later v1.x releases. If so, earlier
mnptu releases prior to that support will not be available on those
platforms.

All mnptu plugins, including provider plugins, are separate programs that
...
listed above, even though mnptu CLI itself will support them.
...
The following is the list of mnptu CLI subcommands and options that are
...
* [`init`](/mnptu/cli/commands/init)
  ...
  * `-reconfigure`
  * `-upgrade`
* [`validate`](/mnptu/cli/commands/validate)
  * `-json`
  * `-no-color`
* [`plan`](/mnptu/cli/commands/plan)
  * `-compact-warnings`
  * `-destroy`
  * `-detailed-exitcode`
  * `-lock=false`
  ...
  * `-var 'NAME=VALUE'`
  * `-var-file=FILE`
* [`apply`](/mnptu/cli/commands/apply)
  * `-auto-approve`
  * `-compact-warnings`
  * `-lock=false`
  * `-lock-timeout=DURATION`
  ...
  * `-var 'NAME=VALUE'`
  * `-var-file=FILE`
* [`show`](/mnptu/cli/commands/show)
  * `-no-color`
  * `-json`
  * (both with and without a plan file)
* [`providers`](/mnptu/cli/commands/providers) (with no subcommand)
* [`providers lock`](/mnptu/cli/commands/providers/lock)
  * `-fs-mirror=PATH`
  * `-net-mirror=URL`
  * `-platform=OS_ARCH`
* [`providers mirror`](/mnptu/cli/commands/providers/mirror)
  * `-platform=OS_ARCH`
* [`providers schema`](/mnptu/cli/commands/providers/schema)
  * `-json`
* [`fmt`](/mnptu/cli/commands/fmt)
  * `-list=false`
  * `-write=false`
  * `-diff`
  * `-recursive`
  * `-check`
* [`version`](/mnptu/cli/commands/version)
  * `-json`
* [`output`](/mnptu/cli/commands/output)
  * `-no-color`
  * `-json`
  * `-raw`
* [`taint`](/mnptu/cli/commands/taint)
  * `-allow-missing`
  * `-lock=false`
  * `-lock-timeout=DURATION`
  * `-ignore-remote-version`
* [`untaint`](/mnptu/cli/commands/untaint)
  * `-allow-missing`
  * `-lock=false`
  * `-lock-timeout=DURATION`
  * `-ignore-remote-version`
* [`force-unlock`](/mnptu/cli/commands/force-unlock)
  * `-force`
* [`state list`](/mnptu/cli/commands/state/list)
  * `-id=ID`
* [`state pull`](/mnptu/cli/commands/state/pull)
* [`state push`](/mnptu/cli/commands/state/push)
  * `-force`
  * `-lock=false`
  * `-lock-timeout=DURATION`
* [`state show`](/mnptu/cli/commands/state/show)
  * `-ignore-remote-version`
* [`login`](/mnptu/cli/commands/login)
...
v1.x series. If you are building automation around mnptu, use only the
commands above to avoid the need for changes when upgrading.

Please note that although mnptu's internal logs (via the `TF_LOG`
...
processing of logs by mnptu developers.
...
While you can freely use these commands when running mnptu interactively
...
* `destroy` (consider `mnptu apply -destroy` instead)
* `console`
* `get` (consider `mnptu init` instead)
* `graph`
* `import`
* `push`
* `refresh` (consider `mnptu apply -refresh-only` instead)
* `state mv`
...
The mnptu codebase includes various unit and integration tests intended to
help us to notice accidental behavior regressions before they ship in a stable
version.

However, mnptu is a relatively complex system with many different features
...
improve mnptu's test coverage over time.
...
promises in this document, please open an issue in mnptu's GitHub
...
regressions by upgrading promptly to new minor and patch releases of mnptu
and reporting any compatibility problems you encounter in mnptu's GitHub
repository.

### Pragmatic Exceptions

We are making the promises above in good faith, with the intent that your
investment in writing mnptu modules or automation will not be invalidated
by future changes to mnptu. However, broad promises like the above can't
possibly cover all nuances of practical problems that might arise as we
continue to develop mnptu.
...
* External Dependencies: mnptu's behavior depends on interfaces provided
  ...
  including potentially removing or changing features that mnptu's own
  features depend on. In that case, if there is no suitable replacement
  mechanism then we may need to change mnptu's design to work within the
  new constraints.
...
* Bugs in New Features: If we introduce a new feature to mnptu and the
  ...
  reflect it.
* Regressions in Existing Features: If we learn that a new mnptu release
...
* Situations we cannot anticipate: Although we've made an effort to consider
  various specific exceptional situations here, mnptu and its development
  ...
  the future of mnptu. In those situations, we will always do our best to
