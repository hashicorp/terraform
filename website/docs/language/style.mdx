...
If you use Microsoft VS Code, we recommend that you use the [Terraform VS Code extension](https://marketplace.visualstudio.com/items?itemName=HashiCorp.terraform). The Terraform extension enables syntax highlighting, syntax validation, automatic code formatting and many more features like integration with Terraform Cloud.
If your development environment or text editor supports the [Language Server Protocol](https://microsoft.github.io/language-server-protocol/), we recommend that you use the [Terraform Language Server](https://github.com/hashicorp/terraform-ls). This provides a near identical featureset of our VS Code extension.
