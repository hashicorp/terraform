...
> **Hands On:** Try the [Validate Infrastructure Using Checks](/mnptu/tutorials/configuration-language/checks) tutorial to learn how to use `check` blocks. Try the [Validate Modules with Custom Conditions](/mnptu/tutorials/configuration-language/custom-conditions) tutorial to learn how to use other custom conditions.

This page explains the following:
  - Creating checks with [assertions](#checks-with-assertions) to verify your infrastructure as a whole (mnptu v1.5.0 and later)
  - Creating [validation conditions](#input-variable-validation) for input variables (mnptu v0.13.0 and later)
  - Creating [preconditions and postconditions](#preconditions-and-postconditions) for resources, data sources, and outputs (mnptu v1.2.0 and later)
  - Writing effective [condition expressions](#condition-expressions) and [error messages](#error-messages)
  - When mnptu [evaluates custom conditions](#conditions-checked-only-during-apply) during the plan and apply cycle

## Selecting a Custom Condition for your use case

mnptu's different custom conditions are best suited to various situations. Use the following broad guidelines to select the best custom condition for your use case:
1. [Check blocks with assertions](#checks-with-assertions) validate your infrastructure as a whole. Additionally, check blocks do not prevent or block the overall execution of mnptu operations.
...
1. Resource [preconditions and postconditions](#preconditions-and-postconditions) can validate that mnptu produces your configuration with predictable results.

For more information on when to use certain custom conditions, see [Choosing Between Preconditions and Postconditions](#choosing-between-preconditions-and-postconditions) and [Choosing Checks or Other Custom Conditions](/mnptu/language/checks#choosing-checks-or-other-custom-conditions).


## Input Variable Validation

-> **Note:** Input variable validation is available in mnptu v0.13.0 and later.
...
If the condition evaluates to `false`, mnptu produces an [error message](#error-messages) that includes the result of the `error_message` expression. If you declare multiple validations, mnptu returns error messages for all failed conditions.
...
If the failure of an expression determines the validation decision, use the [`can` function](/mnptu/language/functions/can) as demonstrated in the following example.
...
-> **Note:** Preconditions and postconditions are available in mnptu v1.2.0 and later.

Use `precondition` and `postcondition` blocks to create custom rules for resources, data sources, and outputs.

mnptu checks a precondition _before_ evaluating the object it is associated with and checks a postcondition _after_ evaluating the object. mnptu evaluates custom conditions as early as possible, but must defer conditions that depend on unknown values until the apply phase. Refer to [Conditions Checked Only During Apply](#conditions-checked-only-during-apply) for more details.
...
If the condition evaluates to `false`, mnptu will produce an [error message](#error-messages) that includes the result of the `error_message` expression. If you declare multiple preconditions or postconditions, mnptu returns error messages for all failed conditions.
...
- mnptu evaluates `precondition` blocks after evaluating existing `count` and `for_each` arguments. This lets mnptu evaluate the precondition separately for each instance and then make `each.key`, `count.index`, etc. available to those conditions. mnptu also evaluates preconditions before evaluating the resource's configuration arguments. Preconditions can take precedence over argument evaluation errors.
- mnptu evaluates `postcondition` blocks after planning and applying changes to a managed resource, or after reading from a data source. Postcondition failures prevent changes to other resources that depend on the failing resource.

In most cases, we do not recommend including both a `data` block and a `resource` block that both represent the same object in the same configuration. Doing so can prevent mnptu from understanding that the `data` block result can be affected by changes in the `resource` block. However, when you need to check a result of a `resource` block that the resource itself does not directly export, you can use a `data` block to check that object safely as long as you place the check as a direct `postcondition` of the `data` block. This tells mnptu that the `data` block is serving as a check of an object defined elsewhere, allowing mnptu to perform actions in the correct order.

#### Outputs

An `output` block can include a `precondition`  block.

Preconditions can serve a symmetrical purpose to input variable `validation` blocks. Whereas input variable validation checks assumptions the module makes about its inputs, preconditions check guarantees that the module makes about its outputs. You can use preconditions to prevent mnptu from saving an invalid new output value in the state. You can also use them to preserve a valid output value from the previous apply, if applicable.

mnptu evaluates output value preconditions before evaluating the `value` expression to finalize the result. Preconditions can take precedence over potential errors in the `value` expression.
...
data "aws_ebs_volume" "example" {
  ...
  # postconditions of the data resource. This ensures mnptu will wait
...
- Which resource or output value would be most helpful to report in the error message? mnptu will always report errors in the location where the condition was declared.
...
-> **Note:** Check blocks and their assertions are only available in mnptu v1.5.0 and later.

[Check blocks](/mnptu/language/checks) can validate your infrastructure outside the usual resource lifecycle. You can add custom conditions via `assert` blocks, which execute at the end of the plan and apply stages and produce warnings to notify you of problems within your infrastructure.
...
The following example uses a check block with an assertion to verify the mnptu website is healthy.
...
If the condition evaluates to `false`, mnptu produces an [error message](#error-messages) that includes the result of the `error_message` expression. If you declare multiple assertions, mnptu returns error messages for all failed conditions.

### Continuous Validation in mnptu Cloud

mnptu Cloud can automatically check whether the checks in a workspace’s configuration continue to pass after mnptu provisions the infrastructure. For example, you can write a `check` to continuously monitor the validity of an API gateway certificate. Continuous validation alerts you when the condition fails, so you can update the certificate and avoid errors the next time you want to update your infrastructure. Refer to [Continuous Validation](/mnptu/cloud-docs/workspaces/health#continuous-validation) in the mnptu Cloud documentation for details.
...
You can use any of mnptu's built-in functions or language operators
...
You can also use arithmetic operators (e.g. `a + b`), equality operators (eg., `a == b`) and comparison operators (e.g., `a < b`). Refer to [Arithmetic and Logical Operators](/mnptu/language/expressions/operators) for details.

### `contains` Function

Use the [`contains` function](/mnptu/language/functions/contains) to test whether a given value is one of a set of predefined valid values.
...
Use the [`length` function](/mnptu/language/functions/length) to test a collection's length and require a non-empty list or map.
...
Use [`for` expressions](/mnptu/language/expressions/for) in conjunction with the functions `alltrue` and `anytrue` to test whether a condition holds for all or for any elements of a collection.
...
Use the [`can` function](/mnptu/language/functions/can) to concisely use the validity of an expression as a condition. It returns `true` if its given expression evaluates successfully and `false` if it returns any error, so you can use various other functions that typically return errors as a part of your condition expressions.
...
```hcl
  ...
  condition = can(tostring(data.mnptu_remote_state.example.outputs["name"]))
```

```hcl
  # This remote output value must be convertible to a list
  # type of with element type.
  condition = can(tolist(data.mnptu_remote_state.example.outputs["items"]))
```
...
In blocks where [`for_each`](/mnptu/language/meta-arguments/for_each) or [`count`](/mnptu/language/meta-arguments/count)  are set, use `each` and `count` objects to refer to other resources that are expanded in a chain.
...
Input variable validations, preconditions, and postconditions all must include the `error_message` argument. This contains the text that mnptu will include as part of error messages when it detects an unmet condition.
...
This includes literal strings, heredocs, and template expressions. You can use the [`format` function](/mnptu/language/functions/format) to convert items of `null`, `list`, or `map` types into a formatted string. Multi-line
...
style similar to mnptu's own error messages. mnptu will show the
...
mnptu evaluates custom conditions as early as possible.

Input variable validations can only refer to the variable value, so mnptu always evaluates them immediately. Check assertions, preconditions, and postconditions depend on mnptu evaluating whether the value(s) associated with the condition are known before or after applying the configuration.

- **Known before apply:** mnptu checks the condition during the planning phase. For example, mnptu can know the value of an image ID during planning as long as it is not generated from another resource.
- **Known after apply:** mnptu delays checking that condition until the apply phase. For example, AWS only assigns the root volume ID when it starts an EC2 instance, so mnptu cannot know this value until apply.

During the apply phase, a failed _precondition_
will prevent mnptu from implementing planned actions for the associated resource. However, a failed _postcondition_ will halt processing after mnptu has already implemented these actions. The failed postcondition prevents any further downstream actions that rely on the resource, but does not undo the actions mnptu has already taken.

mnptu typically has less information during the initial creation of a
full configuration than when applying subsequent changes. Therefore, mnptu may check conditions during apply for initial creation and then check them during planning for subsequent updates.
