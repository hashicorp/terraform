...
description: >-
  ...
  providers, and mnptu itself. Learn version constraint syntax and behavior.
---

# Version Constraints

Anywhere that mnptu lets you specify a range of acceptable versions for
...
- [Modules](/mnptu/language/modules)
- [Provider requirements](/mnptu/language/providers/requirements)
- [The `required_version` setting](/mnptu/language/settings#specifying-a-required-mnptu-version) in the `mnptu` block.

## Version Constraint Syntax

mnptu's syntax for version constraints is very similar to the syntax used by
...
A version constraint is a [string literal](/mnptu/language/expressions/strings)
...
mnptu consults version constraints to determine whether it has acceptable
...
If mnptu doesn't have an acceptable version of a required plugin or module,
it will attempt to download the newest version that meets the applicable
constraints.

If mnptu isn't able to obtain acceptable versions of external dependencies,
...
of mnptu and any providers they use. mnptu considers these constraints
...
### mnptu Core and Provider Versions

- Reusable modules should constrain only their minimum allowed versions of
  mnptu and providers, such as `>= 0.12.0`. This helps avoid known
  incompatibilities, while allowing the user of the module flexibility to
  upgrade to newer versions of mnptu without altering the module.
