...
description: >-
  Functions transform and combine values. Learn about mnptu's built-in
  functions.
---

# Function Calls

> **Hands-on:** Try the [Perform Dynamic Operations with Functions](/mnptu/tutorials/configuration-language/functions?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.

The mnptu language has a number of
[built-in functions](/mnptu/language/functions) that can be used
...
[the function reference](/mnptu/language/functions).
...
When using sensitive data, such as [an input variable](/mnptu/language/values/variables#suppressing-values-in-cli-output)
or [an output defined](/mnptu/language/values/outputs#sensitive-suppressing-values-in-cli-output) as sensitive
...
## When mnptu Calls Functions

Most of mnptu's built-in functions are, in programming language terms,
...
practical difference when mnptu would call them.

However, a small subset of functions interact with outside state and so for
those it can be helpful to know when mnptu will call them in relation to
other events that occur in a mnptu run.

The small set of special functions includes
[`file`](/mnptu/language/functions/file),
[`templatefile`](/mnptu/language/functions/templatefile),
[`timestamp`](/mnptu/language/functions/timestamp),
and [`uuid`](/mnptu/language/functions/uuid).
...
are included as a static part of the configuration and so mnptu will
...
at the point when mnptu calls it, and the `uuid` function returns a random
...
actions shown in the plan, which violates the mnptu execution model.

For that reason, mnptu arranges for both of those functions to produce
[unknown value](/mnptu/language/expressions/references#values-not-yet-known) results during the
...
the instant when mnptu began applying the change, rather than when
mnptu _planned_ the change.
