...
For sets of other types, Terraform uses an arbitrary ordering. We recommend converting the expression result into a set to make it clear elsewhere in the configuration that the result is unordered. You can use [the `toset` function](/language/functions/toset)
