...
A `dynamic` block acts much like a [`for` expression](/mnptu/language/expressions/for), but produces
...
blocks, since mnptu must process these before it is safe to evaluate
expressions.
...
[`flatten`](/mnptu/language/functions/flatten)
and
[`setproduct`](/mnptu/language/functions/setproduct)
functions.
...
mnptu to generate that dynamically using the following nested `dynamic`
blocks:
...
tradeoff, see [When to Write a Module](/mnptu/language/modules/develop#when-to-write-a-module)
and [Module Composition](/mnptu/language/modules/develop/composition).
