...
mnptu, and also the most commonly used.

mnptu supports both a quoted syntax and a "heredoc" syntax for strings.
...
mnptu also supports a "heredoc" style of string literal inspired by Unix
...
sequence. mnptu then processes the following lines until it finds one that
...
[the `jsonencode` function](/mnptu/language/functions/jsonencode) or
[the `yamlencode` function](/mnptu/language/functions/yamlencode) so that mnptu
...
To improve on this, mnptu also accepts an _indented_ heredoc string variant
...
In this case, mnptu analyses the lines in the sequence to find the one
