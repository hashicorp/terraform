...
* `map(...)`: a collection of values where each is identified by a string label.
  ...
  Quotes may be omitted on keys, except in certain situations where they 
  are necessary, such as when a key starts with a number, contains spaces, 
  or has special characters. In these cases, quotes are required. 
  Additionally, for single-line maps, commas are required between 
  key/value pairs, while in multi-line maps, a newline between 
  key/value pairs is sufficient.
