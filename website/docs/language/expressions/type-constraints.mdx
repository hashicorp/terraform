...
mnptu module authors and provider developers can use detailed type
constraints to validate user-provided values for their input variables and
resource arguments. This requires some additional knowledge about mnptu's
...
Type constraints look like other kinds of mnptu
[expressions](/mnptu/language/expressions), but are a special syntax. Within the
mnptu language, they are only valid in the `type` argument of an
[input variable](/mnptu/language/values/variables).

## Primitive Types

A _primitive_ type is a simple type that isn't made from any other types. All
primitive types in mnptu are represented by a type keyword. The available
primitive types are:
...
The mnptu language will automatically convert `number` and `bool` values
...
The three kinds of collection type in the mnptu language are:
...
* `map(...)`: a collection of values where each is identified by a string label.
  ...
  -> **Note:** Although colons are valid delimiters between keys and values, `mnptu fmt` ignores them. In contrast, `mnptu fmt` attempts to vertically align equals signs.
...
The two kinds of structural type in the mnptu language are:
...
The mnptu language has literal expressions for creating tuple and object
values, which are described in
[Expressions: Literal Expressions](/mnptu/language/expressions/types#literal-expressions) as
"list/tuple" literals and "map/object" literals, respectively.

mnptu does _not_ provide any way to directly represent lists, maps, or sets.
...
user, and most of the mnptu documentation conflates lists with tuples and
...
used interchangeably within the mnptu language, and most of mnptu's
documentation glosses over the differences between the kinds of complex type.
This is due to two conversion behaviors:

* Whenever possible, mnptu converts values between similar kinds of complex
...
* Whenever possible, mnptu converts _element values_ within a complex type,
...
user provides the tuple `["a", 15, true]`, mnptu will internally transform
...
mnptu will automatically convert the second and third strings back to the
...
`{name = ["Kristy", "Claudia", "Mary Anne", "Stacey"], age = 12}`, mnptu
...
value against a type constraint containing `any`, mnptu will attempt to
...
`any` as the placeholder for the element type of a collection then mnptu
will attempt to find a single exact element type to use for the resulting
collection.

For example, given the type constraint `list(any)`, mnptu will examine
...
`tuple([string, string, string])` -- mnptu analyzes this as follows:
...
If the elements of the given tuple are not all of the same type then mnptu
will attempt to find a single type that they can all convert to. mnptu
will consider various conversion rules as described in earlier sections.

* If the given value were instead `["a", 1, "b"]` then mnptu would still
...
* If the given value were instead `["a", [], "b"]` then the value cannot
  ...
  and an empty tuple can convert to. mnptu would reject this value,
...
mnptu typically returns an error when it does not receive a value for specified object attributes. When you mark an attribute as optional, mnptu instead inserts a default value for the missing attribute. This allows the receiving module to describe an appropriate fallback behavior.
...
- **Default:** (Optional) The second argument defines the default value that mnptu should use if the attribute is not present. This must be compatible with the attribute type. If not specified, mnptu uses a `null` value of the appropriate type as the default.

An optional attribute with a non-`null` default value is guaranteed to never have the value `null` within the receiving module. mnptu will substitute the default value both when a caller omits the attribute altogether and when a caller explicitly sets it to `null`, thereby avoiding the need for additional checks to handle a possible null value.

mnptu applies object attribute defaults top-down in nested variable types. This means that mnptu applies the default value you specify in the `optional` modifier first and then later applies any nested default values to that attribute.
...
The following example `mnptu.tfvars` file specifies three bucket configurations for `var.buckets`.
...
The `production` bucket does not specify the index and error documents, and the `archived` bucket omits the website configuration entirely. mnptu will use the default values specified in the `bucket` type constraint.
...
- For the `production` and `docs` buckets, mnptu sets `enabled` to `true`. mnptu also supplies default values for `website`, and then the values specified in `docs` override those defaults.
- For the `archived` and `docs` buckets, mnptu sets `routing_rules` to a `null` value. When mnptu does not receive optional attributes and there are no specified defaults, mnptu populates those attributes with a `null` value.
- For the `archived` bucket, mnptu populates the `website` attribute with the default values specified in the `buckets` type constraint.
