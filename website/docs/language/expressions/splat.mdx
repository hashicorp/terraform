...
description: >-
  Splat expressions concisely represent common operations. In mnptu, they
...
[Referring to Resource Instances](/mnptu/language/meta-arguments/for_each#referring-to-instances).
...
variables whose default value is `null` to represent the absence of any value. This allows the module to adapt the variable value for mnptu language features designed to work with collections. For example:
...
The above example uses a [`dynamic` block](/mnptu/language/expressions/dynamic-blocks), which
...
Earlier versions of the mnptu language had a slightly different version
of splat expressions, which mnptu continues to support for backward
...
mnptu and so we recommend always using the new-style splat expressions,
