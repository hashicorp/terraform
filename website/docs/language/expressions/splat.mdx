...
of objects, so you can't directly use splat expressions with those resources.
For more information, see
[Referring to Resource Instances](/terraform/language/meta-arguments/for_each#referring-to-instances).

It is however possible to use the [`values`](/language/functions/values) function to get the
values of a map as a list and then make a splat expression using that list. For example:
