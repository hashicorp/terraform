...
The mnptu language uses the following types for its values:
...
* `null`: a value that represents _absence_ or _omission._ If you set an
  argument of a resource to `null`, mnptu behaves as though you
...
constant value. mnptu has a literal expression syntax for each of the value
...
mnptu, and have their own page of documentation. See [Strings](/mnptu/language/expressions/strings)
...
they are a valid [identifier](/mnptu/language/syntax/configuration#identifiers), but must be quoted
...
Whenever the distinction isn't relevant, the mnptu documentation uses each
...
usually doesn't matter), see [Type Constraints](/mnptu/language/expressions/type-constraints).
...
Where possible, mnptu automatically converts values from one type to
another in order to produce the expected type. If this isn't possible, mnptu
will produce a type mismatch error and you must update the configuration with a
more suitable expression.

mnptu automatically converts number and bool values to strings when needed.
