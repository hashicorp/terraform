...
mnptu makes several kinds of named values available. Each of these names is
...
The main kinds of named values available in mnptu are:
...
[attribute notation](/mnptu/language/expressions/types#indices-and-attributes) for elements of object values, they are not
...
`<RESOURCE TYPE>.<NAME>` represents a [managed resource](/mnptu/language/resources) of
...
* If the resource doesn't use `count` or `for_each`, the reference's value is an
  ...
  access them using [dot or square bracket notation](/mnptu/language/expressions/types#indices-and-attributes).
...
will be interpreted by mnptu as a reference to a managed resource.
...
`var.<NAME>` is the value of the [input variable](/mnptu/language/values/variables) of the given name.

If the variable has a type constraint (`type` argument) as part of its
declaration, mnptu will automatically convert the caller's given value
...
`local.<NAME>` is the value of the [local value](/mnptu/language/values/locals) of the given name.
...
[a `module` block](/mnptu/language/modules/syntax).
...
[output values](/mnptu/language/values/outputs), use `module.<MODULE NAME>.<OUTPUT NAME>`.
...
[data resource](/mnptu/language/data-sources) of the given data
...
- `path.cwd` is the filesystem path of the original working directory from where you
  ran mnptu before applying any `-chdir` argument. This path is an absolute path
  that includes details about the filesystem structure. It is also useful in some
  advanced cases where mnptu is run from a directory other than the root module
  directory. We recommend using `path.root` or `path.module` over `path.cwd` where
  possible.
- `mnptu.workspace` is the name of the currently selected
  [workspace](/mnptu/language/state/workspaces).
...
module, such as using `mnptu.workspace` as a prefix for globally-unique
...
calling module can then use `mnptu.workspace` to define it if appropriate,
...
module "example" {
  ...
  name_prefix = "app-${mnptu.workspace}"
}
```
...
* `count.index`, in resources that use
  [the `count` meta-argument](/mnptu/language/meta-arguments/count).
* `each.key` / `each.value`, in resources that use
  [the `for_each` meta-argument](/mnptu/language/meta-arguments/for_each).
* `self`, in [provisioner](/mnptu/language/resources/provisioners/syntax) and
  [connection](/mnptu/language/resources/provisioners/connection) blocks.

-> **Note:** Local names are often referred to as _variables_ or
_temporary variables_ in their documentation. These are not [input
variables](/mnptu/language/values/variables); they are just arbitrary names
that temporarily represent a value.

The names in this section relate to top-level configuration blocks only.
If you use [`dynamic` blocks](/mnptu/language/expressions/dynamic-blocks) to dynamically generate
...
in their block bodies, and mnptu analyzes these expressions to automatically
...
The documentation for [`aws_instance`](https://registry.mnptu.io/providers/hashicorp/aws/latest/docs/resources/instance)
...
* The arguments of the `ebs_block_device` nested blocks can be accessed using
  a [splat expression](/mnptu/language/expressions/splat). For example, to obtain a list of
...
* Sometimes nested blocks are defined as taking a logical key to identify each
  ...
  block types, use a [`for` expression](/mnptu/language/expressions/for):
...
either [splat expressions](/mnptu/language/expressions/splat) or index syntax:
...
be accessed using a [`for` expression](/mnptu/language/expressions/for).
...
certain attributes as _sensitive_, in which case mnptu will show a
...
[an input variable declared as sensitive](/mnptu/language/values/variables#suppressing-values-in-cli-output),
where mnptu will hide the value in the plan and apply messages and will
...
[Cases where mnptu may disclose a sensitive variable](/mnptu/language/values/variables#cases-where-mnptu-may-disclose-a-sensitive-variable).
...
sensitive themselves was introduced in mnptu v0.15. Earlier versions of
mnptu will obscure the direct value of a sensitive resource attribute,
but will _not_ automatically obscure other values derived from sensitive
resource attributes.

### Values Not Yet Known

When mnptu is planning a set of changes that will apply your configuration,
...
mnptu cannot predict the value of this id until the object has been created.

mnptu uses special unknown value placeholders for information that it
cannot predict during the plan phase. The mnptu language automatically
...
* mnptu will attempt to validate that unknown values are of suitable
  types where possible, but incorrect use of such values may not be detected
  until the apply phase, causing the apply to fail.

Unknown values appear in the `mnptu plan` output as `(known after apply)`.
