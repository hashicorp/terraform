...
The mnptu language has a set of operators for both arithmetic and logic,
...
parentheses, higher levels will be evaluated first, so mnptu will interpret
...
given values to be of a particular type. mnptu will attempt to convert
...
mnptu supports some other less-common numeric operations as
[functions](/mnptu/language/expressions/function-calls). For example, you can calculate exponents
using
[the `pow` function](/mnptu/language/functions/pow).
...
mnptu does not have an operator for the "exclusive OR" operation. If you
know that both operators are boolean values then exclusive OR is equivalent
to the `!=` ("not equal") operator.

The logical operators in mnptu do not short-circuit, meaning `var.foo || var.foo.bar` will produce an error message if `var.foo` is `null` because both `var.foo` and `var.foo.bar` are evaluated.
