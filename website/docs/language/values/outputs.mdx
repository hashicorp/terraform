...
You can mark `output` values as `ephemeral` to avoid persisting those values in your state or plan files. Use the `ephemeral` argument for temporary values that you do not want Terraform to save, such as short-lived credentials, session tokens, or other transient data. 

You can mark an `output` as ephemeral by setting the `ephemeral` field to `true`:

```hcl
output "session_token" {
  value     = aws_iam_role.session_token
  description = "Temporary session token for AWS access."
  ephemeral = true
}
```
The value of ephemeral `output` blocks are available to Terraform during the plan and apply steps. However Terraform still hides the values for ephemeral outputs when it prints out the results of a plan or apply.
...
session_token = "session_token" (ephemeral)

Output values marked "(ephemeral)" may change on the next apply, and are not saved to the state.
```

At the end of a plan or apply operation, Terraform does not persist the value of any ephemeral outputs and does not write them to the state file. 
