...
description: Output values are the return values of a mnptu module.
---

# Output Values

Output values make information about your infrastructure available on the
command line, and can expose information for other mnptu configurations to
use. Output values are similar to return values in programming languages.

> **Hands-on:** Try the [Output Data From
> mnptu](/mnptu/tutorials/configuration-language/outputs)
> tutorial.
...
- A root module can use outputs to print certain values in the CLI output after
  running `mnptu apply`.
- When using [remote state](/mnptu/language/state/remote), root module outputs can be
  accessed by other configurations via a
  [`mnptu_remote_state` data source](/mnptu/language/state/remote-state-data).

Resource instances managed by mnptu each export attributes whose values
...
valid [identifier](/mnptu/language/syntax/configuration#identifiers). In a root module, this name is
displayed to the user; in a child module, it can be used to access the output's
value.

The `value` argument takes an [expression](/mnptu/language/expressions)
...
-> **Note:** Outputs are only rendered when mnptu applies your plan. Running
`mnptu plan` will not render outputs.
...
mnptu will hide values marked as sensitive in the messages from
`mnptu plan` and `mnptu apply`. In the following scenario, our root
...
-> **Note:** In mnptu versions prior to mnptu 0.14, setting an output
value in the root module as sensitive would prevent mnptu from showing its
value in the list of outputs at the end of `mnptu apply`. However, the
...
child modules, the dependencies of that output value allow mnptu to
correctly determine the dependencies between resources defined in different
modules.

Just as with
[resource dependencies](/mnptu/language/resources/behavior#resource-dependencies),
mnptu analyzes the `value` expression for an output value and automatically
