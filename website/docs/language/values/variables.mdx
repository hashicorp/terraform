...
#### Variable Definition Precedence within Terraform Tests

Within Terraform test files variable values can also be specified within
`variables` blocks, either nested within `run` blocks or defined directly within
the file.

Variables defined here take precedence over all other mechanisms, with variables
defined within `run` blocks taking precedence over those defined within the
file.
