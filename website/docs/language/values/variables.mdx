...
-> Ephemeral variables are in public beta and available in Terraform v1.1.0 and later. The workflows of ephemeral variables are subject to change.
...
If another expression references an `ephemeral` variable, then that expression implicitly becomes ephemeral as well.
