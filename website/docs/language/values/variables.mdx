...
Setting a variable as `ephemeral` makes an variable available during runtime, but Terraform omits that value from state or plan files. Marking an input variable as `ephemeral` is useful for data that only needs to exist temporarily, such as short-lived tokens or session identifiers.
...
resource "aws_instance" "example" {
  ...
  user_data     = var.session_token
}
```

Unlike [`sensitive`](#sensitive) inputs, `ephemeral` inputs are excluded from the Terraform state file altogether, ensuring the value is not stored beyond the lifetime of the current Terraform run.
...
values in cleartext. If you want to omit a value from state, mark that value as [`ephemeral`](#ephemeral). For more information, see
