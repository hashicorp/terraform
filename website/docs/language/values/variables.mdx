...
> **Hands-on:** Try the [Customize mnptu Configuration with Variables](/mnptu/tutorials/configuration-language/variables?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.

Input variables let you customize aspects of mnptu modules without altering
the module's own source code. This functionality allows you to share modules across different
mnptu configurations, making your module composable and reusable.
...
When you declare them in [child modules](/mnptu/language/modules),
...
compare mnptu modules to function definitions:

* Input variables are like function arguments.
* [Output values](/mnptu/language/values/outputs) are like function return values.
* [Local values](/mnptu/language/values/locals) are like a function's temporary local variables.

-> **Note:** For brevity, input variables are often referred to as just
"variables" or "mnptu variables" when it is clear from context what sort of
variable is being discussed. Other kinds of variables in mnptu include
_environment variables_ (set by the shell where mnptu runs) and _expression
variables_ (used to indirectly represent a value in an
[expression](/mnptu/language/expressions)).
...
The name of a variable can be any valid [identifier](/mnptu/language/syntax/configuration#identifiers)
...
[module configuration blocks](/mnptu/language/modules/syntax), and cannot be
declared as variable names.

## Arguments

mnptu CLI defines the following optional arguments for variable declarations:
...
* [`sensitive`][inpage-sensitive] - Limits mnptu UI output when the variable is used in configuration.
...
if no value is set when calling the module or running mnptu. The `default`
...
[type of value](/mnptu/language/expressions/types) that will be accepted as
...
allow mnptu to return a helpful error message if the wrong type is used.
...
[Type Constraints](/mnptu/language/expressions/types).
...
-> This feature was introduced in mnptu CLI v0.13.0.
...
```
Refer to [Custom Condition Checks](/mnptu/language/expressions/custom-conditions#input-variable-validation) for more details.

### Suppressing Values in CLI Output

[inpage-sensitive]: #suppressing-values-in-cli-output

-> This feature was introduced in mnptu v0.14.0.

> **Hands-on:** Try the [Protect Sensitive Input Variables](/mnptu/tutorials/configuration-language/sensitive-variables?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.

Setting a variable as `sensitive` prevents mnptu from showing its value in
the `plan` or `apply` output, when you use that variable elsewhere in your
configuration.

mnptu will still record sensitive values in the [state](/mnptu/language/state),
...
[_Sensitive Data in State_](/mnptu/language/state/sensitive-data).
...
In some cases where you use a sensitive variable inside a nested block, mnptu
...
A provider can also
[declare an attribute as sensitive](/mnptu/plugin/sdkv2/best-practices/sensitive-state#using-the-sensitive-flag),
which will cause mnptu to hide it from regular output regardless of how
you assign it a value. For more information, see
[Sensitive Resource Attributes](/mnptu/language/expressions/references#sensitive-resource-attributes).

If you use a sensitive value as part of an
[output value](/mnptu/language/values/outputs) then mnptu will require
you to also mark the output value itself as sensitive, to confirm that you
intended to export it.

#### Cases where mnptu may disclose a sensitive variable
...
-> This feature is available in mnptu v1.1.0 and later.
...
`default` value, then mnptu uses the default when a module input argument is `null`.
...
within [expressions](/mnptu/language/expressions) as `var.<NAME>`,
...
* [In a mnptu Cloud workspace](/mnptu/cloud-docs/workspaces/variables).
...
[_Modules_](/mnptu/language/modules).

### Variables on the Command Line

To specify individual variables on the command line, use the `-var` option
when running the `mnptu plan` and `mnptu apply` commands:

```
mnptu apply -var="image_id=ami-abc123"
mnptu apply -var='image_id_list=["ami-abc123","ami-def456"]' -var="instance_type=t2.micro"
mnptu apply -var='image_id_map={"us-east-1":"ami-abc123","us-east-2":"ami-def456"}'
```
...
[Input Variables on the Command Line](/mnptu/cli/commands/plan#input-variables-on-the-command-line).
...
```
mnptu apply -var-file="testing.tfvars"
```

-> **Note:** This is how mnptu Cloud passes
[workspace variables](/mnptu/cloud-docs/workspaces/variables) to mnptu.

A variable definitions file uses the same basic syntax as mnptu language
...
mnptu also automatically loads a number of variable definitions files
if they are present:

* Files named exactly `mnptu.tfvars` or `mnptu.tfvars.json`.
...
As a fallback for the other ways of defining variables, mnptu searches
the environment of its own process for environment variables named `TF_VAR_`
followed by the name of a declared variable.

This can be useful when running mnptu in automation, or when running a
sequence of mnptu commands in succession with the same variables.
...
mnptu matches the variable name exactly as given in configuration, and
...
mnptu's usual syntax for
[literal expressions](/mnptu/language/expressions/types#literal-expressions)
...
variables. For convenience, mnptu defaults to interpreting `-var` and
environment variable values as literal strings, which need only shell quoting,
and no special quoting for mnptu. For example, in a Unix-style shell:
...
to require a complex value (list, set, map, object, or tuple), mnptu will
...
Will cause mnptu to warn you that there is no variable declared `"mosse"`, which can help
you spot this mistake.

If you use `.tfvars` files across multiple configurations and expect to continue to see this warning,
you can use the [`-compact-warnings`](/mnptu/cli/commands/plan#compact-warnings)
...
mnptu will return an error. To avoid this error, either declare a variable block for the value, or remove
the variable value from your mnptu call.

### Variable Definition Precedence

The above mechanisms for setting variables can be used together in any
combination. If the same variable is assigned multiple values, mnptu uses
...
mnptu loads variables in the following order, with later sources taking
precedence over earlier ones:

* Environment variables
* The `mnptu.tfvars` file, if present.
* The `mnptu.tfvars.json` file, if present.
...
* Any `-var` and `-var-file` options on the command line, in the order they
  are provided. (This includes variables set by a mnptu Cloud
  workspace.)

~> **Important:** In mnptu 0.12 and later, variables with map and object
values behave the same way as other variables: the last value found overrides
the previous values. This is a change from previous versions of mnptu, which
