...
-> **Note**: Ephemeral variables are in public beta and available in Terraform v1.1.0 and later. The workflows of ephemeral variables are subject to change.
...
locals {
  session_token_test = var.session_token ? var.session_token : "default"
}

resource "aws_instance" "example" {
  instance_type = "t2.micro"
  # You cannot use ephemeral variables in non-ephemeral contexts. So, we
  # use the local value here.
  user_data = local.session_token_test
}
```
