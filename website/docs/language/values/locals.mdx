...
The `local.session_token` value is implicitly ephemeral because it relies on an ephemeral variable. Terraform treats ephemeral local values the same way as it does other ephemeral values. Terraform does not store ephemeral local values in state or plan files, or output them as part of a plan or apply.
