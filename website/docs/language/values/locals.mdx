...
You can explicitly mark a `local` block as ephemeral by setting the `ephemeral` attribute to `true`. Ephemeral local values are available during the current Terraform operation, and Terraform does not store them in state or plan files or output as part of a plan or apply.
