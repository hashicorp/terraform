...
Local values implicitly become ephemeral if you reference an ephemeral value when
you assign that local a value. For example, you can create a local that references an ephemeral `service_token`.
...
The `local.session_token` value is implicitly ephemeral because it relies on an ephemeral variable. Terraform treats ephemeral local values the same way as it does other ephemeral values.
Terraform does not store ephemeral local values in state or plan files, or output them as part of a plan
or apply.
