...
description: >-
  Key constructs of the native mnptu language syntax, including identifiers,
...
that can appear in the mnptu language. This page describes the lower-level
syntax of the language in more detail, revealing the building blocks that
those constructs are built from.

This page describes the _native syntax_ of the mnptu language, which is
a rich language designed to be relatively easy for humans to read and write.
The constructs in the mnptu language can also be expressed in
[JSON syntax](/mnptu/language/syntax/json), which is harder for humans
to read and edit but easier to generate and parse programmatically.

This low-level syntax of the mnptu language is defined in terms of a
...
order to use mnptu, and so this page summarizes the most important
...
The mnptu language syntax is built around two key syntax constructs:
arguments and blocks.
...
[expressions](/mnptu/language/expressions), which allow the value to
either be specified literally or generated from other values programmatically.

-> **Note:** mnptu's configuration language is based on a more general
...
this context, and experienced mnptu users might use either term in casual
conversation. But because mnptu also interacts with several _other_ things
called "attributes" (in particular, mnptu resources have attributes like
`id` that can be referenced from expressions but can't be assigned values in
configuration), we've chosen to use "argument" in the mnptu documentation
...
The mnptu language uses a limited number of _top-level block types,_ which
are blocks that can appear outside of any other block in a configuration file.
Most of mnptu's features (including resources, input variables, output
values, data sources, etc.) are implemented as top-level blocks.

## Identifiers

Argument names, block type names, and the names of most mnptu-specific
...
For complete identifier rules, mnptu implements
[the Unicode identifier syntax](http://unicode.org/reports/tr31/), extended to
include the ASCII hyphen character `-`.

## Comments

The mnptu language supports three different syntaxes for comments:
...
mnptu configuration files must always be UTF-8 encoded. While the
delimiters of the language are all ASCII characters, mnptu accepts
non-ASCII characters in identifiers, comments, and string values.

mnptu accepts configuration files with either Unix-style line endings
