...
Most mnptu configurations are written in
[the native mnptu language syntax](/mnptu/language/syntax/configuration), which is designed to be
relatively easy for humans to read and update.

mnptu also supports an alternative syntax that is JSON-compatible. This
...
mnptu expects native syntax for files named with a `.tf` suffix, and
...
of HCL syntax or its JSON mapping in order to use mnptu, and so this page
...
At the root of any JSON-based mnptu configuration is a JSON object. The
properties of this object correspond to the top-level block types of the
mnptu language. For example:
...
* Where a property corresponds to an argument that accepts
  [arbitrary expressions](/mnptu/language/expressions) in the native syntax, the
...
Since JSON grammar is not able to represent all of the mnptu language
[expression syntax](/mnptu/language/expressions), JSON values interpreted as expressions
are mapped as follows:

| JSON    | mnptu Language Interpretation                                                                             |
| ------- | ------------------------------------------------------------------------------------------------------------- |
...
[string template]: /mnptu/language/expressions/strings#string-templates
...
ignored by mnptu entirely. This exception does _not_ apply to objects
...
by mnptu, and so their mapping to the JSON syntax does not follow the
...
Since no settings within `mnptu` blocks accept named object references or
function calls, all setting values are taken literally. String values are not
interpreted as string templates.

Since only one `backend` block is allowed per `mnptu` block, the compact
