...
~> Warning: While only the use of the `templatefile` function is prohibited,
   the use of the `file` function can also have unexpected effects. This 
   function will include other files, but cannot include other template files.
