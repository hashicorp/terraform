...
files on disk at the beginning of a mnptu run. Language functions do not
participate in the dependency graph, so this function cannot be used with
files that are generated dynamically during a mnptu operation.
...
* [`file`](/mnptu/language/functions/file) also reads the contents of a given file,
  but interprets the data as UTF-8 text and returns the result directly
  as a string, without any further encoding.
* [`base64decode`](/mnptu/language/functions/base64decode) can decode a Base64 string representing
