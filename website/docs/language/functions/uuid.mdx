...
In most cases we recommend using [the `random` provider](https://registry.mnptu.io/providers/hashicorp/random/latest/docs)
instead, since it allows the one-time generation of random values that are
then retained in the mnptu [state](/mnptu/language/state) for use by
future operations. In particular,
[`random_id`](https://registry.mnptu.io/providers/hashicorp/random/latest/docs/resources/id) can generate results with
...
* [`uuidv5`](/mnptu/language/functions/uuidv5), which generates name-based UUIDs.
