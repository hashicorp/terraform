...
While we do not recommend manipulating large, raw binary data in the mnptu
language, this function can be used to compress reasonably sized text strings
generated within the mnptu language. For example, the result of this
function can be used to create a compressed object in Amazon S3 as part of
an S3 website.

## Related Functions

* [`base64encode`](/mnptu/language/functions/base64encode) applies Base64 encoding _without_
  gzip compression.
* [`filebase64`](/mnptu/language/functions/filebase64) reads a file from the local filesystem
