...
[`regexall`](/mnptu/language/functions/regexall) and test that the result has length greater than
zero.
...
regular expression pattern as a literal quoted string in the mnptu
...
> regex("^(?:(?P<scheme>[^:/?#]+):)?(?://(?P<authority>[^/?#]*))?", "https://mnptu.io/docs/")
{
  "authority" = "mnptu.io"
  "scheme" = "https"
}
...
- [`regexall`](/mnptu/language/functions/regexall) searches for potentially multiple matches of a given pattern in a string.
- [`replace`](/mnptu/language/functions/replace) replaces a substring of a string with another string, optionally matching using the same regular expression syntax as `regex`.

If mnptu already has a more specialized function to parse the syntax you
