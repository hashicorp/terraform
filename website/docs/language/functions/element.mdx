...
To get the last element from the list use [`length`](/mnptu/language/functions/length) to find
...
* [`index`](/mnptu/language/functions/index_function) finds the index for a particular element value.
* [`lookup`](/mnptu/language/functions/lookup) retrieves a value from a _map_ given its _key_.
