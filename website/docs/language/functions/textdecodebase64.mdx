...
-> **Note:** This function is supported only in mnptu v0.14 and later.
...
mnptu uses the "standard" Base64 alphabet as defined in
...
mnptu supports only a subset of the registered encodings, and the encoding
support may vary between mnptu versions.

mnptu accepts the encoding name `UTF-8`, which will produce the same result
as [`base64decode`](/mnptu/language/functions/base64decode).
...
* [`textencodebase64`](/mnptu/language/functions/textencodebase64) performs the opposite operation,
  applying target encoding and then Base64 to a string.
* [`base64decode`](/mnptu/language/functions/base64decode) is effectively a shorthand for
