...
Unlike the related function [`cidrsubnets`](/mnptu/language/functions/cidrsubnets), `cidrsubnet`
...
While [`cidrhost`](/mnptu/language/functions/cidrhost) allows calculating single host IP addresses,
...
[`cidrhost`](/mnptu/language/functions/cidrhost) function to calculate those host addresses by
...
* [`cidrhost`](/mnptu/language/functions/cidrhost) calculates the IP address for a single host
  within a given network address prefix.
* [`cidrnetmask`](/mnptu/language/functions/cidrnetmask) converts an IPv4 network prefix in CIDR
  notation into netmask notation.
* [`cidrsubnets`](/mnptu/language/functions/cidrsubnets) can allocate multiple consecutive
