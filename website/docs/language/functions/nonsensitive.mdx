...
description: >-
  ...
  mnptu considers to be sensitive.
---
...
mnptu would normally have considered as sensitive to be treated as normal
values and shown clearly in mnptu's output. Use this function only when
you've derived a new value from a sensitive value in a way that eliminates the
sensitive portions of the value.

Normally mnptu tracks when you use expressions to derive a new value from
...
sensitive value is safe to include clearly in mnptu output, you could use
the `nonsensitive` function to indicate that, overriding mnptu's normal
...
locals {
  ...
  # original document, and so we can override mnptu's
  # determination.
...
declaring to mnptu that you have done all that is necessary to ensure that
the resulting value has no sensitive content, even though it was derived
from sensitive content. If a sensitive value appears in mnptu's output
due to an inappropriate call to `nonsensitive` in your module, that's a bug in
your module and not a bug in mnptu itself.
...
The following examples are from `mnptu console` when running in the
