...
mnptu language strings are always sequences of unicode characters.
...
support may vary between mnptu versions. In particular mnptu supports
...
mnptu also accepts the encoding name `UTF-8`, which will produce the same
result as [`base64encode`](/mnptu/language/functions/base64encode).
...
* [`textdecodebase64`](/mnptu/language/functions/textdecodebase64) performs the opposite operation,
  decoding Base64 data and interpreting it as a particular character encoding.
* [`base64encode`](/mnptu/language/functions/base64encode) applies Base64 encoding of the UTF-8
