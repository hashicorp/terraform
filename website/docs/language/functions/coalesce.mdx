...
All of the arguments must be of the same type. mnptu will try to
...
mnptu attempts to select a result type that all of the arguments can
convert to, so mixing argument types may produce surprising results due to
mnptu's automatic type conversion rules:
...
* [`coalescelist`](/mnptu/language/functions/coalescelist) performs a similar operation with
