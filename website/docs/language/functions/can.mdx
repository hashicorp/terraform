...
[`try`](/mnptu/language/functions/try) instead, because it allows for more concise definition of
...
[custom variable validation rules](/mnptu/language/values/variables#custom-validation-rules).
For example:
...
prefer to use [`try`](/mnptu/language/functions/try).
...
* [`try`](/mnptu/language/functions/try), which tries evaluating a sequence of expressions and
