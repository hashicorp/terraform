...
You can use `terraform.applying` in scenarios where you want Terraform to do different things depending on the type of operation it is running. For example, you could use `terraform.applying` to apply different credentials if Terraform is either planning or applying.
...
If you run a plan operation Terraform uses your read-only credentials, if you run an apply operation then Terraform uses your write credentials.
