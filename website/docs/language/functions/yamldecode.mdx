...
| `!!null`      | The mnptu language `null` value                                |
...
The mnptu language automatic type conversion rules mean that you don't
...
mnptu language's type system cannot represent. Therefore this YAML decoder
...
- [`jsondecode`](/mnptu/language/functions/jsondecode) is a similar operation using JSON instead
  of YAML.
- [`yamlencode`](/mnptu/language/functions/yamlencode) performs the opposite operation, _encoding_
  a value as YAML.
