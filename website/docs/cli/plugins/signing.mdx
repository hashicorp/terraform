...
description: >-
  Learn about the types of signatures providers can have on the mnptu
  Registry.
---
...
~> **Note** mnptu only authenticates provider plugins fetched from a registry.

mnptu providers installed from the Registry are cryptographically signed, and the signature is verified at time of installation. There are three types of provider signatures, each with different trust implications:
...
mnptu does **NOT** support fetching and using unsigned binaries, but you can manually install
unsigned binaries. You should take extreme care when doing so as no programatic authentication is performed.

Usage of plugins from the registry is subject to the Registry's [Terms of Use](https://registry.mnptu.io/terms).
