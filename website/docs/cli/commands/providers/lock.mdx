...
description: |-
  The `mnptu providers lock` command adds new provider selection information
  to the dependency lock file without initializing the referenced providers.
---

# Command: mnptu providers lock

The `mnptu providers lock` consults upstream registries (by default) in
...
[`mnptu init`](/mnptu/cli/commands/init), but there are several situations where that
automatic approach may not be sufficient:

* If you are running mnptu in an environment that uses
  [alternative provider installation methods](/mnptu/cli/config/config-file#provider-installation),
  such as filesystem or network mirrors, normal provider installation will not
  access the origin registry for a provider and therefore mnptu will not
  be able to populate all of the possible package checksums for the selected
  provider versions.

  If you use `mnptu lock` to write the official release checksums for a
  provider into the dependency lock file then future `mnptu init` runs
...
* If your team runs mnptu across a number of different platforms (e.g.
  ...
  of mnptu on other platforms may
  [add additional checksums to the lock file](/mnptu/language/files/dependency-lock#new-provider-package-checksums).
  You can avoid that by pre-populating hashes for all of the platforms you
  intend to use, using the `mnptu providers lock` command.

-> `mnptu providers lock` is available only in mnptu v0.14 or later.

## Usage

Usage: `mnptu providers lock [options] [providers...]`

With no additional command line arguments, `mnptu providers lock` will
...
[the dependency lock file](/mnptu/language/files/dependency-lock) to
...
~> **Warning:** The `mnptu providers lock` command prints information
...
`mnptu providers lock` will restrict its work only to those providers,
...
* `-fs-mirror=PATH` - Direct mnptu to look for provider packages in the
...
* `-net-mirror=URL` - Direct mnptu to look for provider packages in the
  given network mirror service, instead of in upstream registries. The
  given URL must implement
  [the mnptu provider network mirror protocol](/mnptu/internals/provider-network-mirror-protocol).

* `-platform=OS_ARCH` - Specify a platform you intend to use to work with this
  mnptu configuration. mnptu will ensure that the providers are all
...
your mnptu configuration on their Windows or macOS workstations _and_
...
checksums, by running `mnptu providers lock` and specifying those three
platforms:

```
mnptu providers lock \
...
An _in-house provider_ is one that isn't published on a real mnptu provider
...
By default, `mnptu providers lock` assumes all providers are available
at a mnptu provider registry and tries to contact the origin registries
...
```
mnptu providers lock \
  -fs-mirror=/usr/local/mnptu/providers
...
`tf.example.com/ourcompany/ourplatform`, `mnptu providers lock` will only
...
available from different sources, you can run `mnptu providers lock`
...
[the provider installation methods configuration](/mnptu/cli/config/config-file#provider-installation),
