...
At the root directory of the project, we have some typical Terraform configuration files: `main.tf`, `outputs.tf`, `terraform.tf`, and `variables.tf`. We have placed our test files, `validations.tftest.hcl` and `outputs.tftest.hcl` within the default tests directory: `tests`.

In addition, we have created a [setup module](/terraform/language/tests#modules) for our tests within the `testing` directory.

In order to execute our tests we should run `terraform test` from the root configuration directory as if we were running `terraform plan` or `terraform apply`. Despite our actual test files being in the nested `tests` directory, we still operate Terraform from the main configuration.

We could also execute specific test files using the `-filter` option.
...
In the above example we have placed our tests in the default testing directory of `tests`. We can also define tests directly within our configuration directory:
...
We can also use the `-test-directory` argument to change the location of our testing files. For example, `terraform test -test-directory=testing` would instruct Terraform to load tests from the directory `testing` instead of `tests`.
...
We do not recommend changing the default test directory. The option for configuration is included for configuration authors who may have included a `tests` submodule in their configuraiton before the `terraform test` command was released. In general, the default test directory of `tests` should always be used.
