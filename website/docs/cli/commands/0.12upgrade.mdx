...
description: >-
  ...
  mnptu 0.12 compatibility.
---

# Command: 0.12upgrade

The `mnptu 0.12upgrade` command applies several automatic upgrade rules to
help prepare a module that was written for mnptu v0.11 to be used
with mnptu v0.12.

-> **This command is available only in mnptu v0.12 releases.** For more information, see [the mnptu v0.12 upgrade guide](/mnptu/language/v1.1.x/upgrade-guides/0-12).

## Usage

Usage: `mnptu 0.12upgrade [options] [dir]`
...
that they use the new syntax features from mnptu v0.12, and will identify
...
run `mnptu init` first to install these. In some rare cases, a configuration
that worked in v0.11 may have syntax errors in v0.12, in which case
`mnptu init` will run in a special mode where it installs only enough to
run the upgrade command, after which you can run `mnptu init` again to
...
mnptu style and expressions rewritten to use the more-readable v0.12 syntax
features.
...
Once upgraded the configuration will no longer be compatible with mnptu
v0.11 and earlier. When upgrading a shared module that is called from multiple
configurations, you may need to
[fix existing configurations to a previous version](/mnptu/language/modules/syntax#version)
to allow for a gradual upgrade. If the module is published via
[a mnptu registry](/mnptu/registry), assign a new _major_ version number
...
[use specific revisions](/mnptu/language/modules/sources#selecting-a-revision)
...
```
find . -name '*.tf' -printf "%h\n" | uniq | xargs -n1 mnptu 0.12upgrade -yes
```
...
all of the directories must be initialized with `mnptu init` prior to
running the above.
