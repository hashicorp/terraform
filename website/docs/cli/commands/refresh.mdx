...
description: >-
  ...
  about (via its state file) with the real-world infrastructure. This can be
  used to detect any drift from the last-known state, and to update the state
  file.
---
...
[the Terraform state](/language/state/).
...
[`terraform apply`](apply) except that it does not accept a saved
