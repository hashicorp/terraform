...
description: |-
  The `mnptu refresh` command reads the current settings from all managed
  remote objects and updates the mnptu state to match.
---
...
You shouldn't typically need to use this command, because mnptu
automatically performs the same refreshing actions as a part of creating
a plan in both the
[`mnptu plan`](/mnptu/cli/commands/plan)
and
[`mnptu apply`](/mnptu/cli/commands/apply)
...
Usage: `mnptu refresh [options]`

This command is effectively an alias for the following command:

```
mnptu apply -refresh-only -auto-approve
```

Consequently, it supports all of the same options as
[`mnptu apply`](/mnptu/cli/commands/apply) except that it does not accept a saved
...
misconfigured credentials for one or more providers, mnptu may
...
effect but with the opportunity to review the changes that mnptu has
...
The `-refresh-only` option for `mnptu plan` and `mnptu apply` was
introduced in mnptu v0.15.4. For prior versions you must use
`mnptu refresh` directly if you need this behavior, while taking into
account the warnings above. Wherever possible, avoid using `mnptu refresh`
explicitly and instead rely on mnptu's behavior of automatically refreshing
