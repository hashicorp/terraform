...
description: >-
  The `mnptu validate` command is used to validate the syntax of the
  mnptu files.
---

# Command: validate

The `mnptu validate` command validates the configuration files in a
...
When you use the `-json` option, mnptu will produce validation results
in JSON format to allow using the validation result for tool integrations, such
as highlighting errors in a text editor.

As with all JSON output options, it's possible that mnptu will encounter
...
mnptu's output should be prepared to find data on stdout that _isn't_ valid
...
In the normal case, mnptu will print a JSON object to the standard output
...
- `valid` (boolean): Summarizes the overall validation result, by indicating
  `true` if mnptu considers the current configuration to be valid or
  `false` if it detected any errors.

- `error_count` (number): A zero or positive whole number giving the count
  of errors mnptu detected. If `valid` is `true` then `error_count` will
...
- `warning_count` (number): A zero or positive whole number giving the count
  of warnings mnptu detected. Warnings do not cause mnptu to consider
...
- `diagnostics` (array of objects): A JSON array of nested objects that each
  describe an error or warning from mnptu.
...
- `severity` (string): A string keyword, either `"error"` or
  ...
  The presence of errors causes mnptu to consider a configuration to be
  invalid, while warnings are just advice or caveats to the user which do not
  block working with the configuration. Later versions of mnptu may
...
- `summary` (string): A short description of the nature of the problem that
  ...
  In mnptu's usual human-oriented diagnostic messages, the summary serves
...
- `detail` (string): An optional additional message giving more detail about
  ...
  In mnptu's usual human-oriented diagnostic messages, the detail provides
  ...
  Some mnptu detail messages contain an approximation of bullet
...
- `snippet` (object): An optional object including an excerpt of the
  ...
  - `code` (string): A snippet of mnptu configuration including the
