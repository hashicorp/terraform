...
description: >-
  ...
  tainted, forcing it to be destroyed and recreated on the next apply.
---
...
to use the `-replace` option with [`terraform apply`](apply).
For example:
...
[the `remote` backend](/language/settings/backends/enhanced/remote)
only, `terraform taint`
also accepts the option
[`-ignore-remote-version`](/language/settings/backends/enhanced/remote#command-line-arguments).
