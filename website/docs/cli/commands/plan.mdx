...
description: >-
  ...
  configuration files. The plan can be saved using `-out`, and then provided to
...
[`terraform apply`](apply) directly. By default, the "apply" command
...
[`terraform apply`](apply) as an extra argument. This two-step workflow
...
earlier versions you must run [`terraform destroy`](destroy).
...
see
[Assigning Values to Root Module Variables](/language/values/variablesl#assigning-values-to-root-module-variables).
...
subset of resources.
You can use [resource address syntax](/cli/state/resource-addressingl)
