...
description: >-
  The mnptu graph command generates a visual representation of a
  configuration or execution plan that you can use to generate charts.
---

# Command: graph

The `mnptu graph` command is used to generate a visual
...
Usage: `mnptu graph [options]`

Outputs the visual execution graph of mnptu resources according to
...
The `-type` flag can be used to control the type of graph shown. mnptu
...
The output of `mnptu graph` is in the DOT format, which can
easily be converted to an image by making use of `dot` provided
by GraphViz:

```shellsession
$ mnptu graph | dot -Tsvg > graph.svg
```
