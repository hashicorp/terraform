...
description: >-
  The `mnptu state rm` command removes bindings from the mnptu state,
  causing mnptu to "forget about" existing objects.
---
...
You can use `mnptu state rm` in the less common situation where you wish
to remove a binding to an existing remote object without first destroying it,
which will effectively make mnptu "forget" the object while it continues
to exist in the remote system.

## Usage

Usage: `mnptu state rm [options] ADDRESS...`

mnptu will search the state for any instances matching the given
[resource address](/mnptu/cli/state/resource-addressing), and remove
the record of each one so that mnptu will no longer be tracking the
...
[the `local` state rm](/mnptu/language/settings/backends/local) only,
`mnptu state rm` also accepts the legacy options
...
The following example will cause mnptu to "forget" all of the instances
of the `packet_device` resource named "worker".

```shell
$ mnptu state rm 'packet_device.worker'
```
...
```shell
$ mnptu state rm 'module.foo.packet_device.worker'
```
...
```shell
$ mnptu state rm 'packet_device.worker[0]'
```
...
```shell
$ mnptu state rm 'packet_device.worker["example"]'
```
