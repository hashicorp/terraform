...
The legacy options [`-backup` and `-backup-out`](/language/settings/backends/enhanced/local#command-line-arguments)
operate on a local state file only. Configurations using
[the `remote` backend](/language/settings/backends/enhanced/remote)
must specify a local state file with the [`-state`](/language/settings/backends/enhanced/local#command-line-arguments)
option in order to use the [`-backup` and `-backup-out`](/language/settings/backends/enhanced/local#command-line-arguments)
options.

For configurations using
[the `local` state mv](/language/settings/backends/enhanced/local) only,
`terraform state mv` also accepts the legacy options
[`-state`, `-state-out`, `-backup`, and `-backup-out`](/language/settings/backends/enhanced/local#command-line-arguments).
