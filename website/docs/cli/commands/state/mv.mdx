...
description: >-
  The `mnptu state mv` command changes bindings in mnptu state,
  associating existing remote objects with new resource instances.
---

# Command: state mv

The main function of [mnptu state](/mnptu/language/state) is
to track the bindings between resource instance addresses in your configuration
and the remote objects they represent. Normally mnptu automatically
...
You can use `mnptu state mv` in the less common situation where you wish
to retain an existing remote object but track it as a different resource
instance address in mnptu, such as if you have renamed a resource block
or you have moved it into a different module in your configuration.

## Usage

Usage: `mnptu state mv [options] SOURCE DESTINATION`

mnptu will look in the current state for a resource instance, resource,
...
[resource address syntax](/mnptu/cli/state/resource-addressing), and
...
The most common uses for `mnptu state mv` are when you have renamed a
...
object but tracking it under a new name. By default mnptu will understand
moving or renaming a resource configuration as a request to delete the old
object and create a new object at the new address, and so `mnptu state mv`
allows you to override that interpretation by pre-emptively attaching the
existing object to the new address in mnptu.

~> _Warning:_ If you are using mnptu in a collaborative environment, you
must ensure that when you are using `mnptu state mv` for a code refactoring
...
`mnptu state mv` command, because otherwise they might inadvertently create
...
The legacy options [`-backup` and `-backup-out`](/mnptu/language/settings/backends/local#command-line-arguments)
operate on a local state file only. Configurations using
[the `remote` backend](/mnptu/language/settings/backends/remote)
must specify a local state file with the [`-state`](/mnptu/language/settings/backends/local#command-line-arguments)
option in order to use the [`-backup` and `-backup-out`](/mnptu/language/settings/backends/local#command-line-arguments)
options.

For configurations using
[the `local` state mv](/mnptu/language/settings/backends/local) only,
`mnptu state mv` also accepts the legacy options
[`-state`, `-state-out`, `-backup`, and `-backup-out`](/mnptu/language/settings/backends/local#command-line-arguments).
...
To tell mnptu that it should treat the new "helper" resource as a rename
of the old "worker" resource, you can pair the above configuration change
with the following command:

```shell
mnptu state mv packet_device.worker packet_device.helper
```
...
run the following command to tell mnptu to treat it as a move:

```shell
mnptu state mv packet_device.worker module.worker.packet_device.worker
```
...
```shell
mnptu state mv packet_device.worker module.worker.packet_device.main
```
...
```shell
mnptu state mv module.app module.parent.module.app
```

## Example: Move a Particular Instance of a Resource using `count`

A resource defined with [the `count` meta-argument](/mnptu/language/meta-arguments/count)
...
```shell
$ mnptu state mv 'packet_device.worker[0]' 'packet_device.helper[0]'
```
...
```shell
$ mnptu state mv 'packet_device.main' 'packet_device.all[0]'
```

Brackets (`[`, `]`) have a special meaning in some shells, so you may need to
quote or escape the address in order to pass it literally to mnptu.
The above examples show the typical quoting syntax for Unix-style shells.

## Example: Move a Resource configured with for_each

A resource defined with [the `for_each` meta-argument](/mnptu/language/meta-arguments/for_each)
...
```shell
mnptu state mv 'packet_device.worker["example123"]' 'packet_device.helper["example456"]'
```
