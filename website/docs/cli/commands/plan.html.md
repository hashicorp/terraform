...
The planning modes are all mutually-exclusive, so activating either of the
default planning modes disables the "normal" planning mode, and you can't
use more than one of the alternative mode options at the same time.

-> **Note:** Refresh-only mode is only available from Terraform v1.0 onwards.
For prior versions, see [`terraform refresh`](./refresh.html) instead.
...
* `-replace=ADDRESS` - Instructs Terraform to plan to replace the single
  ...
  Terraform will choose a "replace" action instead.
    ...
    is incompatible with the `-destroy` and `-refresh-only` options.
...
* `-var 'NAME=VALUE` - Sets a value for a single
  ...
  root module of the configuration.
