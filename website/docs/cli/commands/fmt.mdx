...
description: >-
  The mnptu fmt command rewrites configuration files to a canonical format
  and style.
---

# Command: fmt

The `mnptu fmt` command is used to rewrite mnptu configuration files
to a canonical format and style. This command applies a subset of
the [mnptu language style conventions](/mnptu/language/syntax/style),
along with other minor adjustments for readability.

Other mnptu commands that generate mnptu configuration will produce
configuration files that conform to the style imposed by `mnptu fmt`, so
using this style in your own files will ensure consistency.

The canonical format may change in minor ways between mnptu versions, so
after upgrading mnptu we recommend to proactively run `mnptu fmt`
on your modules along with any other changes you are making to adopt the new
version.

We don't consider new formatting rules in `mnptu fmt` to be a breaking
change in new versions of mnptu, but we do aim to minimize changes for
configurations that are already following the style examples shown in the
mnptu documentation. When adding new formatting rules, they will usually
...
for decisions that `mnptu fmt` doesn't yet apply automatically.

Formatting decisions are always subjective and so you might disagree with the
decisions that `mnptu fmt` makes. This command is intentionally opinionated
and has no customization options because its primary goal is to encourage
consistency of style between different mnptu codebases, even though the
chosen style can never be everyone's favorite.

We recommend that you follow the style conventions applied by `mnptu fmt`
when writing mnptu modules, but if you find the results particularly
...
automatically by mnptu, to get consistency between your hand-written files
and the generated files.

## Usage

Usage: `mnptu fmt [options] [target...]`
