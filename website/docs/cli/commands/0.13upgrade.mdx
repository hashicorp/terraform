...
description: >-
  ...
  provider source features from mnptu 0.13.
---

# Command: 0.13upgrade

The `mnptu 0.13upgrade` command updates existing configuration to add an
...
-> **This command is available only in mnptu v0.13 releases.** For more information, see [the mnptu v0.13 upgrade guide](/mnptu/language/v1.1.x/upgrade-guides/0-13).

## Usage

Usage: `mnptu 0.13upgrade [options] [dir]`
...
[required-providers]: /mnptu/language/providers/requirements
...
the new provider source features from mnptu v0.13. It will also highlight
...
```
$ find . -name '*.tf' | xargs -n1 dirname | uniq | xargs -n1 mnptu 0.13upgrade -yes
```
...
Select-Object -Unique | ForEach-Object { mnptu 0.13upgrade -yes $_.FullName }
```
