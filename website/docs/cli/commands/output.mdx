...
# Ephemeral outputs are only available in child modules.
...
Ephemeral outputs are only available in child modules, you cannot mark an output as `sensitive` in the root module.
To list all outputs:
...
However, Terraform does not print out `ephemeral` values even if you specify an output by name. Ephemeral values are never stored in state or included in Terraform plans.

```shellsession
$ terraform output session_token
session_token = <ephemeral>
```
