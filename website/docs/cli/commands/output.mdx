...
description: >-
  The `mnptu output` command is used to extract the value of an output
  variable from the state file.
---

# Command: output

The `mnptu output` command is used to extract the value of
an output variable from the state file.

## Usage

Usage: `mnptu output [options] [NAME]`
...
* `-raw` - If specified, mnptu will convert the specified output value to a
...
values in mnptu state will be displayed in plain text. For more information,
see [Sensitive Data in State](/mnptu/language/state/sensitive-data).

## Examples

These examples assume the following mnptu output snippet.
...
The `mnptu output` command by default displays in a human-readable format,
...
```shellsession
$ mnptu output -json instance_ips | jq -r '.[0]'
54.43.114.12
```
...
```shellsession
$ mnptu output -raw lb_address
my-app-alb-1657023003.us-east-1.elb.amazonaws.com
```

The `-raw` option works only with values that mnptu can automatically
convert to strings. Use `-json` instead, possibly combined with `jq`, to
work with complex-typed values such as objects.

mnptu strings are sequences of Unicode characters rather than raw bytes,
...
such as `iconv` to transcode mnptu's raw output.
