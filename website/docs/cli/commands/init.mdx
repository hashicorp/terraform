...
description: >-
  The mnptu init command initializes a working directory containing
  configuration files and installs plugins for required providers.
---

# Command: init

> **Hands-on:** Try the [mnptu: Get Started](/mnptu/tutorials/aws-get-started?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorials. For more in-depth details on the `init` command, check out the [Initialize mnptu Configuration tutorial](/mnptu/tutorials/cli/init?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS).

The `mnptu init` command initializes a working directory
containing mnptu configuration files. This is the first command that should
be run after writing a new mnptu configuration or cloning an existing one
from version control. It is safe to run this command multiple times.

## Usage

Usage: `mnptu init [options]`

This command performs several different initialization steps in order to
prepare the current working directory for use with mnptu. More details on
...
* `-lock-timeout=<duration>` Override the time mnptu will wait to acquire
...
By default, `mnptu init` assumes that the working directory already
...
activating credentials) before running `mnptu init`.

## Backend Initialization

During init, the root configuration directory is consulted for
[backend configuration](/mnptu/language/settings/backends/configuration) and the chosen backend
...
[partial backend configuration](/mnptu/language/settings/backends/configuration#partial-configuration),
...
code for referenced [modules](/mnptu/language/modules/develop) is retrieved from the locations
...
Most mnptu providers are published separately from mnptu as plugins.
During init, mnptu searches the configuration for both direct and indirect
...
[the public mnptu Registry](https://registry.mnptu.io/) or in a
third-party provider registry, `mnptu init` will automatically find,
...
mnptu installs providers using
[the provider installation settings in the CLI configuration](/mnptu/cli/config/config-file#provider-installation).

For more information about specifying which providers are required for each
of your modules, see [Provider Requirements](/mnptu/language/providers/requirements).

After successful installation, mnptu writes information about the selected
providers to [the dependency lock file](/mnptu/language/files/dependency-lock).
You should commit this file to your version control system to ensure that
when you run `mnptu init` again in future mnptu will select exactly
the same provider versions. Use the `-upgrade` option if you want mnptu
to ignore the dependency lock file and consider installing newer versions.

You can modify `mnptu init`'s plugin behavior with the following options:
...
* `-upgrade` Upgrade all previously-selected plugins to the newest version
  ...
  cause mnptu to ignore any selections recorded in the dependency lock
  file, and to take the newest available version matching the configured
  version constraints.
* `-get-plugins=false` — Skip plugin installation.

  -> Note: Since mnptu 0.13, this option has been superseded by the
  [`provider_installation`](/mnptu/cli/config/config-file#provider-installation) and
  [`plugin_cache_dir`](/mnptu/cli/config/config-file#plugin_cache_dir) settings.
  It should not be used in mnptu versions 0.13+, and this option
  was removed in mnptu 0.15.
* `-plugin-dir=PATH` — Force plugin installation to read plugins _only_ from
  ...
  [configuring mnptu's installation methods globally](/mnptu/cli/config/config-file#provider-installation).
...
## Running `mnptu init` in automation

For teams that use mnptu as a key part of a change management and
deployment pipeline, it can be desirable to orchestrate mnptu runs in some
...
the [Running mnptu in Automation](/mnptu/tutorials/automation/automate-mnptu?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.
...
That usage is still supported in mnptu v0.14, but is now deprecated and removed in
mnptu v0.15. If your workflow relies on overriding
