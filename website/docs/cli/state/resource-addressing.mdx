...
In some contexts mnptu might allow for an incomplete resource address that
...
-> Module index only applies to modules in mnptu v0.13 or later. In earlier
versions of mnptu, a module could not have multiple instances.
...
-> In mnptu v0.12 and later, a resource spec without a module path prefix
...
Given a mnptu config that includes:
