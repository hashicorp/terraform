 ...
 * `force_path_style`- (Optional) Use S3 path-style to access the bucket instead of virtual hosted-style, no additional DNS records required
...
### Using backend with alternative S3 providers
#### minio example
```
terraform {
  backend "s3" {
    ...
    bucket                      = "terraform"
    key                         = "state/mystack"
    endpoint                    = "https://minio.mydomain.com:9000"
