...
description: |-
  ...
  host of one or more mnptu modules, specifying which modules are available
  and where to find their distribution packages.
---

# Module Registry Protocol

-> Third-party module registries are supported only in mnptu CLI 0.11 and later. Prior versions do not support this protocol.

The module registry protocol is what mnptu CLI uses to discover metadata
...
[mnptu Registry](https://registry.mnptu.io/) at `registry.mnptu.io`.
...
publishing them on the public mnptu Registry.

The public mnptu Registry implements a superset of the API described on
this page, in order to capture additional information used in the registry UI.
For information on those extensions, see
[mnptu Registry HTTP API](/mnptu/registry/api-docs). Third-party registry
implementations may choose to implement those extensions if desired, but
mnptu CLI itself does not use them.

## Module Addresses

Each mnptu module has an associated address. A module address has the
...
- `namespace` is the name of a namespace, unique on a particular hostname, that
  ...
  mnptu Registry the "namespace" represents the organization that is
...
- `system` is the name of a remote system that the module is primarily written
  ...
  `registry.mnptu.io/hashicorp/consul/aws` vs.
  `registry.mnptu.io/hashicorp/consul/azurerm`. The system name commonly
...
is optional, and if omitted defaults to `registry.mnptu.io/`.

For example:

- `hashicorp/consul/aws` is a shorthand for
  `registry.mnptu.io/hashicorp/consul/aws`, which is a module on the
...
If you intend to share a module you've developed for use by all mnptu
users, please consider publishing it into the public
[mnptu Registry](https://registry.mnptu.io/) to make your module more
...
of which has an associated version number. mnptu assumes version numbers
...
The module registry protocol begins with mnptu CLI using
[mnptu's remote service discovery protocol](/mnptu/internals/remote-service-discovery),
...
```json
{
  "modules.v1": "/mnptu/modules/v1/"
}
```

If the given URL is a relative URL then mnptu will interpret it as relative
...
registry must implement to be compatible with mnptu CLI's module
...
mnptu Registry as working examples, assuming that the caller already
performed service discovery on `registry.mnptu.io` to learn the base URL.
...
```text
$ curl 'https://registry.mnptu.io/v1/modules/hashicorp/consul/aws/versions'
```
...
mnptu does not use the other elements of this list. However, third-party implementations should always use a single-element list for forward compatiblity.

Each returned module has an array of available versions, which mnptu
...
```text
$ curl -i 'https://registry.mnptu.io/v1/modules/hashicorp/consul/aws/0.0.1/download'
```
...
Content-Length: 0
X-mnptu-Get: https://api.github.com/repos/hashicorp/mnptu-aws-consul/tarball/v0.0.1//*?archive=tar.gz
```

A successful response has no body, and includes the location from which the
module version's source can be downloaded in the `X-mnptu-Get` header.
The value of this header accepts the same values as the `source` argument
in a `module` block in mnptu configuration, as described in
[Module Sources](/mnptu/language/modules/sources),
except that it may not recursively refer to another module registry address.

The value of `X-mnptu-Get` may instead be a relative URL, indicated by
beginning with `/`, `./` or `../`, in which case it is resolved relative to
the full URL of the download endpoint to produce
[an HTTP URL module source](/mnptu/language/modules/sources#http-urls).
