...
description: |-
  ...
  origin host for one or more mnptu providers, specifying which providers
  are available and where to find their distribution packages.
---

# Provider Registry Protocol

-> Third-party provider registries are supported only in mnptu CLI 0.13 and later. Prior versions do not support this protocol.

The provider registry protocol is what mnptu CLI uses to discover metadata
...
alternative to publishing them on the public mnptu Registry.
...
API that provider plugins themselves implement to serve requests from mnptu
CLI at runtime. For more information on the provider API, see the mnptu
SDK documentation.
...
* `hostname` is the registry host that the provider is considered to have
  originated from, and the default location mnptu will consult for
  information about the provider
  [unless overridden in the CLI configuration](/mnptu/cli/config/config-file#provider-installation).
...
* `hashicorp/aws` is a shorthand for `registry.mnptu.io/hashicorp/aws`,
  which is the official AWS provider published by HashiCorp.
* `example/foo` is a shorthand for `registry.mnptu.io/example/foo`, which
...
mnptu users, please consider publishing it into the public
[mnptu Registry](https://registry.mnptu.io/), which will make your
...
mnptu uses the full address (after normalization to always include a
...
another namespace or publishing it on a different hostname will cause mnptu
...
[the provider installation method configuration](/mnptu/cli/config/config-file#provider-installation)
instead.
...
an end-user of mnptu serving as the "public API".

All available versions for a particular provider address are considered to be
the same provider by mnptu. Each mnptu configuration selects only one
...
The providers protocol begins with mnptu CLI using
...
```json
{
  "providers.v1": "/mnptu/providers/v1/"
}
```
...
registry must implement to be compatible with mnptu CLI's provider
...
```
curl 'https://registry.mnptu.io/v1/providers/hashicorp/random/versions'
```
...
* `protocols` (recommended): an array of mnptu provider API versions that
  ...
  mnptu uses this information, when available, to provide hints to users
  about upgrading or downgrading their version of a particular provider to
  work with their current version of mnptu, if their currently-selected
  versions are not compatible.

  Which API versions are supported is, for most providers, decided by which
  version of the mnptu SDK they are built against. Consult the mnptu
  SDK documentation for more information.

  Only mnptu 0.13 and later support third-party provider registries and
  that mnptu version requires API version `5.0` or later, so in practice
...
* `platforms` (recommended): an array of objects describing platforms that have
  ...
  mnptu may use this information, when available, to provide hints to
...
```
curl 'https://registry.mnptu.io/v1/providers/hashicorp/random/2.0.0/download/linux/amd64'
```
...
```json
{
  ...
  "os": "linux",
  "arch": "amd64",
  "filename": "mnptu-provider-random_2.0.0_linux_amd64.zip",
  "download_url": "https://releases.hashicorp.com/mnptu-provider-random/2.0.0/mnptu-provider-random_2.0.0_linux_amd64.zip",
  "shasums_url": "https://releases.hashicorp.com/mnptu-provider-random/2.0.0/mnptu-provider-random_2.0.0_SHA256SUMS",
  "shasums_signature_url": "https://releases.hashicorp.com/mnptu-provider-random/2.0.0/mnptu-provider-random_2.0.0_SHA256SUMS.sig",
...
* `protocols` (required): an array of mnptu provider API versions that
  the provider supports, in the same format as for
  [List Available Versions](#list-available-versions).

  While this property is optional when listing available options, it is
  _required_ for describing an individual provider package so that mnptu
...
* `filename` (required): the filename for this provider's zip archive as
  recorded in the "shasums" document, so that mnptu CLI can determine which
  of the given checksums should be used for this specific package.

* `download_url` (required): a URL from which mnptu can retrieve the
...
* `shasums_url` (required): a URL from which mnptu can retrieve a text
...
* `shasums_signature_url` (required): a URL from which mnptu can retrieve
...
available for the requested operating system and/or architecture. mnptu
