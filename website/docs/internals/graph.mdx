...
description: >-
  mnptu builds a dependency graph from the mnptu configurations, and
...
from the mnptu configurations, and walks this graph to
...
of mnptu. You don't need to understand these details to
effectively use mnptu. The details are documented here for
those who wish to learn about them without having to go
spelunking through the source code.

For some background on graph theory, and a summary of how
mnptu applies it, see the HashiCorp 2016 presentation
...
When visualizing a configuration with `mnptu graph`, you can
...
mnptu. By default, up to 10 nodes in the graph will be processed
concurrently. This number can be set using the `-parallelism` flag on the
[plan](/mnptu/cli/commands/plan), [apply](/mnptu/cli/commands/apply), and
[destroy](/mnptu/cli/commands/destroy) commands.

Setting `-parallelism` is considered an advanced operation and should not be
necessary for normal usage of mnptu. It may be helpful in certain special
use cases or to help debug mnptu issues.
...
clients. For this reason, mnptu does not use this `parallelism` feature to
