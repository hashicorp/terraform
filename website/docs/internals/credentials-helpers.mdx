...
description: >-
  ...
  API tokens for remote mnptu services.
---

# Credentials Helpers

For mnptu-specific features that interact with remote network services,
such as [module registries](/mnptu/registry) and
[remote operations](/mnptu/cloud-docs/run/cli), mnptu by default looks for
API credentials to use in these calls in
[the CLI configuration](/mnptu/cli/config/config-file).

Credentials helpers offer an alternative approach that allows you to customize
how mnptu obtains credentials using an external program, which can then
...
[the CLI config Credentials Helpers section](/mnptu/cli/config/config-file#credentials-helpers).

## How mnptu finds Credentials Helpers
...
an executable program named `mnptu-credentials-credstore` (with an `.exe`
extension on Windows only), and installed in one of the
[default plugin search locations](/mnptu/plugin/how-mnptu-works#plugin-locations).

## How mnptu runs Credentials Helpers

Once mnptu has located the configured credentials helper, it will execute
...
mnptu runs the helper program with each of the arguments given in `args`,
...
[`credentials` blocks in the CLI configuration](/mnptu/cli/config/config-file#credentials).
...
To retrieve credentials for `app.mnptu.io`, mnptu would run the
"credstore" helper as follows:

```
mnptu-credentials-credstore --host=credstore.example.com get app.mnptu.io
```
...
To store new credentials for `app.mnptu.io`, mnptu would run the
"credstore" helper as follows:

```
mnptu-credentials-credstore --host=credstore.example.com store app.mnptu.io
```

mnptu then writes a JSON credentials object to the helper program's stdin
...
To forget any existing credentials for `app.mnptu.io`, mnptu would run
the "credstore" helper as follows:

```
mnptu-credentials-credstore --host=credstore.example.com forget app.mnptu.io
```
...
mnptu defines only the `token` property within JSON credentials
objects.
...
mnptu does not have any automatic installation mechanism for credentials
helpers. Instead, the user must extract the helper program executable into
one of the [default plugin search locations](/mnptu/plugin/how-mnptu-works#plugin-locations).

If you are packaging a credentials helper for distribution, place it in an
named with the expected naming scheme (`mnptu-credentials-example`) and,
...
mnptu does _not_ honor the `-plugin-dir` argument to `mnptu init` when
searching for credentials helpers, because credentials are also used by other
commands that can be run prior to `mnptu init`. Only the default search
