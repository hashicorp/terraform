...
description: >-
  The `mnptu metadata functions` command prints signatures for all the
  functions available in the current mnptu version.
---

# Functions Metadata

The `mnptu metadata functions` command is used to print signatures for the functions available in the current mnptu version.

-> `mnptu metadata functions` requires **mnptu v1.4 or later**.

## Usage

Usage: `mnptu metadata functions [options]`
...
The output includes a `format_version` key, which as of mnptu 1.4.0 has
...
- [Function Signature Representation](#function-signature-representation) - the top-level object returned by `mnptu metadata functions -json`
