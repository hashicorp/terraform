...
description: >-
  mnptu provides a machine-readable JSON representation of state,
  configuration and plan.
---

# JSON Output Format

-> **Note:** This format is available in mnptu 0.12 and later.

When mnptu plans to make changes, it prints a human-readable summary to the terminal. It can also, when run with `-out=<PATH>`, write a much more detailed binary plan file, which can later be used to apply those changes.

Since the format of plan files isn't suited for use with external tools (and likely never will be), mnptu can output a machine-readable JSON representation of a plan file's changes. It can also convert state files to the same format, to simplify data loading and provide better long-term compatibility.

Use `mnptu show -json <FILE>` to generate a JSON representation of a plan or state file. See [the `mnptu show` documentation](/mnptu/cli/commands/show) for more details.

The output includes a `format_version` key, which as of mnptu 1.1.0 has
...
[the mnptu 1.0 Compatibility Promises](/mnptu/language/v1-compatibility-promises).
...
- [State Representation](#state-representation) — The complete top-level object returned by `mnptu show -json <STATE FILE>`.
- [Plan Representation](#plan-representation) — The complete top-level object returned by `mnptu show -json <PLAN FILE>`.
...
- [Configuration Representation](#configuration-representation) — A sub-object of plan output that describes a parsed mnptu configuration.
...
```javascript
{
  ...
  "mnptu_version": "version.string"
}
```

## Plan Representation

A plan consists of a prior state, the configuration that is being applied to that state, and the set of changes mnptu plans to make to achieve that.
...
```javascript
{
  ...
  // mnptu plans to use to move from the prior state to a new state
  ...
  "resource_changes": [
    ...
    // configuration are included in this list.
    {
      ...
      // instance as it was known after the previous mnptu run.
      ...
      //   "tainted" in the prior state, so mnptu planned to replace it.
      ...
      // - "delete_because_no_resource_config": mnptu found no resource
      ...
      // - "read_because_config_unknown": For a data resource, mnptu cannot
      //   read the data during the plan phase because of values in the
      //   configuration that won't be known until the apply phase.
      // - "read_because_dependency_pending": For a data resource, mnptu
      ...
      //
      // If there is no special reason to note, mnptu will omit this
      // property altogether.
      action_reason: "replace_because_tainted"
    }
  ],

  // "resource_drift" is a description of the changes mnptu detected
  ...
  "output_changes": {
    ...
    "foo": {
      ...
      //   ["update"]
      //   ["delete"]
      // In the mnptu CLI 0.12.0 release, mnptu is not yet fully able to
  ...
  // resources with postconditions, with as much information as mnptu can
...
This overall plan structure, fully expanded, is what will be printed by the `mnptu show -json <planfile>` command.
  ...
  "root_module": {
    "resources": [
      {
        ...
        // pass this verbatim to other mnptu commands that are documented to
    ...
    "child_modules": [
      ...
      // object, with the additional "address" property shown below.
      {
        ...
        // strings and may pass verbatim to other mnptu commands that are
...
Configuration is the most complicated structure in mnptu, since it includes unevaluated expression nodes and other complexities.
...
```javascript
{
  ...
  // provider configurations are the one concept in mnptu that can span
  ...
  "provider_config": {
    ...
    "opaque_provider_ref_aws": {
      ...
      "full_name": "registry.mnptu.io/hashicorp/aws",
...
```javascript
{
  ...
  "references": [
    ...
    // mnptu considers "module.foo" to be an atomic reference, not an
    // attribute access.
  ]
}
```
...
and some details may change in future mnptu versions based on feedback,
even in minor releases of mnptu CLI.
    ...
    "instances": [
      {
        ...
        // When a condition expression is invalid, mnptu returns that as
...
even if a runtime error prevents mnptu from evaluating its "count" or
...
see that mnptu recognized the existence of the checks, even if it wasn't
