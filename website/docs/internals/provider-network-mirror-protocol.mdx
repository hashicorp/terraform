...
description: |-
  ...
  to provide a mirror or read-through caching proxy for mnptu providers,
...
-> Provider network mirrors are supported only in mnptu CLI v0.13.2 and later. Prior versions do not support this protocol.

The provider network mirror protocol is an optional protocol which you can
implement to provide an alternative installation source for mnptu providers,
regardless of their origin registries.

mnptu uses network mirrors only if you activate them explicitly in
[the CLI configuration's `provider_installation` block](/mnptu/cli/config/config-file#provider-installation).
When enabled, a network mirror can serve providers belonging to any registry
hostname, which can allow an organization to serve all of the mnptu
...
serve as an origin registry for mnptu Providers. To provide an origin
registry (whose hostname would then be included in the source addresses of the
providers it hosts), implement
[the provider registry protocol](/mnptu/internals/provider-registry-protocol)
instead.

## Provider Addresses

Each mnptu provider has an associated address which uniquely identifies it
within mnptu. A provider address has the syntax `hostname/namespace/type`,
which is described in more detail in
[the Provider Requirements documentation](/mnptu/language/providers/requirements).
...
from. However, when you configure mnptu to install providers from a network
...
providers from the public mnptu Registry at `registry.mnptu.io`, from a
single server.
...
Most mnptu-native services use
[the remote service discovery protocol](/mnptu/internals/remote-service-discovery) so
...
dependency in a mnptu configuration.
...
provider_installation {
  network_mirror {
    url = "https://mnptu.example.com/providers/"
  }
}
```

mnptu uses the base URL only as a stem to resolve the operation endpoint
...
network mirror server must implement to be compatible with mnptu CLI's
...
[credentials](/mnptu/cli/config/config-file#credentials) for the hostname
given in the network mirror base URL, mnptu will include those credentials
...
such as `mnptu.example.com:8443`.

mnptu does _not_ send credentials when retrieving the archives whose
...
```
curl 'https://mnptu.example.com/providers/registry.mnptu.io/hashicorp/random/index.json'
```
...
use only a subset of the target platforms that mnptu supports.

mnptu CLI uses this operation after it has selected the newest available
...
```
curl 'https://mnptu.example.com/providers/registry.mnptu.io/hashicorp/random/2.0.0.json'
```
...
```json
{
  "archives": {
    "darwin_amd64": {
      "url": "mnptu-provider-random_2.0.0_darwin_amd64.zip",
      "hashes": [
        "h1:4A07+ZFc2wgJwo8YNlQpr1rVlgUDlxXHhPJciaPY5gs="
      ]
    },
    "linux_amd64": {
      "url": "mnptu-provider-random_2.0.0_linux_amd64.zip",
      "hashes": [
...
* `url` (required): a string specifying the URL from which mnptu should
  download the `.zip` archive containing the requested provider plugin version.

  mnptu resolves the URL relative to the URL from which the current
  JSON document was returned, so the examples above containing only a
  filename would cause mnptu to construct a URL like:

  ```
  https://mnptu.example.com/providers/registry.mnptu.io/hashicorp/random/mnptu-provider-random_2.0.0_darwin_amd64.zip
  ```

* `hashes` (optional): a JSON array of strings containing one or more hash
  values for the indicated archive. These hashes use mnptu's provider
  package hashing algorithm. At present, the easiest way to populate these
  is to construct a mirror's JSON indices using the `mnptu providers mirror`
  command, as described in a later section, which will include the calculated
  hashes of each provider.

  If the response includes at least one hash, mnptu will select the hash
  ...
  property then mnptu will install the indicated archive with no
  verification.

mnptu CLI will only attempt to download versions that it has previously
...
As a convenience, mnptu CLI includes
[the `mnptu providers mirror` subcommand](/mnptu/cli/commands/providers/mirror),
...
The `mnptu providers mirror` subcommand also generates `index.json` and
...
mnptu configurations, run `mnptu providers mirror` in each configuration
in turn while providing the same output directory each time. mnptu will
