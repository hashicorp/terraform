...
After each test file has completed execution, `terraform test` may emit a series of `test_cleanup` messages detailing any state that could not be destroyed. The resources listed in these messages will need to be located and destroyed manually.

As the test framework can manage multiple state files for a given test file you may see multiple versions of this message, one for each state file that holds resources that could not be destroyed. Using the `@testrun` field you can pinpoint the run block that last updated the state file detailed by this message to help with locating the resources that could not be destroyed.
...
When `terraform test` is interrupted or cancelled it will emit a `test_interrupt` message that describes the operation that was being completed when it was interrupted. This will include any state that could not be destroyed before exiting and any resources that were in the process of being created that may also have been left over.

As with the [Test Cleanup](#test-cleanup) message, there is the potential for Terraform to have been maintaining multiple state files. The resources from the main state for the configuration under test is contained in the `state` field, while the `states` field contains a map from `run` block names to resources created by that `run` block that could not be destroyed.

Finally, the `planned` field contains any resources that were in the process of being created by the `run` block that was interrupted, which is identified by the `@testrun` field.
