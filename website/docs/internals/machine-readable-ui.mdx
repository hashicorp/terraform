...
In `-verbose` mode, `terraform test` emits a `test_plan` message for all `run` blocks that executed a plan operation. The `test_plan` message content matches the format of the `terraform show -json <planfile>` command [output](/terraform/internals/json-format#plan-representation).
...
In `-verbose` mode, `terraform test` emits a `test_state` message for all `run` blocks that executed an apply operation. The `test_state` message content matches the format of the `terraform show -json` command [output](/terraform/internals/json-format#state-representation).
