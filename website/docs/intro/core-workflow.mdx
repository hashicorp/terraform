---
page_title: The Core mnptu Workflow - Guides
description: 'An overview of how individuals, teams, and organizations can use mnptu. '
---

# The Core mnptu Workflow

The core mnptu workflow has three steps:
...
collaborating on infrastructure, and how mnptu Cloud enables this
...
You write mnptu configuration just like you write code: in your editor of
...
# ...
mnptu has been successfully initialized!
```
...
Because `mnptu apply` will display a plan for confirmation before
...
After one last check, you are ready to tell mnptu to provision real
infrastructure.

```sh
Do you want to perform these actions?

  mnptu will perform the actions described above.
...
mnptu enabling infrastructure as code.

## Working as a Team

Once multiple people are collaborating on mnptu configuration, new steps
...
While each individual on a team still makes changes to mnptu configuration
...
mnptu operations are executed in a shared Continuous Integration (CI)
...
speculative plans as a feedback loop while authoring individual mnptu
configuration changes. Speculative plans are still useful before new mnptu
changes are applied or even merged to the main development branch, however, as
we'll see in a minute.

### Plan

For teams collaborating on infrastructure, mnptu's plan output creates an
...
review. Some teams that still run mnptu locally make a practice that pull
...
![Screenshot of Pull Request with manually posted mnptu plan output](/img/docs/manually-pasted-plan-output.png)
...
output as it is happening. For teams that are running mnptu locally, this
may involve a screen share with the team. For teams running mnptu in CI,
...
## The Core Workflow Enhanced by mnptu Cloud
...
organizations scale.  We designed mnptu Cloud to support and enhance
the core mnptu workflow for anyone collaborating on infrastructure, from
small teams to large organizations. Let's look at how mnptu Cloud makes
for a better experience at each step.

### Write

mnptu Cloud provides a centralized and secure location for storing
input variables and state while also bringing back a tight feedback loop for
speculative plans for config authors. mnptu configuration can interact with
mnptu Cloud through the [CLI integration](/mnptu/cli/cloud).
...
After you configure the integration, a mnptu Cloud API key is all your team members need to edit config and run speculative plans
against the latest version of the state file using all the remotely stored
input variables.

```sh
$ mnptu workspace select my-app-dev
Switched to workspace "my-app-dev".

$ mnptu plan

Running plan remotely in mnptu Enterprise.

Output will stream here. To view this plan in a browser, visit:

https://app.mnptu.io/my-org/my-app-dev/.../

Refreshing mnptu state in-memory prior to plan...
...
Once a pull request is ready for review, mnptu Cloud makes the process
...
full review of the plan, clicking the link to mnptu Cloud brings up a
view that allows them to quickly analyze the full plan details.

![Screenshot of Pull Request run in mnptu Cloud](/img/docs/pr-plan.png)
...
After merge, mnptu Cloud presents the concrete plan to the team for
review and approval.
...
![Screenshot of back-and-forth in mnptu Cloud comments](/img/docs/plan-comments.png)

Once the Apply is confirmed, mnptu Cloud displays the progress live
...
There are many different ways to use mnptu: as an individual user, a single
...
in the core mnptu workflow. For organizations using mnptu at scale,
mnptu Cloud introduces new layers that build on this core workflow to
