...
When working in a team environment, following consistent conventions helps maintain clarity and organization:

**Branch Naming Conventions:**
- Use descriptive prefixes like `feature/`, `bugfix/`, or `hotfix/`
- Include the issue or ticket number if applicable
- Use kebab-case for readability
Example: `feature/add-vpc-endpoints-123` or `bugfix/fix-s3-permissions`
...
- Include more detailed explanation after a blank line if needed
Example:
```sh
$ git commit -m "Add VPC endpoints for S3 and DynamoDB

This change improves security by keeping traffic within the AWS network
and reduces data transfer costs. Relates to issue #123."
```
