...
The `for_each` value must be an explicit map or set with one element per desired
resource instance. To prevent unwanted surprises during conversion, the `for_each` 
argument does not support implicit type conversion.
