...
* `guest_id` - (Optional) guest OS identifier, as specified in the vSphere API.
...
* `adapter_type` - (Optional) Adapter type, one of 'e1000' or 'vmxnet3'
* `creation_order` - (Optional) Integer, order of creation for said NIC.
