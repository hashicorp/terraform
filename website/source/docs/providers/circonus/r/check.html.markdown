...
* `check_blacklist` - (Optional) A list of check names to exclude from the
  ...
  `service_name`, and `state` check modes.
...
* `node` - (Optional) Check the health of this node.  The value can be either a
  ...
  attribute conflicts with the `service_name` and `state` attributes.
...
* `node_blacklist` - (Optional) A list of node IDs or node names to exclude from
  ...
  `node_blacklist`).  This blacklist is applied to the `node`, `service_name`,
  and `state` check modes.

* `service_name` - (Optional) Check the cluster-wide health of this named
  service.  See also the `service_blacklist`, `node_blacklist`, and
  `check_blacklist` attributes.  This attribute conflicts with the `node` and
  `state` attributes.
...
Available metrics depend on the consul check being performed (`node`,
`service_name`, or `state`).  In addition to the data avilable from the
endpoints, the `consul` check also returns a set of metrics that are a variant
of: `{Num,Pct}{,Passing,Warning,Critical}{Checks,Nodes,Services}` (see the
