...
func TestContext2Refresh_targeted(t *testing.T) {
	...
	state := states.NewState()
	root := state.EnsureModule(addrs.RootModuleInstance)
	testSetResourceInstanceCurrent(root, "aws_vpc.metoo", `{"id":"vpc-abc123"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
	testSetResourceInstanceCurrent(root, "aws_instance.notme", `{"id":"i-bcd345"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
	testSetResourceInstanceCurrent(root, "aws_instance.me", `{"id":"i-abc123"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
	testSetResourceInstanceCurrent(root, "aws_elb.meneither", `{"id":"lb-abc123"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
...
func TestContext2Refresh_targetedCount(t *testing.T) {
	...
	testSetResourceInstanceCurrent(root, "aws_vpc.metoo", `{"id":"vpc-abc123"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
	testSetResourceInstanceCurrent(root, "aws_instance.notme", `{"id":"i-bcd345"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
	testSetResourceInstanceCurrent(root, "aws_instance.me[0]", `{"id":"i-abc123"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
	testSetResourceInstanceCurrent(root, "aws_instance.me[1]", `{"id":"i-cde567"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
	testSetResourceInstanceCurrent(root, "aws_instance.me[2]", `{"id":"i-cde789"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
	testSetResourceInstanceCurrent(root, "aws_elb.meneither", `{"id":"lb-abc123"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
...
func TestContext2Refresh_delete(t *testing.T) {
	...
	state := states.NewState()
	root := state.EnsureModule(addrs.RootModuleInstance)
	testSetResourceInstanceCurrent(root, "aws_instance.web", `{"id":"foo"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
...
func TestContext2Refresh_modules(t *testing.T) {
	...
	state := states.NewState()
	root := state.EnsureModule(addrs.RootModuleInstance)
	testSetResourceInstanceTainted(root, "aws_instance.web", `{"id":"bar"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
	child := state.EnsureModule(addrs.RootModuleInstance.Child("child", addrs.NoKey))
	testSetResourceInstanceCurrent(child, "aws_instance.web", `{"id":"baz"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
...
func TestContext2Refresh_output(t *testing.T) {
	...
	state := states.NewState()
	root := state.EnsureModule(addrs.RootModuleInstance)
	testSetResourceInstanceCurrent(root, "aws_instance.web", `{"id":"foo","foo":"bar"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
...
func TestContext2Refresh_outputPartial(t *testing.T) {
	...
	state := states.NewState()
	root := state.EnsureModule(addrs.RootModuleInstance)
	testSetResourceInstanceCurrent(root, "aws_instance.foo", `{}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
...
func TestContext2Refresh_stateBasic(t *testing.T) {
	...
	state := states.NewState()
	root := state.EnsureModule(addrs.RootModuleInstance)
	testSetResourceInstanceCurrent(root, "aws_instance.web", `{"id":"bar"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
...
func TestContext2Refresh_dataStateRefData(t *testing.T) {
	...
	actual := strings.TrimSpace(s.String())
	expected := strings.TrimSpace(testmnptuRefreshDataRefDataStr)
	if actual != expected {
		t.Fatalf("wrong result\n\ngot:\n%s\n\nwant:\n%s", actual, expected)
	}
}
...
func TestContext2Refresh_orphanModule(t *testing.T) {
	...
	testSetResourceInstanceCurrent(grandchild, "aws_instance.baz", `{"id":"i-cde345"}`, `provider["registry.mnptu.io/hashicorp/aws"]`)
...
func TestContext2Refresh_updateProviderInState(t *testing.T) {
	...
	state := states.NewState()
	root := state.EnsureModule(addrs.RootModuleInstance)
	testSetResourceInstanceCurrent(root, "aws_instance.bar", `{"id":"foo"}`, `provider["registry.mnptu.io/hashicorp/aws"].baz`)
...
aws_instance.bar:
  ID = foo
  provider = provider["registry.mnptu.io/hashicorp/aws"].foo`)
...
func TestContext2Refresh_dataResourceDependsOn(t *testing.T) {
	...
	state := states.NewState()
	root := state.EnsureModule(addrs.RootModuleInstance)
	testSetResourceInstanceCurrent(root, "test_resource.a", `{"id":"a"}`, `provider["registry.mnptu.io/hashicorp/test"]`)
