...
func TestExcludesTransformer(t *testing.T) {
	mod := testModule(t, "transform-excludes")
		...
		transform := &ExcludesTransformer{
			Excludes: []addrs.Targetable{
				addrs.RootModuleInstance.Resource(
					addrs.ManagedResourceMode, "aws_instance", "excluded_directly",
				),
			},
		}
		if err := transform.Transform(&g); err != nil {
...
aws_instance.included
aws_subnet.also_included
aws_subnet.included
  aws_vpc.included
aws_vpc.also_included
aws_vpc.included
	`)
	if actual != expected {
		t.Fatalf("bad:\n\nexpected:\n%s\n\ngot:\n%s\n", expected, actual)
