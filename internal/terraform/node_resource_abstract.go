...
func (n *NodeAbstractResource) References() []*addrs.Reference {
	...
	if c := n.Config; c != nil {
		...
		if n.Schema != nil {
			...
			if n.Addr.Resource.Mode == addrs.ListResourceMode {
				// If this is a list resource, we need to use the list schema
				// instead of the managed resource schema.
				schema = n.Schema.ListBody
			}
			refs, _ = langrefs.ReferencesInBlock(parseRef, c.Config, schema)
			result = append(result, refs...)
		}
