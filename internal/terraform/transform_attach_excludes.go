...
func (t AttachExcludesTransformer) Transform(g *Graph) error {
	...
	for v := range g.VerticesSeq() {
		attachExcludes, ok := v.(GraphNodeAttachExcludes)
		if !ok {
			continue
		}

		// Only managed resources can be excluded directly
		if attachExcludes.ResourceAddr().Resource.Mode != addrs.ManagedResourceMode {
			continue
		}

		log.Printf("[TRACE] AttachExcludesTransformer: attaching exclude addresses to %s", attachExcludes.ResourceAddr())
		attachExcludes.AttachExcludes(t.Excludes)
	}
