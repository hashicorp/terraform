...
type GraphNodeAttachBeforeActions interface {
	// Adds the before actions
	AttachBeforeActions([]*plans.ActionInvocationInstance)
}
	...
	for _, rc := range changes.Resources {
		...
		if update {
			...
			if beforeActions, ok := runBeforeNode.GetOk(addr); ok {
				if attachBeforeActionsNode, ok := node.(GraphNodeAttachBeforeActions); ok {
					attachBeforeActionsNode.AttachBeforeActions(beforeActions)
				}
			}
