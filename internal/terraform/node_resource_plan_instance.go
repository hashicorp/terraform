...
func (n *NodePlannableResourceInstance) importState(ctx EvalContext, addr addrs.AbsResourceInstance, importId string, provider providers.Interface, providerSchema providers.ProviderSchema) (*states.ResourceInstanceObject, *providers.Deferred, tfdiags.Diagnostics) {
	...
	if len(imported) == 0 {
		...
		// TODO: Check if unknown or null is better here. If it is null, we'll
		//   have to fix the validation errors below as they check against
		//   null.
		//
		// Unknown actually represents the value a bit better, as we don't know
		// what it would be. BUT, this value is used in follow-up calls to plan
		// and refresh, and they are not expecting unknown values in the state.
		// They can accept null values, and that would model the change as a
		// create which would at least mean the configuration is validated by
		// the provider IF that is possible. I'd actually lean towards using
		// null, but I've put unknown here so the diff is highlighted.
		//
		// Also, it could be valid to return early from this function if the
		// import is deferred and just do no refresh. The early return should
		// follow the same pattern though, so the plan does still get executed
		// as normal (probably with a null state).
		imported = []providers.ImportedResource{
			{
				TypeName: addr.Resource.Resource.Type,
				State:    cty.UnknownVal(schema.ImpliedType()),
			},
		}
	}
