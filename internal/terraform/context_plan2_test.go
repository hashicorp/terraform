...
func TestContext2Plan_crossResourceMoveWithIdentity(t *testing.T) {
	...
	expectedSourceIdentity := cty.ObjectVal(map[string]cty.Value{
		"oneId": cty.StringVal("before"),
	})
	...
	if !expectedSourceIdentity.RawEquals(sourceIdentity) {
		t.Fatalf("unexpected source identity; expected %s, got %s", expectedSourceIdentity.GoString(), sourceIdentity.GoString())
	}
