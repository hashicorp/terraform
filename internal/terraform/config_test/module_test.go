...
func TestNewModule_provider_local_name(t *testing.T) {
	mod, diags := testModuleFromDirWithInitGraph("testdata/providers-explicit-fqn")
	if diags.HasErrors() {
		t.Fatal(diags.Error())
	}
...
func TestNewModule_resource_providers(t *testing.T) {
	cfg, diags := testConfigFromDirWithInitGraph(t, "testdata/valid-modules/nested-providers-fqns")
	if diags.HasErrors() {
		t.Fatal(diags.Error())
	}
