...
func TestContext2Plan_PolicyValidatePolicies(t *testing.T) {
	...
	t.Run("schemas are sent and a clean run proceeds", func(t *testing.T) {
		...
		if !policyClient.ValidatePoliciesCalled {
			t.Fatal("expected ValidatePolicies to be called before the walk")
		}
		got := policyClient.ValidatePoliciesRequest.ProviderSchemas
	...
	t.Run("an apply validation error blocks provider operations", func(t *testing.T) {
		...
		if calls != 1 {
			t.Fatalf("ValidatePolicies called %d times during apply, want once", calls)
		}
...
func TestValidatePoliciesRequest(t *testing.T) {
	...
	diags := validatePolicies(t.Context(), client, schemas)
	tfdiags.AssertNoDiagnostics(t, diags)
	...
	if got[1].Source != testAddr.String() {
		t.Fatalf("got provider source %q, want %q", got[1].Source, testAddr.String())
	}
...
func TestValidatePoliciesTypeCollision(t *testing.T) {
...
func TestContext2Plan_PolicySchemaValidationCancellation(t *testing.T) {
	...
	policyClient.ValidatePoliciesFn = func(ctx context.Context, req policy.ValidatePoliciesRequest) policy.ValidatePoliciesResponse {
		close(entered)
		select {
		case <-ctx.Done():
		case <-release:
