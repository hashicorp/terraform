...
func TestContext2Apply_targetExcludeInstance(t *testing.T) {
	...
	exc := mustResourceInstanceAddr(`test_object.a[0]`)
	cp := *DefaultPlanOpts
	cp.Excluded = []addrs.Targetable{exc}
	plan, diags := ctx.Plan(m, states.NewState(), &cp)
	assertNoErrors(t, diags)
	...
	// confirm that the excluded resource instance is not created
	var foundResource bool
	for _, ch := range state.RootModule().Resources {
		if ch.Addr.Resource.Equal(exc.Resource.Resource) {
			foundResource = true
			for key := range ch.Instances {
				if key == addrs.IntKey(0) && ch.Addr.Resource.Equal(exc.Resource.Resource) {
	...
	if !foundResource {
		t.Fatalf("expected resource %s to be present", exc)
	}
}
...
func TestContext2Apply_targetExcludeDeferred(t *testing.T) {
	m := testModuleInline(t, map[string]string{
		"main.tf": `
		resource "test_object" "a" {
			# This resource would fail if a plan request is made,
			# But since it is excluded, it should not be planned in the first place
			# This is to test that the exclusion is respected
			test_string = "bad"
		}
		resource "test_object" "s" {
		}
...
func TestContext2Apply_excludeDependsOnDeferred(t *testing.T) {
	...
	// The plan should now include test_instance.foo and its dependants
	...
	// confirm that test_instance.foo and its dependents are now in the state
