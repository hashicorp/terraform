...
// callback. This allows you to modify the behavior of mnptu at runtime.
...
const (
	...
	// and the action that mnptu was about to take is cancelled.
	HookActionHalt
)

// Hook is the interface that must be implemented to hook into various
// parts of mnptu, allowing you to inspect or change behavior at runtime.
//
// There are MANY hook points into mnptu. If you only want to implement
...
type Hook interface {
	...
	// Stopping is called if an external signal requests that mnptu
	// gracefully abort an operation in progress.
	//
	// This notification might suggest that the user wants mnptu to exit
	// ASAP and in that case it's possible that if mnptu runs for too much
	...
	//
	// This hook cannot control whether mnptu continues, because the
