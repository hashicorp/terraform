...
// StateStorage is the minimal interface that must be implemented to enable Terraform.
type StateStorage interface {
	// ConfigSchema returns a description of the expected configuration
	// structure for the receiving state storage.
	//
	// This method does not have any side-effects for the state storage and can
	...
	// via PrepareConfig.
	//
	// This method may be called only once per state storage instance, and must be
	...
	// in this state storage.
	Workspaces() ([]string, error)
}

type InitFn func() StateStorage

func Storage(typ string) (InitFn, tfdiags.Diagnostics) {
	return nil, nil
}
