...
func (p *GRPCProvider) ListResource(r providers.ListResourceRequest) providers.ListResourceResponse {
	...
	// Filter out the "data" attribute from the list resource schema, as that represents
	// the data returned by the list resource, not the configuration for the resource.
	configSchema := listResourceSchema.Body.Filter(configschema.FilterAttrByName("data"), nil)
	mp, err := msgpack.Marshal(r.Config, configSchema.ImpliedType())
	if err != nil {
		resp.Diagnostics = resp.Diagnostics.Append(err)
