...
func (p *provider) ApplyResourceChange(_ context.Context, req *tfplugin5.ApplyResourceChange_Request) (*tfplugin5.ApplyResourceChange_Response, error) {
	...
	resSchema, ok := p.schema.ResourceTypes[req.TypeName]
	if !ok {
		return resp, fmt.Errorf("could not find schema for resource type %s", req.TypeName)
	}
	ty := resSchema.Body.ImpliedType()
...
func (p *provider) MoveResourceState(_ context.Context, request *tfplugin5.MoveResourceState_Request) (*tfplugin5.MoveResourceState_Response, error) {
	...
	if request.SourceIdentity != nil && request.SourceIdentity.IdentityData != nil {
		sourceResSchema := p.schema.ResourceTypes[request.SourceTypeName]

		if sourceResSchema.Identity == nil {
			return resp, fmt.Errorf("identity schema not found for type %s", request.SourceTypeName)
		}

		sourceIdentity, err = decodeDynamicValue(request.SourceIdentity.IdentityData, sourceResSchema.Identity.ImpliedType())
		if err != nil {
			resp.Diagnostics = convert.AppendProtoDiag(resp.Diagnostics, err)
			return resp, nil
		}
