...
func TestDiagnosticComparerWithSource(t *testing.T) {
	...
	cases := map[string]struct {
		...
		// Correctly identifying things that match
		"reports that identical diagnostics match, and tolerates lack of sources": {
			diag1:      hclDiagnostic{&baseError},
			diag2:      hclDiagnostic{&baseError},
			expectDiff: false,
		},
	...
	for tn, tc := range cases {
		t.Run(tn, func(t *testing.T) {
			...
			output := cmp.Diff(tc.diag1, tc.diag2, DiagnosticComparerWithSource)
