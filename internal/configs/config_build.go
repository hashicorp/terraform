...
func loadModule(root *Config, req *ModuleRequest, walker ModuleWalker) (*Config, hcl.Diagnostics) {
	...
	if mod.StateStore != nil {
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   "Any selected state store applies to the entire configuration, so Terraform expects 'state_store' configurations only in the root module.\n\nThis is a warning rather than an error because it's sometimes convenient to temporarily call a root module as a child module for testing purposes, but this 'state_store' configuration block will have no effect.",
