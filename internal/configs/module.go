...
func (m *Module) appendFile(file *File) hcl.Diagnostics {
	...
	for _, i := range file.Import {
		...
		// KEM: This panics in the presence of an invalid import block without To or From argument.
		// We don't know enough about import and moved blocks to cross-validate here: essentially we need to defer this
		// to the moved block validation, which only happens after plan.
		...
		// 	if mb.From != nil && mb.From.String() == i.ToResource.String() {
		// 		diags = append(diags, &hcl.Diagnostic{
		// 			Severity: hcl.DiagError,
