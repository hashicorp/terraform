	...
	diags, err := c.validate.Do(ctx, func(ctx context.Context) (tfdiags.Diagnostics, error) {
		...
		if !valid {
			diags = diags.Append(&hcl.Diagnostic{
				...
				Detail:   fmt.Sprintf("Cannot validate %q because its provider configuration assignments are invalid.", "TODO: Do we need an address here?"),
		...
		if !exists {
			...
			diags = diags.Append(&hcl.Diagnostic{
				...
				Detail: fmt.Sprintf(
					...
					"TODO: Maybe we need an adress????", componentAddr.StringCompact(), typeAddr.ForDisplay(),
				),
...
func (c *RemovedConfig) StackConfig(ctx context.Context) *StackConfig {
	return c.main.mainStack.StackConfig(ctx) // TODO: Maybe we do need an address???
}
