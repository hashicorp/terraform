...
func (r *Removed) Config(ctx context.Context) *RemovedConfig {
	...
	return stackConfig.RemovedBlock(ctx, configAddr.Item)
}

func (r *Removed) Declaration(ctx context.Context) *stackconfig.Component {
	cfg := r.Config(ctx)
	if cfg == nil {
		return nil
	}
...
func (r *Removed) ForEachValue(ctx context.Context, phase EvalPhase) cty.Value {
	ret, _ := r.CheckForEachValue(ctx, phase)
	return ret
}
...
func (r *Removed) CheckForEachValue(ctx context.Context, phase EvalPhase) (cty.Value, tfdiags.Diagnostics) {
	val, diags := doOnceWithDiags(
		ctx, r.forEachValue.For(phase), r.main,
		func(ctx context.Context) (cty.Value, tfdiags.Diagnostics) {
			var diags tfdiags.Diagnostics
			cfg := r.Declaration(ctx)

			switch {

			case cfg.ForEach != nil:
				result, moreDiags := evaluateForEachExpr(ctx, cfg.ForEach, phase, r.Stack(ctx), "removed")
...
func (r *Removed) Instances(ctx context.Context, phase EvalPhase) (map[addrs.InstanceKey]*ComponentInstance, bool) {
	ret, unknown, _ := r.CheckInstances(ctx, phase)
	return ret, unknown
}

func (r *Removed) CheckInstances(ctx context.Context, phase EvalPhase) (map[addrs.InstanceKey]*ComponentInstance, bool, tfdiags.Diagnostics) {
	result, diags := doOnceWithDiags(
		ctx, r.instances.For(phase), r.main,
		func(ctx context.Context) (instancesResult[*ComponentInstance], tfdiags.Diagnostics) {
			var diags tfdiags.Diagnostics
			forEachVal, forEachValueDiags := r.CheckForEachValue(ctx, phase)
			...
			result := instancesMap(forEachVal, func(ik addrs.InstanceKey, rd instances.RepetitionData) *RemovedBlockInstance {
				return newRemovedBlockInstance(r, ik, rd)
			})
...
func (r *Removed) UnknownInstance(ctx context.Context, phase EvalPhase) *ComponentInstance {
	inst, err := r.unknownInstance.For(PlanPhase).Do(ctx, func(ctx context.Context) (*ComponentInstance, error) {
		return newComponentInstance(r, addrs.WildcardKey, instances.UnknownForEachRepetitionData(r.ForEachValue(ctx, phase).Type())), nil
	})
	if err != nil {
...
func (r *Removed) ResultValue(ctx context.Context, phase EvalPhase) cty.Value {
	decl := r.Declaration(ctx)
	insts, unknown := r.Instances(ctx, phase)
...
func (r *Removed) PlanIsComplete(ctx context.Context) bool {
	...
	insts, unknown := r.Instances(ctx, PlanPhase)
	if insts == nil {
...
func (r *Removed) checkValid(ctx context.Context, phase EvalPhase) tfdiags.Diagnostics {
	var diags tfdiags.Diagnostics

	_, _, moreDiags := r.CheckInstances(ctx, phase)
	diags = diags.Append(moreDiags)
...
func (r *Removed) References(ctx context.Context) []stackaddrs.AbsReference {
	cfg := r.Declaration(ctx)
	...
	return makeReferencesAbsolute(ret, r.Addr().Stack)
}

// RequiredComponents implements Applyable
func (r *Removed) RequiredComponents(ctx context.Context) collections.Set[stackaddrs.AbsComponent] {
	return r.main.requiredComponentsForReferrer(ctx, r, PlanPhase)
}
...
func (r *Removed) CheckApply(ctx context.Context) ([]stackstate.AppliedChange, tfdiags.Diagnostics) {
	return nil, r.checkValid(ctx, ApplyPhase)
}
...
func (r *Removed) ApplySuccessful(ctx context.Context) bool {
	...
	// apply phases.
	insts, _ := r.Instances(ctx, ApplyPhase)
...
func (r *Removed) reportNamedPromises(cb func(id promising.PromiseID, name string)) {
	...
	r.instances.Each(func(ep EvalPhase, o *promising.Once[withDiagnostics[instancesResult[*ComponentInstance]]]) {
