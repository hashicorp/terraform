...
// the collection's resource attribute.
func decompose(expr hcl.Expression, env map[string][]hcl.Traversal) (traversals []hcl.Traversal, hasNonRef bool, ok bool) {
	switch e := hcl.UnwrapExpression(expr).(type) {
	case *hclsyntax.ParenthesesExpr:
		return decompose(e.Expression, env)
	...
	case *hclsyntax.RelativeTraversalExpr:
		base, nn, bok := decompose(e.Source, env)
		if !bok {
	...
	case *hclsyntax.IndexExpr:
		...
		return decompose(e.Collection, env)
	...
	case *hclsyntax.ObjectConsExpr:
		...
		for _, item := range e.Items {
			vt, vn, vok := decompose(item.ValueExpr, env)
			if !vok {
				return nil, false, false
			}
			all = append(all, vt...)
	...
	case *hclsyntax.ConditionalExpr:
		tt, tn, tok := decompose(e.TrueResult, env)
		ft, fn, fok := decompose(e.FalseResult, env)
		if !tok || !fok {
			return nil, false, false
		}
	...
	case *hclsyntax.ForExpr:
		...
		collBase, collNonRef, collOk := decompose(e.CollExpr, env)
		if !collOk {
			return nil, false, false
		}
		...
		vt, vn, vok := decompose(e.ValExpr, child)
		if !vok {
			return nil, false, false
		}
		all := vt
		nn := collNonRef || vn
		if e.KeyExpr != nil {
			kt, kn, kok := decompose(e.KeyExpr, child)
			if !kok {
				return nil, false, false
			}
			all = append(all, kt...)
	...
	case *hclsyntax.TemplateWrapExpr:
		...
		return decompose(e.Wrapped, env)
