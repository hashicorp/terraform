...
func TestKeyToJwks(t *testing.T) {
	tests := []struct {
		...
		// RSA Public Key
		{
			cty.StringVal(PublicKey),
			cty.StringVal(`{"e":"AQAB","kty":"RSA","n":"gUElV5mwqkloIrM8ZNZ72gSCcnSJt7-_Usa5G-D15YQUAdf9c1zEekTfHgDP-04nw_uFNFaE5v1RbHaPxhZYVg5ZErNCa_hzn-x10xzcepeS3KPVXcxae4MR0BEegvqZqJzN9loXsNL_c3H_B-2Gle3hTxjlWFb3F5qLgR-4Mf4ruhER1v6eHQa_nchi03MBpT4UeJ7MrL92hTJYLdpSyCqmr8yjxkKJDVC2uRrr-sTSxfh7r6v24u_vp_QTmBIAlNPgadVAZw17iNNb7vjV7Gwl_5gHXonCUKURaV--dBNLrHIZpqcAM8wHRph8mD1EfL9hsz77pHewxolBATV-7Q"}`),
			"",
		},
		// EC Private Key
		{
			cty.StringVal(ECPrivateKey),
			cty.StringVal(`{"crv":"P-384","d":"WL_isXL5n9Ux2yd2phRRMH_aPCZwVfghUEhEU9F0L0EKoi7MD0oRirlsqWuYsNwd","kty":"EC","x":"QnSqFbKGdEiz_g0CoZFXBAvsMMbMBqqnZ3jJ_zrLxuBShDTmrsRoU7MCOALbt04f","y":"FoRI8H06aS1Wq1XsAbMiMLUqpl5joOWeUChxyPb0v-7B86IE9299UHBpbAfcnthd"}`),
			"",
		},
		// EC Public Key
		{
			cty.StringVal(ECPublicKey),
			cty.StringVal(`{"crv":"P-384","kty":"EC","x":"QnSqFbKGdEiz_g0CoZFXBAvsMMbMBqqnZ3jJ_zrLxuBShDTmrsRoU7MCOALbt04f","y":"FoRI8H06aS1Wq1XsAbMiMLUqpl5joOWeUChxyPb0v-7B86IE9299UHBpbAfcnthd"}`),
			"",
		},
		// Bad key
		{
			cty.StringVal(BadPrivateKey),
			cty.UnknownVal(cty.String),
			"unable to parse private or public key pem",
		},
		// non-pem encoded data
		{
			cty.StringVal("this is not a key"),
			cty.UnknownVal(cty.String),
			"unable to decode key pem",
		},
	}
	for _, test := range tests {
		t.Run(fmt.Sprintf("KeyToJwks(%#v)", test.Key), func(t *testing.T) {
			got, err := KeyToJwks(test.Key)
