...
// TextEncodeBase64Func constructs a function that encodes a string to a target
// encoding and then to a base64 sequence.
//
// Terraform uses the "standard" Base64 alphabet as defined in RFC 4648 section
// 4.
//
// First step is to apply the target IANA encoding (e.g. UTF-16LE). Strings in
// the Terraform language are sequences of unicode characters rather than bytes,
// so this function will first encode the characters from the string as UTF-8,
// and then apply Base64 encoding to the result.
