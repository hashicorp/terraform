...
// StacksCLIPluginClient fetches and verifies release distributions of the stacks-cli plugin
// that correspond to an upstream backend.
type StacksCLIPluginClient struct {
	serviceURL *url.URL
...
func requestLogHook(logger retryablehttp.Logger, req *http.Request, i int) {
	if i > 0 {
		logger.Printf("[INFO] Previous request to the remote stacks-cli manifest failed, attempting retry.")
	}
}
...
// NewStacksCLIPluginClient creates a new client for downloading and verifying
// stacks-cli plugin archives
func NewStacksCLIPluginClient(ctx context.Context, serviceURL *url.URL) (*StacksCLIPluginClient, error) {
...
// FetchManifest retrieves the stacks-cli plugin manifest from HCP Terraform,
// but returns a nil manifest if a 304 response is received, depending
// on the lastModified time.
func (c StacksCLIPluginClient) FetchManifest(lastModified time.Time) (*Release, error) {
	...
	case http.StatusNotFound:
		return nil, ErrStacksCLIPluginNotSupported
	default:
		return nil, ErrQueryFailed{
...
func (c StacksCLIPluginClient) DownloadFile(pathOrURL string, writer io.Writer) error {
	...
	if err != nil {
		return fmt.Errorf("invalid URL %q was provided by the stacks-cli plugin manifest: %w", url, err)
	}
	resp, err := c.httpClient.Do(req)
	if err != nil {
		if errors.Is(err, context.Canceled) {
	...
	case http.StatusOK:
		// OK
	case http.StatusNotFound:
		return ErrStacksCLIPluginNotFound
	default:
...
func (c StacksCLIPluginClient) resolveManifestURL(pathOrURL string) (*url.URL, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("received malformed URL %q from stacks-cli plugin manifest: %w", pathOrURL, err)
	}
	return result, nil
}
...
func (m Release) Select(goos, arch string) (*BuildArtifact, error) {
	...
	log.Printf("[TRACE] checking for stacks-cli plugin archive for %s. Supported architectures: %v", osArchKey, supported)
