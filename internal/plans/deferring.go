...
func (rcs *DeferredActionInvocation) Encode(schema *providers.ActionSchema) (*DeferredActionInvocationChangeSrc, error) {
	src, err := rcs.ActionInvocationInstance.Encode(schema)
	if err != nil {
		return nil, err
	}

	return &DeferredActionInvocationChangeSrc{
		DeferredReason:              rcs.DeferredReason,
		ActionInvocationInstanceSrc: src,
	}, nil
}

// DeferredActionInvocationChangeSrc tracks information about an action invocation
// that has been deferred for some reason.
type DeferredActionInvocationChangeSrc struct {
	// DeferredReason is the reason why this action invocation was deferred.
	DeferredReason providers.DeferredReason
...
func (rcs *DeferredActionInvocationChangeSrc) Decode(schema *providers.ActionSchema) (*DeferredActionInvocation, error) {
	instance, err := rcs.ActionInvocationInstanceSrc.Decode(schema)
	if err != nil {
		return nil, err
	}
