...
func (c *Change) Encode(schema *providers.Schema) (*ChangeSrc, error) {
	...
	if !c.BeforeIdentity.IsNull() {
		...
		if len(unsupportedBeforeIdentityMarks) != 0 {
			return nil, fmt.Errorf(
				"prior identity %s: can't serialize value marked with %#v (this is a bug in Terraform)",
				tfdiags.FormatCtyPath(unsupportedBeforeIdentityMarks[0].Path),
				unsupportedBeforeIdentityMarks[0].Marks,
			)
		}
	...
	if !c.AfterIdentity.IsNull() {
		...
		if len(unsupportedAfterIdentityMarks) != 0 {
			return nil, fmt.Errorf(
				"new identity %s: can't serialize value marked with %#v (this is a bug in Terraform)",
				tfdiags.FormatCtyPath(unsupportedAfterIdentityMarks[0].Path),
				unsupportedAfterIdentityMarks[0].Marks,
			)
		}
