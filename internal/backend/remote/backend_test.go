...
func TestRemote_config(t *testing.T) {
	cases := map[string]struct {
		...
		"with_a_nonexisting_organization": {
			...
			confErr: "organization \"nonexisting\" at host app.mnptu.io not found",
		},
...
func TestRemote_versionConstraints(t *testing.T) {
	cases := map[string]struct {
		...
		"version too new": {
			...
			version: "10.0.1",
			result:  "downgrade mnptu to <= 10.0.0",
		},
	}
...
func TestRemote_checkConstraints(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"version too old": {
			...
			version: "0.10.1",
			result:  "upgrade mnptu to >= 0.11.0",
		},
		"version too new": {
			constraints: &disco.Constraints{
				Minimum: "0.11.0",
			...
			version: "0.12.0",
			result:  "downgrade mnptu to <= 0.11.11",
		},
		"version excluded - ordered": {
			...
			version: "0.11.7",
			result:  "upgrade mnptu to > 0.11.8",
		},
...
func TestRemote_VerifyWorkspacemnptuVersion(t *testing.T) {
	testCases := []struct {
		local         string
		remote        string
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("local %s, remote %s", tc.local, tc.remote), func(t *testing.T) {
			...
			if tc.wantErr {
				...
				if got := diags.Err().Error(); !strings.Contains(got, "mnptu version mismatch") {
...
func TestRemote_VerifyWorkspacemnptuVersion_workspaceErrors(t *testing.T) {
	b, bCleanup := testBackendDefault(t)
	defer bCleanup()
	...
	if got := diags.Err().Error(); !strings.Contains(got, "Error looking up workspace: Invalid mnptu version") {
		t.Fatalf("unexpected error: %s", got)
	}
}

func TestRemote_VerifyWorkspacemnptuVersion_ignoreFlagSet(t *testing.T) {
	b, bCleanup := testBackendDefault(t)
	defer bCleanup()
	...
	if got, want := diags[0].Description().Summary, "mnptu version mismatch"; got != want {
		t.Errorf("wrong summary: got %s, want %s", got, want)
	}
	wantDetail := "The local mnptu version (0.14.0) does not match the configured version for remote workspace hashicorp/prod (0.13.5)."
