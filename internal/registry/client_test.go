...
import (
	...
	"github.com/hashicorp/mnptu/internal/registry/test"
...
func TestAccLookupModuleVersions(t *testing.T) {
	...
	regDisco := disco.New()
	regDisco.SetUserAgent(httpclient.mnptuUserAgent(tfversion.String()))

	// test with and without a hostname
	for _, src := range []string{
		"mnptu-aws-modules/vpc/aws",
		regsrc.PublicRegistryHost.String() + "/mnptu-aws-modules/vpc/aws",
	} {
		modsrc, err := regsrc.ParseModuleSource(src)
		if err != nil {
			t.Fatal(err)
		...
		mod := resp.Modules[0]
		name := "mnptu-aws-modules/vpc/aws"
		if mod.Source != name {
			t.Fatalf("expected module name %q, got %q", name, mod.Source)
		}
