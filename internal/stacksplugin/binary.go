...
type BinaryManager struct {
	signingKey          string
	binaryName          string
	stacksPluginDataDir string
	overridePath        string
	host                svchost.Hostname
	client              *StacksPluginClient
	goos                string
	arch                string
	ctx                 context.Context
}
...
func NewBinaryManager(ctx context.Context, stacksPluginDataDir, overridePath string, serviceURL *url.URL, goos, arch string) (*BinaryManager, error) {
...
func (v BinaryManager) binaryLocation() string {
	return path.Join(v.stacksPluginDataDir, "bin", fmt.Sprintf("%s_%s", v.goos, v.arch))
}
...
func (v BinaryManager) latestManifest(ctx context.Context) (*Release, error) {
	manifestCacheLocation := path.Join(v.stacksPluginDataDir, v.host.String(), "manifest.json")
