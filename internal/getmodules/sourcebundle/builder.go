...
func (b *Builder) AddRemoteSource(ctx context.Context, src addrs.ModuleSourceRemote, srcRng *tfdiags.SourceRange, depFinder DependencyFinder) tfdiags.Diagnostics {
	...
	err = b.fetcher.FetchPackage(ctx, tmpDir, src.Package.String())
	if err != nil {
		...
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("Could not download module package %q: %s.", src, err),
			Subject:  hclRng,
		})
		return diags
	}
	...
	if err != nil {
		diags = diags.Append(&hcl.Diagnostic{
			Severity: hcl.DiagError,
			Summary:  "Invalid remote module package",
			Detail:   fmt.Sprintf("Module package from %q has invalid contents: %s.", src.Package, err),
			Subject:  hclRng,
		})
		return diags
	}
	...
	if err != nil {
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("Cannot calculate checksum for contents of package %q: %s.", src.Package, err),
			Subject:  hclRng,
		})
		return diags
	}
	...
	if _, exists := b.knownPackageHashes[hash]; exists {
		...
		if err != nil {
			diags = diags.Append(tfdiags.Sourceless(
				...
				fmt.Sprintf("Failed to move module package %q to its final bundle location: %s.", src.Package, err),
			))
			return diags
		}
	}

	b.manifest.saveModulePackageBundleDir(src.Package, hash)
