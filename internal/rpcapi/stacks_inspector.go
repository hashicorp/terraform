...
func (i *stacksInspector) InspectExpressionResult(ctx context.Context, req *rawstacks1.InspectExpressionResult_Request) (*rawstacks1.InspectExpressionResult_Response, error) {
	var diags tfdiags.Diagnostics

	expr, hclDiags := hclsyntax.ParseExpression(req.ExpressionSrc, "<external expression>", hcl.InitialPos)
	diags = diags.Append(hclDiags)
	if diags.HasErrors() {
		return &rawstacks1.InspectExpressionResult_Response{
	...
	return &rawstacks1.InspectExpressionResult_Response{
		Result:      rawstacks1.NewDynamicValue(valRaw, sensitivePaths),
		Diagnostics: diagnosticsToProto(diags),
	}, nil
}
