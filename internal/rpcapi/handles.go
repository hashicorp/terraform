...
func (t *handleTable) NewStackPriorState(state *stackstate.State) handle[*stackstate.State] {
	return newHandle(t, state)
}

func (t *handleTable) StackPriorState(hnd handle[*stackstate.State]) *stackstate.State {
	ret, _ := readHandle(t, hnd) // non-existent or invalid returns nil
	return ret
}

func (t *handleTable) CloseStackPriorState(hnd handle[*stackstate.State]) error {
	return closeHandle(t, hnd)
}
