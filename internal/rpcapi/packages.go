...
import (
	...
	regaddrs "github.com/hashicorp/terraform-registry-address"
...
func (p *packagesServer) ComponentPackageSourceAddr(ctx context.Context, request *packages.ComponentPackageSourceAddr_Request) (*packages.ComponentPackageSourceAddr_Response, error) {
	...
	comp, err := regaddrs.ParseComponentSource(request.SourceAddr)
	if err != nil {
		response.Diagnostics = append(response.Diagnostics, &terraform1.Diagnostic{
			Severity: terraform1.Diagnostic_ERROR,
			Summary:  "Invalid Component Sources",
	...
	if err != nil {
		response.Diagnostics = append(response.Diagnostics, &terraform1.Diagnostic{
			Severity: terraform1.Diagnostic_ERROR,
			Summary:  "Failed to query component package metadata",
			Detail:   fmt.Sprintf("Could not retrieve package metadata for provider %s at %s: %s", comp.ForDisplay(), request.Version, err),
		})
		return response, nil
	}
