...
// SafeInitAction describes the different kinds of checkable objects.
...
func (c *InitCommand) getProvidersFromConfig(
	...
	upgrade bool,
	pluginDirs arguments.FlagStringSlice, // Used to set up the installer's sources.
	flagLockfile string,
	view views.Init,
) (
	output bool,
	resultingLocks *depsfile.Locks,
	safeInitAction SafeInitAction,
	...
	evts := &providercache.InstallerEvents{
		...
		FetchPackageBegin: func(provider addrs.Provider, version getproviders.Version, location getproviders.PackageLocation) {
			...
			//
			// FetchPackageBegin is the callback hook at the start of the process of obtaining a provider from a source
			// that isn't yet in the dependency lock file. Providers that are processed here will not be processed here
			// on the next init, as then they will be in the lock file. The same provider type would only be processed
			// here again if the provider version changed via an `init -upgrade` command.
	...
	if config.Module.StateStore != nil {
		...
		if !ok {
			...
			case getproviders.PackageLocalArchive, getproviders.PackageLocalDir:
				...
				// TODO - do we notify the user about this? E.g. output to the terminal, or a warning?
				// No - silently tolerate; user should know where they've configured providers to come from.
			...
			case getproviders.PackageHTTPURL:
				...
				if c.input {
					...
					// advice to users - does the installation source match what the user expects?
					// hash with combo of OS and Arch - only relevant hash to show in prompt
				} else {
