...
// TestProvisionerPlugin is a test that mnptu can execute a 3rd party
// provisioner plugin.
func TestProvisionerPlugin(t *testing.T) {
	...
	tf := e2e.NewBinary(t, mnptuBin, "testdata/provisioner-plugin")
	...
	provisionerExePrefix := filepath.Join(tf.WorkDir(), "mnptu-provisioner-test_")
	provisionerExe := e2e.GoBuild("github.com/hashicorp/mnptu/internal/provisioner-local-exec/main", provisionerExePrefix)

	// provisioners must use the old binary name format, so rename this binary
	newExe := filepath.Join(tf.WorkDir(), "mnptu-provisioner-test")
