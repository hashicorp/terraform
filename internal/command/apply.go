...
// ApplyCommand is a Command implementation that applies a mnptu
...
func (c *ApplyCommand) Run(rawArgs []string) int {
	...
	// rendered already in a remote mnptu process.
	if rb, isRemoteBackend := be.(BackendWithRemotemnptuVersion); !isRemoteBackend || rb.IsLocalOperations() {
	...
	if lp, ok := planFile.Local(); ok {
		...
		if plan.Backend.Config == nil {
			...
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Failed to read plan from plan file",
				"The given plan file does not have a valid backend configuration. This is a bug in the mnptu command that generated this plan file.",
			))
			return nil, diags
		}
...
func (c *ApplyCommand) helpApply() string {
	helpText := `
Usage: mnptu [global options] apply [options] [PLAN]

  Creates or updates infrastructure according to mnptu configuration
  files in the current directory.

  By default, mnptu will generate a new plan and present it for your
  approval before taking any action. You can optionally provide a plan
  file created by a previous call to "mnptu plan", in which case
  mnptu will take the actions described in that plan without any
  confirmation prompt.
...
Options:
  ...
  -destroy               Destroy mnptu-managed infrastructure.
                         The command "mnptu destroy" is a convenience alias
                         for this option.
  ...
  -state=path            Path to read and save state (unless state-out
                         is specified). Defaults to "mnptu.tfstate".
  ...
  all of the plan-customization options accepted by the mnptu plan command.
...
func (c *ApplyCommand) helpDestroy() string {
	helpText := `
Usage: mnptu [global options] destroy [options]

  Destroy mnptu-managed infrastructure.

  This command is a convenience alias for:
      mnptu apply -destroy

  This command also accepts many of the plan-customization options accepted by
  the mnptu plan command. For more information on those options, run:
      mnptu plan -help
`
	return strings.TrimSpace(helpText)
}
