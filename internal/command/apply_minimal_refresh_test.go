...
func TestApply_minimal_refresh(t *testing.T) {
	td := t.TempDir()
	testCopyDir(t, testFixturePath("apply-minimal-refresh"), td)
	t.Chdir(td)
	...
	if fooRefreshed {
		t.Error(`Unexpected call to ReadResource for the "foo" resource. This resource should not be refreshed with ` +
			`the -minimal-refresh flag as the configuration did not change from prior state.`)
	}

	if !bazRefreshed {
		t.Error(`Expected a call to ReadResource for the "baz" resource but received none. This resource should be refreshed with ` +
			`the -minimal-refresh flag as the configuration changed from prior state.`)
	}
}

func TestApply_minimal_refresh_destroy(t *testing.T) {
	td := t.TempDir()
	testCopyDir(t, testFixturePath("apply-minimal-refresh"), td)
	t.Chdir(td)
	...
	testState := states.BuildState(func(s *states.SyncState) {
		s.SetResourceInstanceCurrent(
			...
			&states.ResourceInstanceObjectSrc{
				// No refresh should occur because we're destroying and -minimal-refresh optimizes to skip refresh
				AttrsJSON: []byte(`{"id":"bar","ami": "bar"}`),
				Status:    states.ObjectReady,
			},
			addrs.AbsProviderConfig{
...
func TestDestroy_minimal_refresh_destroy(t *testing.T) {
	td := t.TempDir()
	testCopyDir(t, testFixturePath("apply-minimal-refresh"), td)
	t.Chdir(td)
...
func TestApply_minimal_refresh_invalid_flags(t *testing.T) {
	td := t.TempDir()
	testCopyDir(t, testFixturePath("apply-minimal-refresh"), td)
	t.Chdir(td)
