...
func TestNewInit_ReusingPreviousVersion_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	p := addrs.MustParseProviderSourceString("hashicorp/test")
	initView.ReusingPreviousVersion(p)
...
func TestNewInit_FindingLatestVersion_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	p := addrs.MustParseProviderSourceString("hashicorp/test")
	initView.FindingLatestVersion(p)
...
func TestNewInit_InstallingProvider_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)
	...
	p := addrs.MustParseProviderSourceString("hashicorp/test")
	v := versions.MustParseVersion("1.0.0")
	initView.InstallingProvider(p, v)
...
func TestNewInit_BuiltInProviderAvailable_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	p := addrs.MustParseProviderSourceString("hashicorp/test")
	initView.BuiltInProviderAvailable(p)
...
func TestNewInit_UsingProviderFromCacheDirInfo_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)
	...
	p := addrs.MustParseProviderSourceString("hashicorp/test")
	v := versions.MustParseVersion("1.0.0")
	initView.UsingProviderFromCacheDirInfo(p, v)
...
func TestNewInit_PartnerAndCommunityProviders_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	initView.PartnerAndCommunityProviders()
...
func TestNewInit_InitializingStateStoreProviderPlugin_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	storeType := "test_store"
	initView.InitializingStateStoreProviderPlugin(storeType)
...
func TestNewInit_LockfileCreated_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	initView.LockfileCreated()

	// Assert output
	output := done(t)
	expectedOutputFields := []string{
		`"@level":"info"`,
		`"@message":"Terraform has created a lock file .terraform.lock.hcl to record the provider\nselections it made above. Include this file in your version control repository\nso that Terraform can guarantee to make the same selections by default when\nyou run \"terraform init\" in the future."`,
		`"@module":"terraform.ui"`,
		//@timestamp is dynamic
...
func TestNewInit_LockfileUpdated_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	initView.LockfileUpdated()
...
func TestNewInit_InitializingStateStore_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	storeType := "test_store"
	initView.InitializingStateStore(storeType)

	// Assert output
	output := done(t)
	expectedOutputFields := []string{
		`"@level":"info"`,
		`"@message":"Initializing the state store \"test_store\"..."`,
		`"@module":"terraform.ui"`,
		//@timestamp is dynamic
...
func TestNewInit_InitializingBackend_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	initView.InitializingBackend()
...
func TestNewInit_CopyingConfiguration_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	source := "/path/to/module"
	initView.CopyingConfiguration(source)

	// Assert output
	output := done(t)
	expectedOutputFields := []string{
		`"@level":"info"`,
		`"@message":"Copying configuration from \"/path/to/module\"..."`,
		`"@module":"terraform.ui"`,
		//@timestamp is dynamic
...
func TestNewInit_OutputInitEmpty_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	initView.OutputInitEmpty()

	// Assert output
	output := done(t)
	expectedOutputFields := []string{
		`"@level":"info"`,
		`"@message":"Terraform initialized in an empty directory!\n\nThe directory has no Terraform configuration files. You may begin working\nwith Terraform immediately by creating Terraform configuration files.`,
		`"@module":"terraform.ui"`,
		//@timestamp is dynamic
...
func TestNewInit_OutputInitSuccess_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	initView.OutputInitSuccess()
...
func TestNewInit_OutputInitSuccessCloud_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	initView.OutputInitSuccessCloud()
...
func TestNewInit_OutputInitSuccessCLI_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	initView.OutputInitSuccessCLI()
...
func TestNewInit_OutputInitSuccessCLICloud_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	initView.OutputInitSuccessCLICloud()

	// Assert output
	output := done(t)
	expectedOutputFields := []string{
		`"@level":"info"`,
		`"@message":"You may now begin working with HCP Terraform. Try running \"terraform plan\" to\nsee any changes that are required for your infrastructure.\n\nIf you ever set or change modules or Terraform Settings, run \"terraform init\"\nagain to reinitialize your working directory."`,
		`"@module":"terraform.ui"`,
		//@timestamp is dynamic
...
func TestNewInit_InitializingModules_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	initView.InitializingModules()
...
func TestNewInit_UpgradingModules_json(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	view := NewView(streams)
	initView := NewInit(arguments.ViewJSON, view)

	initView.UpgradingModules()
