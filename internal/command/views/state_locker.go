...
func NewStateLocker(vt arguments.ViewType, view *View) StateLocker {
	...
	case arguments.ViewJSON:
		// A state locker view is always a secondary view during a command,
		// so we don't want to include the version log a second+ time.
		return &StateLockerJSON{view: NewJSONViewNoVersionLog(view)}
	default:
