...
func TestTaintHuman_success(t *testing.T) {
	...
	got := done(t).Stdout()
	if want := "Resource instance test_instance.foo has been marked as tainted."; !strings.Contains(got, want) {
		t.Errorf("wrong output\ngot:  %q\nwant: %q", got, want)
	}
}

func TestTaintHuman_allowMissingWarning(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	baseView := NewView(streams)
	baseView.Configure(&arguments.View{NoColor: true})
	v := NewTaint(baseView)
...
func TestTaintHuman_diagnostics(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	baseView := NewView(streams)
	baseView.Configure(&arguments.View{NoColor: true})
	v := NewTaint(baseView)
...
func TestTaintHuman_helpPrompt(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	baseView := NewView(streams)
	baseView.Configure(&arguments.View{NoColor: true})
	v := NewTaint(baseView)
	...
	got := done(t).Stderr()
	if !strings.Contains(got, "terraform taint -help") {
		t.Errorf("expected help prompt for taint, got: %q", got)
	}
}
