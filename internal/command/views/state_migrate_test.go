...
func TestNewStateMigrate_ReusingPreviousVersion_json(t *testing.T) {
	...
	p := addrs.MustParseProviderSourceString("hashicorp/test")
	v := versions.MustParseVersion("1.0.0")
	initView.LogReusingPreviousProviderVersion(p, v)
...
func TestNewStateMigrate_InitializingStateStoreProviderPlugin_json(t *testing.T) {
	...
	p := addrs.MustParseProviderSourceString("hashicorp/test")
	vc := getproviders.MustParseVersionConstraints("~> 1.1")
	storeType := "test_store"
	smView.LogInitializingStateStoreProviderStart(p, vc, storeType)

	// Assert output
	output := done(t)
	expectedOutputFields := []string{
		`"@level":"info"`,
		`"@message":"Initializing provider hashicorp/test (~\u003e 1.1) for state store \"test_store\"..."`,
		`"@module":"terraform.ui"`,
