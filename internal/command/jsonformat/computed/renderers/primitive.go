	...
	default:
		...
		processIndices := func(beforeIx, afterIx int) {
			if beforeIx < 0 || beforeIx >= len(beforeLines) {
				lines = append(lines, fmt.Sprintf("%s%s %s", formatIndent(indent+1), format.DiffActionSymbol(plans.Create), afterLines[afterIx]))
				return
			}

			if afterIx < 0 || afterIx >= len(afterLines) {
				lines = append(lines, fmt.Sprintf("%s%s %s", formatIndent(indent+1), format.DiffActionSymbol(plans.Delete), beforeLines[beforeIx]))
				return
			}
...
func (renderer primitiveRenderer) renderStringDiffAsJson(diff computed.Diff, indent int, opts computed.RenderHumanOpts, before evaluatedString, after evaluatedString) string {
	...
	if jsonDiff.Action == plans.NoOp && diff.Action == plans.Update {
		...
		// Whitespace changes should appear as if edited.
		action = plans.NoOp
	} else {
	...
	if strings.Contains(renderedJsonDiff, "\n") {
		return fmt.Sprintf("jsonencode(%s\n%s%s %s%s\n%s)", whitespace, formatIndent(indent+1), format.DiffActionSymbol(action), renderedJsonDiff, replace, formatIndent(indent+1))
	}
