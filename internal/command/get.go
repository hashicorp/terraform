...
func (c *GetCommand) Run(args []string) int {
	...
	abort, diags := getModules(&c.Meta, path, testsDirectory, update)
...
func getModules(m *Meta, path string, testsDir string, upgrade bool) (abort bool, diags tfdiags.Diagnostics) {
	hooks := uiModuleInstallHooks{
		Ui:             m.Ui,
		ShowLocalPaths: true,
	}
	return m.installModules(path, testsDir, upgrade, hooks)
}
