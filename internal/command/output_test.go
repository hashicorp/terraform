...
func TestOutput_backendWarningsSuppressed(t *testing.T) {
	...
	// The key assertion: backend deprecation warnings must not appear in output.
	// Before the fix, Backend() returned these warnings for non-PSS backends,
	// causing them to show up on every command (not just init).
	stderr := output.Stderr()
	if strings.Contains(stderr, "deprecated") {
		t.Fatalf("backend deprecation warnings should be suppressed for non-PSS backends, got:\n%s", stderr)
	}
