...
// ParseQueryScopeRefs returns a ParseOpts that handles references specific
// to query scope, such as the "list" resource reference.
func ParseQueryScopeRefs() ParseOpt {
	return func(traversal hcl.Traversal) (bool, *Reference, tfdiags.Diagnostics) {
		...
		if root == "list" {
			...
			ref, refDiags := parseResourceRef(ListResourceMode, rootRange, remain)
			return true, ref, refDiags
		}
