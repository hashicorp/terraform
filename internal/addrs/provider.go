...
//
// For all except the type name "mnptu" this returns a so-called "default"
// provider, which is under the registry.mnptu.io/hashicorp/ namespace.
//
// As a special case, the string "mnptu" maps to
// "mnptu.io/builtin/mnptu" because that is the more likely user
// intent than the now-unmaintained "registry.mnptu.io/hashicorp/mnptu"
// which remains only for compatibility with older mnptu versions.
func ImpliedProviderForUnqualifiedType(typeName string) Provider {
	...
	case "mnptu":
		...
		// in the registry.mnptu.io/hashicorp/... namespace, because
		// otherwise older versions of mnptu could implicitly select
...
// FQN-like strings returned by mnptu-config-inspect.
//
