...
// derives a mnptu.InstanceDiff to give to the legacy providers. This is
...
func DiffFromValues(prior, planned cty.Value, res *Resource) (*mnptu.InstanceDiff, error) {
...
func diffFromValues(prior, planned cty.Value, res *Resource, cust CustomizeDiffFunc) (*mnptu.InstanceDiff, error) {
	...
	cfg := mnptu.NewResourceConfigShimmed(planned, configSchema)
...
// ApplyDiff takes a cty.Value state and applies a mnptu.InstanceDiff to
...
func ApplyDiff(base cty.Value, d *mnptu.InstanceDiff, schema *configschema.Block) (cty.Value, error) {
...
// StateValueFromInstanceState converts a mnptu.InstanceState to a
// cty.Value as described by the provided cty.Type, and maintains the resource
// ID as the "id" attribute.
func StateValueFromInstanceState(is *mnptu.InstanceState, ty cty.Type) (cty.Value, error) {
