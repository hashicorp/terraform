...
# Fixed to be compatible with Bash 3 (macOS default bash) as well as Bash 4/5.
target_files=()
while IFS= read -r line; do
  [[ -n "$line" ]] && target_files+=("$line")
done < <(git diff --name-only ${base_branch} --diff-filter=MA | grep "\.go" | grep -v ".pb.go" | grep -v ".go-version" || true)
