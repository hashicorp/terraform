...
// GraphNodeExecutable
func (n *NodeApplyableOutput) Execute(ctx EvalContext) tfdiags.Diagnostics {
	...
	case walkEval, walkRefresh, walkPlan, walkApply, walkValidate, walkDestroy, walkPlanDestroy:
		...
		if diags.HasErrors() {
			if flagWarnOutputErrors {
				...
				return retDiags.Append(EvalEarlyExitError{})
			}
			return diags
		}
...
// GraphNodeExecutable
func (n *NodeDestroyableOutput) Execute(ctx EvalContext) tfdiags.Diagnostics {
	state := ctx.State()
	if state == nil {
		return nil
	}
...
func (n *NodeApplyableOutput) setValue(state *states.SyncState, changes *plans.ChangesSync, val cty.Value) {
	...
	if changes != nil {
		...
		log.Printf("[TRACE] EvalWriteOutput: Saving %s change for %s in changeset", change.Action, n.Addr)
