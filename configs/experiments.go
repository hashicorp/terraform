	...
	for _, expr := range exprs {
		...
		case experiments.UnavailableError:
			diags = diags.Append(&hcl.Diagnostic{
				...
				Detail:   fmt.Sprintf("There is no available experiment with the keyword %q.", kw),
				Subject:  expr.Range().Ptr(),
			})
		case experiments.DefunctError:
			diags = diags.Append(&hcl.Diagnostic{
				Severity: hcl.DiagError,
				Summary:  "Experiment is defunct",
				Detail:   fmt.Sprintf("Experiment %q is no longer available: %s.", kw, err.Error()),
				Subject:  expr.Range().Ptr(),
			})
		case nil:
