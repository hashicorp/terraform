...
// resourceAsmStorageContainer returns the *schema.Resource associated
// to a storage container on Azure.
func resourceAsmStorageContainer() *schema.Resource {
	return &schema.Resource{
		Create: resourceAsmStorageContainerCreate,
		Read:   resourceAsmStorageContainerRead,
		Exists: resourceAsmStorageContainerExists,
		Delete: resourceAsmStorageContainerDelete,
...
// resourceAsmStorageContainerCreate does all the necessary API calls to
// create the storage container on Azure.
func resourceAsmStorageContainerCreate(d *schema.ResourceData, meta interface{}) error {
	...
	d.SetId(name)
	return resourceAsmStorageContainerRead(d, meta)
}

// resourceAsmStorageContainerRead does all the necessary API calls to
// read the status of the storage container off Azure.
func resourceAsmStorageContainerRead(d *schema.ResourceData, meta interface{}) error {
	azureClient := meta.(*AzureClient)
	storName := d.Get("storage_service_name").(string)
...
// resourceAsmStorageContainerExists does all the necessary API calls to
// check if the storage container already exists on Azure.
func resourceAsmStorageContainerExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	azureClient := meta.(*AzureClient)
	storName := d.Get("storage_service_name").(string)
...
// resourceAsmStorageContainerDelete does all the necessary API calls to
// delete a storage container off Azure.
func resourceAsmStorageContainerDelete(d *schema.ResourceData, meta interface{}) error {
	azureClient := meta.(*AzureClient)
	storName := d.Get("storage_service_name").(string)
