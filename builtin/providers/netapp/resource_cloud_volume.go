...
func processVolumeResourceData(d *schema.ResourceData, res *workenv.VolumeResponse) error {
  d.Set("name", res.Name)
...
func flattenShareInfo(info *workenv.CIFSShareInfo) []interface{} {
...
func resourceCloudVolumeCreate(d *schema.ResourceData, meta interface{}) error {
  ...
  // if no aggregate was provided, use one from the quote response
  // and set it as resource data
  if _, ok := d.GetOk("aggregate_name"); !ok {
    d.Set("aggregate_name", quoteRes.AggregateName)
  }

  // if no max volumes was provided, use one from the quote response
  // and set it as resource data
  if _, ok := d.GetOk("max_num_disks_approved_to_add"); !ok {
    d.Set("max_num_disks_approved_to_add", quoteRes.NumOfDisks)
  }
...
func resourceCloudVolumeRead(d *schema.ResourceData, meta interface{}) error {
  ...
  processVolumeResourceData(d, res)
...
func resourceCloudVolumeUpdate(d *schema.ResourceData, meta interface{}) error {
  ...
  if volumeTierChanged {
    err = updateVolumeTier(d, apis, volumeType, workenvId, svmName, volumeName, isHA)
    if err != nil {
      return err
    }
...
func updateVolumeTier(d *schema.ResourceData, apis *APIs, volumeType, workenvId, svmName, volumeName string, isHA bool) error {
