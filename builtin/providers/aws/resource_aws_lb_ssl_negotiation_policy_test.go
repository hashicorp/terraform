...
func TestAccAWSLBSSLNegotiationPolicy_updateParam(t *testing.T) {
	lbName := fmt.Sprintf("tf-test-lb-%s", acctest.RandString(5))
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { testAccPreCheck(t) },
		Providers:    testAccProviders,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSslNegotiationPolicyConfig(
					fmt.Sprintf("tf-acctest-%s", acctest.RandString(10)), lbName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBSSLNegotiationPolicy(
						"aws_elb.lb",
						"aws_lb_ssl_negotiation_policy.foo",
			...
			{
				Config: testAccSslNegotiationPolicyConfigUpdatedVariable(
					fmt.Sprintf("tf-acctest-%s", acctest.RandString(10)), lbName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBSSLNegotiationPolicy(
						"aws_elb.lb",
...
func testAccSslNegotiationPolicyConfigUpdatedVariable(certName string, lbName string) string {
	return fmt.Sprintf(`
resource "aws_iam_server_certificate" "test_cert" {
  name = "%s"
  certificate_body = <<EOF
