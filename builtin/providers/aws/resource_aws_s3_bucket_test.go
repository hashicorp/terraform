		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("aws_s3_bucket.bucket", "replication_configuration.0.rules.0.id", "foobar"),
					resource.TestCheckResourceAttr("aws_s3_bucket.bucket", "replication_configuration.0.rules.0.prefix", "foo"),
					resource.TestCheckResourceAttr("aws_s3_bucket.bucket", "replication_configuration.0.rules.0.status", s3.ReplicationRuleStatusEnabled),
				),
			},
		},
	})
