...
// AWS AMI description resource - use to get AMI data, kind of like
// describe-instances in the CLI.
//
// Example:
//
...
//
// Id - the AMI returned by this resource.
// Tons of other attributes - populated from DescribeImagesOutput.
//
// NOTE: if more or less than a single match is returned by the search,
// Terraform will fail. Ensure that your search is specific enough to return
// a single AMI ID only, or use most_recent to choose the most recent one.

func resourceAwsAmiDescription() *schema.Resource {
	return &schema.Resource{
		Create: resourceAwsAmiDescriptionRead,
		Read:   resourceAwsAmiDescriptionRead,
		Delete: resourceAwsAmiDescriptionDelete,
...
// Delete the "Resource".
func resourceAwsAmiDescriptionDelete(d *schema.ResourceData, meta interface{}) error {
	d.SetId("")
	return nil
}

// Perform the AMI lookup.
func resourceAwsAmiDescriptionRead(d *schema.ResourceData, meta interface{}) error {
	...
	if len(resp.Images) < 1 {
		...
		if (d.Get("most_recent").(bool)) == true {
			log.Printf("[DEBUG] aws_ami_description - multiple results found and most_recent is set")
			image = mostRecentAmi(resp.Images)
		} else {
			log.Printf("[DEBUG] aws_ami_description - multiple results found and most_recent not set")
			return fmt.Errorf("Your query returned more than one result. Please try a more specific search, or set most_recent to true.")
		}
	} else {
		log.Printf("[DEBUG] aws_ami_description - Single AMI found: %s", *resp.Images[0].ImageId)
		image = resp.Images[0]
	}
...
func amiBlockDeviceMappings(m []*ec2.BlockDeviceMapping) *schema.Set {
	...
	for _, v := range m {
		...
		log.Printf("[DEBUG] aws_ami_description - adding block device mapping: %v", mapping)
		s.Add(mapping)
	}
	return s
}
