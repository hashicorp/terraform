...
func TestAccAzureRMNetworkInterface_primaryNetworkInterface(t *testing.T) {
	rInt := acctest.RandInt()
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { testAccPreCheck(t) },
		Providers:    testAccProviders,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAzureRMNetworkInterface_primaryNetworkInterface(rInt),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMNetworkInterfaceExists("azurerm_network_interface.test1"),
					resource.TestCheckResourceAttr("azurerm_network_interface.test1", "ip_configuration[0].primary", "true"),
					resource.TestCheckResourceAttr("azurerm_network_interface.test1", "ip_configuration[1].primary", "false"),
				),
			},
		},
	})
...
func testAccAzureRMNetworkInterface_primaryNetworkInterface(rInt int) string {
	return fmt.Sprintf(`
	resource "azurerm_resource_group" "test" {
		name = "acctest-rg-%d"
		location = "West US"
