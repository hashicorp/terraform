...
func TestAccScalewayServer_Volumes(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccCheckScalewayServerConfig_volumes,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckScalewayServerExists("scaleway_server.base"),
					testAccCheckScalewayServerVolumes("scaleway_server.base"),
					resource.TestCheckResourceAttr(
...
func testAccCheckScalewayServerVolumes(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]
		if !ok {
			return fmt.Errorf("Unknown resource: %s", n)
		...
		if len(server.Volumes) != 2 {
			return fmt.Errorf("Wrong number of volumes: expected 2, got %d", len(server.Volumes))
		}
...
var testAccCheckScalewayServerConfig_volumes = fmt.Sprintf(`
resource "scaleway_server" "base" {
  name = "test"
  # ubuntu 14.04
  image = "%s"
