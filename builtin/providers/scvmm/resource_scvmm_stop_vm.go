...
func resourceSCVMMStopVM() *schema.Resource {
	return &schema.Resource{
		Create: resourceStopVM,
		Read:   resourceStopVMRead,
		Delete: resourceStopVMDelete,
...
func resourceStopVM(d *schema.ResourceData, meta interface{}) error {
	...
	d.SetId("stop_" + vmmServer + "_" + vmName)
	return nil
}

func resourceStopVMRead(d *schema.ResourceData, meta interface{}) error {
	connection := meta.(*winrm.Client)
	vmmServer := d.Get("vmm_server").(string)
	vmName := d.Get("vm_name").(string)
	...
	if err != "" {
		d.SetId("")
		log.Printf("[Error] Virtual Machine is not in Stop State: %s", err)
	}
	return nil
}

func resourceStopVMDelete(d *schema.ResourceData, meta interface{}) error {
	connection := meta.(*winrm.Client)
	vmmServer := d.Get("vmm_server").(string)
	vmName := d.Get("vm_name").(string)
	timeout := d.Get("timeout").(string)
