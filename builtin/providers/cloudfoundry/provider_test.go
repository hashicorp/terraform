...
func testAccPreCheck(t *testing.T) {
	...
	if endpoint == "" ||
		...
		uaaClientSecret == "" ||
		skipSslValidation == "" {
		t.Fatal("CF_API_URL, CF_USER, CF_PASSWORD, CF_UAA_CLIENT_ID, CF_UAA_CLIENT_SECRET " +
			"and CF_SKIP_SSL_VALIDATION must be set for acceptance tests to work.")
	}

	// _, err := cfapi.NewSession(apiURL, user, password, uaaClientID, uaaClientSecret, caCert, skipSslValidation == "true")
	// if err != nil {
	// 	t.Errorf("Failed to connect to Cloud Foundry: %s", err.Error())
	// }
}
