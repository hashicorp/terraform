...
func TestAccQuota_normal(t *testing.T) {
	...
	resource.Test(t,
		resource.TestCase{
			...
			CheckDestroy: testAccCheckQuotaResourceDestroy,
...
func TestAccSpaceQuota_normal(t *testing.T) {
	...
	resource.Test(t,
		resource.TestCase{
			PreCheck:     func() { testAccPreCheck(t) },
			Providers:    testAccProviders,
			CheckDestroy: testAccCheckSpaceQuotaResourceDestroy,
			Steps: []resource.TestStep{
...
func testAccCheckQuotaResourceDestroy(s *terraform.State) error {

	session := testAccProvider.Meta().(*cfapi.Session)
	if _, err := session.QuotaManager().FindQuota("50g"); err != nil {
		switch err.(type) {
	...
	return fmt.Errorf("quota with name '50g' still exists in cloud foundry")
}

func testAccCheckSpaceQuotaResourceDestroy(s *terraform.State) error {

	session := testAccProvider.Meta().(*cfapi.Session)
	if _, err := session.QuotaManager().FindSpaceQuota("10g", defaultPcfDevOrgID()); err != nil {
		switch err.(type) {
		case *errors.ModelNotFoundError:
			return nil
		default:
	...
	return fmt.Errorf("space quota with name '10g' still exists in cloud foundry")
}
