...
func TestResourceProvisioner_windowsIsPuppetEnterprise(t *testing.T) {
	cases := map[string]struct {
		Config         map[string]interface{}
		CommandFunc    func(*remote.Cmd) error
		ExpectedError  bool
		...
		"When the web request prints an unparsable number to stdout": {
			Config: map[string]interface{}{
				"server":   "puppet.test.com",
				"use_sudo": false,
			},
		...
		"When the web request prints a 2xx status to stdout": {
			Config: map[string]interface{}{
				"server":   "puppet.test.com",
				"use_sudo": false,
			},
		...
		"When the web request prints a 4xx status to stdout": {
			Config: map[string]interface{}{
				"server":   "puppet.test.com",
				"use_sudo": false,
			},
	...
	for k, tc := range cases {
		...
		result, err := p.windowsIsPuppetEnterprise()
		if tc.ExpectedError {
			if err == nil {
				t.Fatalf("Test %q failed: expected error but no error returned", k)
			}
		} else {
			if err != nil {
				t.Fatalf("Test %q failed: %v", k, err)
