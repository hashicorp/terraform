...
if ($defaultBaseURL != $baseURL) {
	...
	$url = "$defaultBaseURL/%s/chef/download?p=windows&pv=$machine_os&m=$machine_arch&v=%s"
}
...
func (p *provisioner) windowsInstallChefClient(o terraform.UIOutput, comm communicator.Communicator) error {
	...
	content := fmt.Sprintf(installScript, defaultBaseInstallURL, p.InstallURL, p.Channel, p.Version, p.HTTPProxy, strings.Join(p.NOProxy, ","))
