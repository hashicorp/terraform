	...
	for name, blockS := range schema.BlockTypes {
		...
		case configschema.NestingList:
			...
			for it := plannedV.ElementIterator(); it.Next(); {
				...
				moreErrs := assertPlanValid(&blockS.Block, priorEV, configEV, plannedEV, append(path, cty.IndexStep{Key: idx}))
				errs = append(errs, moreErrs...)
			}
		case configschema.NestingMap:
			...
			if plannedV.Type().IsObjectType() {
				...
				for k := range plannedAtys {
					...
					moreErrs := assertPlanValid(&blockS.Block, priorEV, configEV, plannedEV, append(path, cty.GetAttrStep{Name: k}))
					errs = append(errs, moreErrs...)
				}
